...
void test_foo_1(float fv, double dv, float _Complex fc, double _Complex dc) {
  ...
  char *cp3 = foo(fc); // expected-error {{implicit conversion discards imaginary component: '_Complex float' to 'float'}}
  char *cp4 = foo(dc); // expected-error {{implicit conversion discards imaginary component: '_Complex double' to 'float'}}
}
