...
void testA() {
  ...
  *p = 1;
  ...
  // CHECK-LABEL: text-diagnostics.c:{{.*}}:6: clang_sa_warning: Dereference of null pointer (loaded from variable 'p')
...
void testB(int *q) {
  ...
  *q = 1;
  ...
  // CHECK-LABEL: text-diagnostics.c:{{.*}}:6: clang_sa_warning: Dereference of null pointer (loaded from variable 'q')
