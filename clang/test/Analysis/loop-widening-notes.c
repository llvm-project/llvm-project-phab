...
int test_for_bug_25609() {
  if (p_a == 0) // expected-note {{Assuming 'p_a' is equal to null}} 
    ...
    bar();
  for (int i = 0; i < flag_a; ++i) {} // expected-note {{Loop condition is true.  Entering loop body}} 
                                      // expected-note@-1 {{Value assigned to 'p_a'}} 
                                      // expected-note@-2 {{Loop condition is false. Execution continues on line 13}}
...
int while_analyzer_output() {
  ...
  while (flag_b-- > 0) { // expected-note {{Loop condition is true.  Entering loop body}} 
                         ...
                         // expected-note@-2 {{Loop condition is false. Execution continues on line 27}}
    num = flag_b;
  }
...
int do_while_analyzer_output() {
  int num = 10;
  do {   // expected-note {{Loop condition is true. Execution continues on line 44}} 
...
int test_for_loop() {
  ...
  for (int i = 0; // expected-note {{Loop condition is true.  Entering loop body}} 
                  // expected-note@-1 {{Loop condition is false. Execution continues on line 65}}
