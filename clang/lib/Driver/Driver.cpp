...
static bool searchForFile(SmallVectorImpl<char> &FilePath,
  ...
  for (const StringRef &Dir : Dirs) {
    assert(!Dir.empty() && "Directory path should not be empty");
    WPath.clear();
...
bool Driver::loadConfigFile() {
  ...
  SmallVector<StringRef, 3> CfgFileSearchDirs;
#if defined(CLANG_CONFIG_FILE_USER_DIR)
  CfgFileSearchDirs.push_back(CLANG_CONFIG_FILE_USER_DIR);
#endif
#if defined(CLANG_CONFIG_FILE_SYSTEM_DIR)
  CfgFileSearchDirs.push_back(CLANG_CONFIG_FILE_SYSTEM_DIR);
#endif
