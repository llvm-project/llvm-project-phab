...
static bool ParseCodeGenArgs(CodeGenOptions &Opts, ArgList &Args, InputKind IK,
  ...
  if (Arg *A = Args.getLastArg(OPT_fclang_abi_compat_EQ)) {
    ...
    if (!VerParts.first.startswith("0") &&
      ...
      if (Major == 3 && Minor <= 2)
        Opts.setClangABICompat(CodeGenOptions::ClangABI::Ver3_2);
  ...
  // On PS4, the platform ABI is "whatever Clang 3.2 did", so a newer
  // ABI version does not make sense.
  if (Triple.isPS4() &&
      Opts.getClangABICompat() > CodeGenOptions::ClangABI::Ver3_2)
    Opts.setClangABICompat(CodeGenOptions::ClangABI::Ver3_2);
