...
public:
  ...
  RecordByFileTy::const_iterator
  record_begin() const { return RecordByFile.begin(); }

  RecordByFileTy::const_iterator
  record_end() const { return RecordByFile.end(); }
...
public:
  ...
  virtual void attachToPreprocessor(Preprocessor &PP) override {
    ...
    PP.addPPCallbacks(
      llvm::make_unique<IncludePPCallbacks>(IndexCtx, RecordOpts, Includes,
                                            PP.getSourceManager()));
  }
  ...
  void visitIncludes(llvm::function_ref<void(const FileEntry *Source,
                                             unsigned Line,
                                             const FileEntry *Target)> visitor)
      override {
...
void IndexRecordActionBase::finish(CompilerInstance &CI) {
  ...
  if (IndexUnitWriter::initIndexDirectory(DataPath, Error)) {
    Diag.Report(diag::err_index_store_dir_create_failed) << Error;
    return;
  }
...
static void writeUnitData(const CompilerInstance &CI,
  ...
  std::string ModuleName = UnitModule ?
    UnitModule->getFullModuleName() : std::string();

  auto getModuleInfo = [](writer::OpaqueModule mod,
  ...
  DepProvider.visitFileDependencies(CI, [&](const FileEntry *FE,
                                            bool isSystemFile) {
    UnitWriter.addFileDependency(FE, isSystemFile, findModuleForHeader(FE));
  });
  DepProvider.visitIncludes([&](const FileEntry *Source, unsigned Line,
                                const FileEntry *Target) {
  ...
  for (auto I = Recorder.record_begin(), E = Recorder.record_end();
       I != E; ++I) {
    FileID FID = I->first;
    ...
    if (RecordWriter.writeRecord(FE->getName(), Rec, Error, &RecordFile)) {
      Diag.Report(diag::err_index_store_record_write_failed) << RecordFile
                                                             << Error;
      return;
    }
...
createIndexDataRecordingAction(IndexingOptions IndexOpts,
  ...
  return llvm::make_unique<IndexRecordAction>(
    std::move(IndexOpts), std::move(RecordOpts));
}
