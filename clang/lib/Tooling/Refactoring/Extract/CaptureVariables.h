...
class CapturedEntity {
  /// The kind of the entity that is captured by the extracted function.
  enum EntityKind {
    /// An entity that represents a local variable defined in the callee
    /// that is captured by the extracted function.
    CapturedVarDecl,

    // FIXME: Add entity kinds for FieldDecl, this and self.
  };

public:
  /// Creates a local variable capture entity.
  explicit CapturedEntity(const VarDecl *VD) : Kind(CapturedVarDecl), VD(VD) {}
