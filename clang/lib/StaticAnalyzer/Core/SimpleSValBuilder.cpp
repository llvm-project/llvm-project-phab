      ...
      if (BinaryOperator::isComparisonOp(op) &&
        ...
        if (Sym->getType() == rSym->getType()) {
          ...
          if (T->isUnsignedIntegerOrEnumerationType ()) {
            T = Context.getCorrespondingSignedType(T);
          }
          ...
          if (const SymIntExpr *lSymIntExpr = dyn_cast<SymIntExpr>(Sym)) {
            ...
            if (BinaryOperator::isAdditiveOp(lop)) {
              lInt = &BasicVals.Convert(T, lSymIntExpr->getRHS());
          ...
          if (const SymIntExpr *rSymIntExpr = dyn_cast<SymIntExpr>(rSym)) {
            ...
            if (BinaryOperator::isAdditiveOp(rop)) {
              rInt = &BasicVals.Convert(T, rSymIntExpr->getRHS());
          ...
          if (Sym == rSym) {
            return nonloc::ConcreteInt(*BasicVals.evalAPSInt(
                op, BasicVals.getValue(0, T), *newRhs));
          } else {
            if (reverse) {
              op = BinaryOperator::reverseComparisonOp(op);
            }
            const SymExpr *newLhs =
                reverse
                    ? SymMgr.getSymSymExpr(rSym, BO_Sub, Sym, T)
                    : SymMgr.getSymSymExpr(Sym, BO_Sub, rSym, T);
