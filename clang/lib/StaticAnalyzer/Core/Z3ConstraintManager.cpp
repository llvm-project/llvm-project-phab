...
private:
  ...
  const llvm::APSInt &fixAPSInt(const llvm::APSInt &Int,
...
ProgramStateRef Z3ConstraintManager::assumeSymInclusiveRange(
  ...
  QualType LTy;
  Z3Expr FromExp = Z3Expr::fromAPSInt(fixAPSInt(From, &LTy));
  ...
  QualType RTy;
  Z3Expr ToExp = Z3Expr::fromAPSInt(fixAPSInt(To, &RTy));
...
Z3Expr Z3ConstraintManager::getZ3SymBinExpr(const BinarySymExpr *BSE,
  ...
  if (const SymIntExpr *SIE = dyn_cast<SymIntExpr>(BSE)) {
    ...
    Z3Expr RHS = Z3Expr::fromAPSInt(fixAPSInt(SIE->getRHS(), &RTy));
    return getZ3BinExpr(LHS, LTy, Op, RHS, RTy, RetTy);
  } else if (const IntSymExpr *ISE = dyn_cast<IntSymExpr>(BSE)) {
    Z3Expr LHS = Z3Expr::fromAPSInt(fixAPSInt(ISE->getLHS(), &LTy));
...
const llvm::APSInt &Z3ConstraintManager::fixAPSInt(const llvm::APSInt &Int,
  ...
  if (Int.getBitWidth() == 1 && getAPSIntType(Int).isNull()) {
    BasicValueFactory &BVF = getBasicVals();
    ASTContext &Ctx = BVF.getContext();
    const llvm::APSInt &New =
        BVF.getValue(Int.extend(Ctx.getTypeSize(Ctx.BoolTy)));
    if (Ty)
      *Ty = getAPSIntType(New);
    return New;
  }
