...
private:
  ...
  const ProgramStateRef programState;
  SValBuilder &sValBuilder;

public:
  ConstraintBasedEQEvaluator(CheckerContext &C,
                             const DefinedOrUnknownSVal compareValue)
      : compareValue(compareValue), programState(C.getState()),
        sValBuilder(C.getSValBuilder()) {}

  bool operator()(const llvm::APSInt &enumDeclInitValue) {
    DefinedOrUnknownSVal enumDeclValue =
        sValBuilder.makeIntVal(enumDeclInitValue);
    const auto ElemEqualsValueToCast =
        sValBuilder.evalEQ(programState, enumDeclValue, compareValue);

    return static_cast<bool>(programState->assume(ElemEqualsValueToCast, true));
  }
};
