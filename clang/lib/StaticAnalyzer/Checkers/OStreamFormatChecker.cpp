...
static Optional<int> tryEvaluateAsInt(const Expr *E, ProgramStateRef S,
  ...
  Optional<nonloc::ConcreteInt> NonLocConcreteOptional =
      S->getSVal(E, C.getLocationContext()).getAs<nonloc::ConcreteInt>();
  if (NonLocConcreteOptional)
    return (*NonLocConcreteOptional).getValue().getExtValue();

  Optional<loc::ConcreteInt> LocConcreteOptional =
      S->getSVal(E, C.getLocationContext()).getAs<loc::ConcreteInt>();
  if (LocConcreteOptional)
    return (*LocConcreteOptional).getValue().getExtValue();
