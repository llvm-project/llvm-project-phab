...
bool OStreamFormatChecker::evalCall(const CallExpr *CE,
  ...
  SVal InStream = CurrentState->getSVal(CE->getArg(0), C.getLocationContext());
  CurrentState = CurrentState->BindExpr(CE, C.getLocationContext(), InStream);
  ...
  // Add the SVal of GlobalsRegion and the StreamObject.
  SmallVector<SVal, 1> ValuesToInvalidate{StreamSV};
  RegionAndSymbolInvalidationTraits ETraits;

  int BlockCount = C.blockCount();

  ProgramStateRef RegionsInvalidated =
