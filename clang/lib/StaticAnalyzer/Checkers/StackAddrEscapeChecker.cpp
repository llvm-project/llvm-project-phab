...
private:
  ...
  static bool isInAncestorFrame(const MemRegion *R, CheckerContext &C);
  static bool isSemaphoreCaptured(const BlockDecl &B);
};
} // namespace
...
SourceRange StackAddrEscapeChecker::genName(raw_ostream &os, const MemRegion *R,
  ...
  } else if (const CXXTempObjectRegion *TOR =
...
bool StackAddrEscapeChecker::isInAncestorFrame(const MemRegion *R,
                                               CheckerContext &C) {
...
bool StackAddrEscapeChecker::isSemaphoreCaptured(const BlockDecl &B) {
  for (const auto &C : B.captures()) {
    ...
    if (Q->isObjCObjectPointerType() &&
        Q->getPointeeType()
                .getCanonicalType()
                .getUnqualifiedType()
                .getAsString() == "NSObject<OS_dispatch_semaphore>")
      return true;
  }
  return false;
}
...
void StackAddrEscapeChecker::checkReturnedBlockCaptures(
    const BlockDataRegion &B, CheckerContext &C) const {
  for (const MemRegion *Region : getCapturedStackRegions(B, C)) {
    if (isArcManagedBlock(Region, C) || isInAncestorFrame(Region, C))
      continue;
    ExplodedNode *N = C.generateNonFatalErrorNode();
    if (!N)
      continue;
...
void StackAddrEscapeChecker::checkPreStmt(const ReturnStmt *RS,
  ...
  if (!isa<StackSpaceRegion>(R->getMemorySpace()) || isInAncestorFrame(R, C) ||
      isArcManagedBlock(R, C))
    return;
  ...
  public:
    ...
    bool HandleBinding(StoreManager &SMgr, Store S, const MemRegion *Region,
      ...
      if (VR && isa<StackSpaceRegion>(VR->getMemorySpace()) &&
          !isArcManagedBlock(VR, Ctx) && !isInAncestorFrame(VR, Ctx))
