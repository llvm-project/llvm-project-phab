...
// following the 'define' keyword in a PP define directive
static bool endsMacroIdentifier(const FormatToken *Current,
                                bool SpacesRequiredBefore) {
...
static bool endsMacroWithArgsIdentifier(const FormatToken *Current) {
  const FormatToken *Param = Current->Previous;
  ...
  return Param && endsMacroIdentifier(Param->Previous, false);
}

static bool endsPPIdentifier(const FormatToken *Current) {
  return endsMacroIdentifier(Current, true) ||
         endsMacroWithArgsIdentifier(Current);
}
