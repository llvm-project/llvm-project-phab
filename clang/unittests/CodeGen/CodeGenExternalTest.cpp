  ...
  for (auto decl : my->toplevel_decls ) {
    if (FunctionDecl *fd = dyn_cast<FunctionDecl>(decl)) {
      ...
      if (rd->getName() == "mytest_struct") {
        ...
        // Generate the type of a pointer to the llvmTy.
        //llvm::Type* ptrTy = llvmTy->getPointerTo(0);
        //ASSERT_TRUE(ptrTy != NULL);

        // Check getFieldNumber
        ...
        unsigned x = CodeGen::getFieldNumber(CGM, rd, xField);
        unsigned y = CodeGen::getFieldNumber(CGM, rd, yField);
        unsigned z = CodeGen::getFieldNumber(CGM, rd, zField);
