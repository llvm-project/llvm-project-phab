...
/// ----------------------------------------------------------------------------
/// j-loop   | []  -> { []       -> [(1)] }
/// i-loop   | [i] -> { [l0]     -> [(1)] : 0 <= i0 < i }
/// max/none | [N] -> { [l0, l1] -> [(1)] : 0 <= i1 < i0 and i0 < N }
///
///            /\       /\           /\         /\
///            ||       ||           ||         ||
...
public:
  ...
  /// Assign the combination of @p PE0 and @p PE1 using @p Combinator.
  PEXP *assign(const PEXP *PE0, const PEXP *PE1,
...
private:
  ...
  /// Combine the invalid and known domain of @p PE with this PEXP.
  ///
  /// @param PE The polyhedral representation to combine.
  PEXP *combine(const PEXP *PE);
  ...
  PEXP *combine(const PEXP *PE, PVAff::CombinatorFn Combinator,
                const PVSet *Domain = nullptr);
  PEXP *combine(const PEXP *PE, PVAff::IslCombinatorFn Combinator,
                const PVSet *Domain = nullptr);
...
class PolyhedralValueInfoCache final {
  ...
  /// Mapping from basic blocks to their domain expressed as a PEXP.
  using DomainMapKey = std::pair<BasicBlock *, Loop *>;
  DenseMap<DomainMapKey, PEXP *> DomainMap;

  /// Mapping from values to their polyhedral representation.
...
public:
  ~PolyhedralValueInfoCache() { releaseMemory(); }
...
public:
  ...
  /// @p Scope. Thus @p PE is not parametric in an instruction in @p Scope.
  bool hasScope(const PEXP *PE, Loop *Scope) const;

  /// Return true if @p PE represents a value that is fixed for one function
  /// invocation, thus @p PE is not parametric in any llvm::Instruction.
  bool hasFunctionScope(const PEXP *PE) const;
