...
//
// Caveats (fix me!):
//  - integers could be embedded directly in the ValueRep
...
// Value is the public representation of a generic JSON value.
// It's not directly copyable or movable, users should take references to it.
//
// It has a pointer into the document's allocator where the data is stored,
// knows what type the data is, and runs its destructor in ~Value().
// However it doesn't know about the allocator in use, and doesn't free memory.
class Value {
 public:
  Value(Value &&) = delete;
  Value(Value &) = delete;
...
class Document {
 public:
  ...
  Document &operator=(Document &&Move) {
    // Default implementation would delete the allocator, then the Value!
    Root.~ValueWrapper();
...
class Array {
  using Rep = SmallVector<detail::ValueWrapper, 2>;
  Rep Elems;
  detail::Arena &Alloc;
 ...
 public:
  ...
  void push_back(const Literal &V) { Elems.emplace_back(V.construct(Alloc)); }
  ...
  class iterator : public iterator_adaptor_base<iterator, Rep::iterator,
                                                std::random_access_iterator_tag,
                                                value_type> {
   public:
    iterator() = default;
    explicit iterator(Rep::iterator I) : iterator_adaptor_base(std::move(I)) {}
    value_type &operator*() { return *this->wrapped(); }
  };

  class const_iterator
      : public iterator_adaptor_base<const_iterator, Rep::const_iterator,
                                     std::random_access_iterator_tag,
                                     const value_type> {
   public:
    const_iterator() = default;
    explicit const_iterator(Rep::const_iterator I)
        : iterator_adaptor_base(std::move(I)) {}
    const value_type &operator*() { return *this->wrapped(); }
  };
};

class Object {
  using Rep = StringMap<detail::ValueWrapper, detail::Arena &>;
  Rep Props;
 ...
 public:
  ...
  void set(StringRef K, const Literal &V) {
    ...
    if (!R.second) {
      R.first->second.~ValueWrapper();
      new (&R.first->second)
          detail::ValueWrapper(V.construct(Props.getAllocator()));
    }
  }
  ...
  class iterator
      : public iterator_adaptor_base<iterator, Rep::iterator,
