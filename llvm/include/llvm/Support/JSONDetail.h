...
template <>
struct PointerLikeTypeTraits<json::Object *> {
...
template <>
struct PointerLikeTypeTraits<json::Array *> {
...
// ValeWrapper wraps Value and adds movability for use in internal containers.
enum ValueType {
  VT_Sentinel = 0,  // true, false, null
  VT_SmallInt,      // 53-bit integer embedded in the ValueRep
  VT_Double,
  VT_Array,
  VT_Object,
  VT_OwnedString,
...
using SmallInt = PointerEmbeddedInt<int64_t, 53>;
...
class Literal {
 public:
  ...
  template <int N>
  Literal(const char (&V)[N]) : Type(LT_StaticString) {
    assert(strlen(V) == N - 1 &&
  ...
  template <int N>
  Literal(char (&V)[N]) : Literal((const char *)V) {}
  ...
  Literal(T V) {
    if (V == SmallInt(V)) {
      Type = LT_Immediate;
      R.Immediate = ValueRep::create<VT_SmallInt>(SmallInt(V));
    } else {
      Type = LT_Double;
      R.Double = V;
    }
