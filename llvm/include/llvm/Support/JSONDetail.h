...
class Literal {
public:
  // This moves from a *const* rvalue reference! (all members are mutable)
  // This allows moving rather than copying out of initializer_list<Literal>.
  // We lose the compiler's sanity-checking and the ability to usefully overload
  // on const vs non-const Literals, luckily we need neither.
  Literal(const Literal &&);
  ~Literal();

  // Strings: std::string, SmallString, char* (by value).
  // StringRef, Twine, (by reference).
  // char literals (by reference, and never copied into the document).
  Literal(std::string &&V) : Type(LT_String) {
    new (&R.String) std::string(std::move(V));
  }
  Literal(const std::string &V) : Type(LT_String) {
    new (&R.String) std::string(V);
  }
  // We don't actually move SmallString/formatv, but copy to get value semantics.
  Literal(const SmallVectorImpl<char> &V) : Type(LT_String) {
    new (&R.String) std::string(V.data(), V.size());
  }
  Literal(const formatv_object_base &V) : Type(LT_String) {
    new (&R.String) std::string(V.str());
  }
...
private:
  ...
  Literal(StringMap<Literal> &&Object, LiteralType Dummy) : Type(LT_Object) {
    new (&R.Object) StringMap<Literal>(std::move(Object));
  }
  friend obj;
  Literal(std::vector<Literal> &&Array, LiteralType Dummy) : Type(LT_Array) {
    new (&R.Array) std::vector<Literal>(std::move(Array));
  }
  friend ary;
};
