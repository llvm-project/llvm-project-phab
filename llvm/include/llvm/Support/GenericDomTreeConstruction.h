...
// For postdominators and without further information, nodes with no forward
// successors are trivial roots that are always selected as tree roots. Roots
// with forward successors correspond to CFG nodes within infinite loops and
// cannot be identified easily as exit nodes of the graph, but must be found
// via a DFS search.
//
// By specializing this function, it is possible to further refine the set of
// forward successor free roots, e.g., by distinguishing between real exit
// nodes and unreachable nodes.
template <typename NodePtr>
struct ExitChecker {
  static bool IsGraphExit(const NodePtr N) {
  assert(N && "N must be a valid node");
  using TraitsTy = GraphTraits<NodePtr>;
...
struct SemiNCAInfo {
  ...
  // The DFS number after which only "unreachable" nodes are processed.
  unsigned UnreachableMinDFS;
  ...
  void runSemiNCA(DomTreeT &DT, const unsigned MinLevel = 0) {
    ...
    for (unsigned i = NextDFSNum - 1; i >= 2; --i) {
      ...
      for (const auto &N : WInfo.ReverseChildren) {
        ...
        auto &NInfo = NodeToInfo[eval(N, i + 1)];

        if (NInfo.DFSNum >= UnreachableMinDFS &&
            WInfo.DFSNum < UnreachableMinDFS)
          continue;

        unsigned SemiU = NInfo.Semi;
  ...
  static bool IsGraphExit(const NodePtr N) {
    return ExitChecker<NodePtr>::IsGraphExit(N);
  }
  ...
  static RootsT FindRoots(const DomTreeT &DT,
                          unsigned *UnreachableMinDFS = nullptr) {
    ...
    if (!IsPostDom) {
      ...
      if (UnreachableMinDFS)
        *UnreachableMinDFS = std::numeric_limits<unsigned>::max();
      return Roots;
    }
  ...
  void calculateFromScratch(DomTreeT &DT) {
    ...
    DT.Roots = FindRoots(DT, &UnreachableMinDFS);
  ...
  static void UpdateRootsAfterUpdate(DomTreeT &DT) {
    ...
    if (std::all_of(DT.Roots.begin(), DT.Roots.end(), IsGraphExit))
      return;
  ...
  bool verifyParentProperty(const DomTreeT &DT) {
    ...
    for (auto &NodeToTN : DT.DomTreeNodes) {
      ...
      DEBUG(dbgs() << "Verifying parrent property of node "
                   << BlockNamePrinter(TN) << "\n");
      clear();
      doFullDFSWalk(DT, [BB, Roots](NodePtr From, NodePtr To) {
        // Exclude anything that is backwards unreachable.
        for (auto Root : Roots)
          if (From == Root && !IsGraphExit(Root))
            return false;
