...
class MachineIRBuilder {
  ...
  template <typename DstTy, typename UseArg1>
  MachineInstrBuilder buildInstrImpl(unsigned Opc, DstTy &&DstArg,
                                     UseArg1 &&Arg1) {
    switch (Opc) {
    case TargetOpcode::G_ZEXT:
      return buildZExt(getDestFromArg(DstArg), getRegFromArg(Arg1));
    case TargetOpcode::G_ANYEXT:
      return buildAnyExt(getDestFromArg(DstArg), getRegFromArg(Arg1));
