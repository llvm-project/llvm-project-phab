...
#undef CALLSITE_DELEGATE_SETTER
  ...
  SmallVector<FunTy *, 1> getKnownCallees() {
    ...
    if (getCalledFunction()) {
    ...
    } else if (auto *Node = Inst->getMetadata(LLVMContext::MD_callees)) {
      ...
      for (const MDOperand &Op : Node->operands())
        if (auto *MDConstant = mdconst::extract_or_null<Constant>(Op))
          Callees.push_back(cast<FunTy>(MDConstant->stripPointerCasts()));
      if (!Callees.empty()) {
        // Ensure a stable iteration order over the known callees. To do this,
        // we sort the functions by name.
        std::sort(Callees.begin(), Callees.end(),
                  [](const Function *RHS, const Function *LHS) {
