...
struct InlineAsmIdentifierInfo {
  enum IdKind {
    Invalid,      // Initial state. Unexpected after a successful parsing.
    LabelKind,    // Function/Label reference.
    EnumValKind,  // Value of enumration type.
    VarKind       // Variable.
  };
  ...
  void setEnum(int64_t enumVal) {
    assert(isKind(Invalid) && "should be initialized only once");
    Kind = EnumValKind;
  ...
  InlineAsmIdentifierInfo() : Kind(Invalid) {}
private:
