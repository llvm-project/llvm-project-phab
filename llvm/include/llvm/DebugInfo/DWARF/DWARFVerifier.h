...
public:
  ...
  struct DieRangeInfo {
    ...
    std::vector<DWARFRange>::const_iterator findRange(const DWARFRange &R) const {
      auto Begin = Ranges.begin();
      auto Iter = std::upper_bound(Begin, Ranges.end(), R);
      if (Iter != Begin)
        --Iter;
      return Iter;
    }
  ...
  struct NonOverlappingRanges {
    ...
    /// Returns a valid DWARFDie object if a DIE overlaps with any ranges in
    /// RI, or an invalid DWARFDie if no other DIEs overlap.
    DWARFDie GetDieThatOverlapsWithAnyRange(const DieRangeInfo &RI) const;
