...
TEST(CommandLineTest, ConfigFileErrors) {
  ...
  cl::CfgFileSearch Result;

  // Config in not specified, default is absent.
  Result = cl::findConfigFile(CfgFileName, Argv, Dirs, "Test");
  EXPECT_TRUE(Result == cl::CfgFileSearch::Ignored);

  // Missed argument of --config
  Argv = { "--config" };
  Result = cl::findConfigFile(CfgFileName, Argv, Dirs, "Test");
  EXPECT_TRUE(Result == cl::CfgFileSearch::NoArgument);

  // Inexistent file specified by full path.
  Argv = { "--config", "/abcd.cfg" };
  Result = cl::findConfigFile(CfgFileName, Argv, Dirs, "Test");
  EXPECT_TRUE(Result == cl::CfgFileSearch::NotFoundOpt);
  ...
  Argv = { "--config", "abcd.cfg" };
  Result = cl::findConfigFile(CfgFileName, Argv, Dirs, "Test");
  EXPECT_TRUE(Result == cl::CfgFileSearch::NotFoundCfg);
  EXPECT_TRUE(Argv.empty());
  EXPECT_TRUE(CfgFileName.equals("abcd.cfg"));
...
TEST(CommandLineTest, ReadDefaultConfigFile) {
  ...
  llvm::SmallString<128> CfgFileName;
  cl::CfgFileSearch Result = cl::findConfigFile(CfgFileName, Argv,
                                              ArrayRef<const char *>(), "test");
...
TEST(CommandLineTest, ReadDefaultConfigFileFromStdDir) {
  ...
  llvm::SmallVector<const char *, 1> Argv = { TestTool.c_str(), "-flag_1" };
  ArrayRef<const char *> Dirs = { TestCfgDir.c_str() };
  llvm::SmallString<128> CfgFileName;
  cl::CfgFileSearch Result = cl::findConfigFile(CfgFileName, Argv, Dirs, "test");
