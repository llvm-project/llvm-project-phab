...
TEST(CommandLineTest, ConfigFileErrors) {
  ...
  Result = cl::findConfigFile(CfgFileName, Argv, Dirs, false, ErrText);
  EXPECT_FALSE(Result);
  ...
  EXPECT_FALSE(Result);
  EXPECT_TRUE(ErrText ==
              "Option '--config' must be followed by configuration file name");

  // Multiple options --config
  Argv = { "--config", "a", "--config", "b" };
  Result = cl::findConfigFile(CfgFileName, Argv, Dirs, false, ErrText);
  EXPECT_FALSE(Result);
  EXPECT_TRUE(ErrText == "More than one options '--config' are specified");

  // Inexistent file specified by full path.
  Argv = { "--config", "/inexistent_file.cfg" };
  Result = cl::findConfigFile(CfgFileName, Argv, Dirs, false, ErrText);
  EXPECT_FALSE(Result);
  EXPECT_TRUE(ErrText == "Configuration file '/inexistent_file.cfg' specified "
...
TEST(CommandLineTest, ReadConfigFile) {
  ...
  unsigned NumOpts = 0;
  Result = llvm::cl::readConfigFile(CfgFileName, Saver, Argv, NumOpts);
...
TEST(CommandLineTest, ReadConfigFileFromBinDir) {
  ...
  Result = cl::findConfigFile(CfgFileName, Argv, Dirs, true, ErrText);
  EXPECT_TRUE(Result);
  EXPECT_TRUE(ErrText.empty());
  ...
  Result = cl::findConfigFile(CfgFileName, Argv, Dirs, false, ErrText);
  EXPECT_FALSE(Result);
  EXPECT_TRUE(ErrText == std::string("Configuration file '") + ConfigFileName +
