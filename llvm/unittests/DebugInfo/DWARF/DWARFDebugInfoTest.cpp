...
TEST(DWARFDebugInfo, TestDwarfVerifyLexicalBlockRanges) {
  ...
  ASSERT_TRUE((bool)ErrOrSections);
  DWARFContextInMemory DwarfContext(*ErrOrSections, 8);
  VerifyError(DwarfContext, "error: DIE has a child DIE whose address ranges "
                            "are not contained in its ranges:");
}
...
TEST(DWARFDebugInfo, TestDwarfRangesContains) {
  DWARFVerifier::DWARFRange R(0x10, 0x20);
  ...
  ASSERT_FALSE(R.contains({0x10, 0x10}));
  // 1 byte of Range
  ...
  ASSERT_FALSE(R.contains({0x20, 0x20}));
  // valid past Range
...
TEST(DWARFDebugInfo, TestDWARFDieRangeInfoContains) {
  ...
  ASSERT_FALSE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x40, 0x41}})));
  ASSERT_TRUE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x10, 0x20}})));
  ASSERT_TRUE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x11, 0x12}})));
  ASSERT_TRUE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x1f, 0x20}})));
  ASSERT_TRUE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x30, 0x40}})));
  ASSERT_TRUE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x31, 0x32}})));
  ASSERT_TRUE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x3f, 0x40}})));
  ASSERT_TRUE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x10, 0x20},
                                                           {0x30, 0x40}})));
  ASSERT_TRUE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x11, 0x12},
                                                           {0x31, 0x32}})));
  ASSERT_TRUE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x11, 0x12},
                                                           ...
                                                           {0x31, 0x32},
                                                           {0x32, 0x33}})));
  ASSERT_FALSE(Ranges.contains(DWARFVerifier::DieRangeInfo({{0x11, 0x12},
                                                            {0x12, 0x13},
                                                            {0x20, 0x21},
                                                            {0x31, 0x32},
                                                            {0x32, 0x33}})));
...
void AssertRangesIntersect(const DWARFVerifier::DWARFRange &LHS,
                           const DWARFVerifier::DWARFRange &RHS) {
  ASSERT_TRUE(LHS.intersects(RHS));
  ASSERT_TRUE(RHS.intersects(LHS));
}
void AssertRangesDontIntersect(const DWARFVerifier::DWARFRange &LHS,
                               const DWARFVerifier::DWARFRange &RHS) {
