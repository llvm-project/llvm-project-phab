; RUN: llc -march=mips64 -relocation-model=static -mabi=n32 < %s | FileCheck --check-prefix=ALL --check-prefix=N32 %s
; RUN: llc -march=mips64el -relocation-model=static -mabi=n32 < %s | FileCheck --check-prefix=ALL --check-prefix=N32 %s

; RUN: llc -march=mips64 -relocation-model=static -mabi=n64 < %s | FileCheck --check-prefix=ALL --check-prefix=N64 %s
; RUN: llc -march=mips64el -relocation-model=static -mabi=n64 < %s | FileCheck --check-prefix=ALL --check-prefix=N64 %s
