; RUN: llc -march=mips64 -relocation-model=static -mattr=+soft-float -mabi=n32 < %s | FileCheck --check-prefix=ALL --check-prefix=SYM32 %s
; RUN: llc -march=mips64el -relocation-model=static -mattr=+soft-float -mabi=n32 < %s | FileCheck --check-prefix=ALL --check-prefix=SYM32 %s

; RUN: llc -march=mips64 -relocation-model=static -mattr=+soft-float -mabi=n64 < %s | FileCheck --check-prefix=ALL --check-prefix=SYM64 %s
; RUN: llc -march=mips64el -relocation-model=static -mattr=+soft-float -mabi=n64 < %s | FileCheck --check-prefix=ALL --check-prefix=SYM64 %s
