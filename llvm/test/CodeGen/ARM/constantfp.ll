...
; RUN: llc -mtriple=thumbv7m -arm-execute-only -mcpu=cortex-m4 -relocation-model=pic %s -o - \
; RUN: | FileCheck --check-prefix=CHECK-XO-PIC %s
...
; RUN: llc -mtriple=thumbv8m.main -arm-execute-only -mattr=fp-armv8 -relocation-model=pic %s -o - \
; RUN: | FileCheck --check-prefix=CHECK-XO-PIC %s
...
; CHECK-XO-FLOAT: movw [[REG:r[0-9]+]], :lower16:[[LABEL:.?LPC[0-9]+_[0-9]+]]
...
; CHECK-XO-PIC-LABEL: lower_fpconst_select
; CHECK-XO-PIC: ldr [[REG:r[0-9]+]], [[LABEL1:.?LCPI[0-9]+_[0-9]+]]
; CHECK-XO-PIC: [[LABEL2:.?LPC[0-9]+_[0-9]+]]:
; CHECK-XO-PIC: vldr {{s[0-9]+}}, {{[[]}}[[REG]]{{[]]}}
; CHECK-XO-PIC: [[LABEL1]]:
; CHECK-XO-PIC: .long   [[LABEL3:.?LPC[0-9]+_[0-9]+]]-([[LABEL2]]+4)
; CHECK-XO-PIC: .rodata
; CHECK-XO-PIC-NOT: .text
; CHECK-XO-PIC: [[LABEL3]]:
; CHECK-XO-PIC: .long   1335165689
; CHECK-XO-PIC: .long   1307470632

; CHECK-XO-ROPI-LABEL: lower_fpconst_select
; CHECK-XO-ROPI: movw [[REG:r[0-9]+]], :lower16:([[LABEL1:.?LPC[0-9]+_[0-9]+]]-([[LABEL2:.?LPC[0-9]+_[0-9]+]]+4))
