...
; RUN:   | FileCheck %s
; RUN: llc -ppc-strip-register-prefix -verify-machineinstrs \
; RUN:   -mtriple=powerpc64le-unknown-linux-gnu -mcpu=pwr8 -mattr=-vsx < %s \
; RUN:   | FileCheck %s
; RUN: llc -ppc-strip-register-prefix -verify-machineinstrs \
; RUN:   -mtriple=powerpc64-unknown-linux-gnu -mcpu=pwr7 -mattr=+power8-vector \
; RUN:   -mattr=-vsx < %s | FileCheck %s
; RUN: llc -ppc-strip-register-prefix -verify-machineinstrs \
; RUN:   -mtriple=powerpc64le-unknown-linux-gnu -mcpu=pwr8 < %s \
; RUN:   | FileCheck %s -check-prefix=CHECK-VSX
