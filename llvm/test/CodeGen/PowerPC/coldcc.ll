; RUN: llc -verify-machineinstrs -mtriple=powerpc64le-unknown-linux-gnu -ppc-enable-coldcc -ppc-enable-coldcc-stress-test  < %s | FileCheck %s -check-prefix=COLDCC
...
if.then:                                          ; preds = %entry
  ...
  %call = tail call fastcc signext i32 @callee(i32 signext %a, i32 signext %b)
  %add = add i32 %mul2, %a
...
define internal fastcc signext i32 @callee(i32 signext %a, i32 signext %b) {
entry:
; COLDCC: @callee
; COLDCC: std 6, -8(1)
; COLDCC: std 7, -16(1)
