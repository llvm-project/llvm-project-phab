...
; RUN:   -mtriple=powerpc64-unknown-linux-gnu -mcpu=g5 | FileCheck %s
; RUN: llc -ppc-strip-register-prefix -verify-machineinstrs < %s \
; RUN:   -mtriple=powerpc64-unknown-linux-gnu -mcpu=pwr6 | FileCheck -check-prefix=CHECK-PWR6 %s
; RUN: llc -ppc-strip-register-prefix -verify-machineinstrs < %s \
; RUN:   -mtriple=powerpc64-unknown-linux-gnu -mcpu=a2 | FileCheck -check-prefix=CHECK-A2 %s
; RUN: llc -ppc-strip-register-prefix -verify-machineinstrs < %s \
; RUN:   -mtriple=powerpc64-unknown-linux-gnu -mcpu=pwr7 -mattr=+vsx \
