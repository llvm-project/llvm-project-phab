; RUN: llc -relocation-model=static -verify-machineinstrs < %s \
; RUN:   -ppc-strip-register-prefix -mcpu=pwr7 | FileCheck %s
; RUN: llc -relocation-model=static -verify-machineinstrs < %s \
; RUN:   -ppc-strip-register-prefix -code-model=small -mcpu=pwr7 \
; RUN:   | FileCheck %s -check-prefix=SCM
