; RUN: llc -verify-machineinstrs -ppc-strip-register-prefix -O3 \
; RUN:   -mcpu=pwr7 < %s | FileCheck %s
; RUN: llc -verify-machineinstrs -ppc-strip-register-prefix \
; RUN:   -O3 -print-after=codegenprepare -mcpu=ppc64 \
; RUN:   < %s >%t 2>&1 && FileCheck \
; RUN:   --check-prefix=CHECK-NoAA <%t %s
; RUN: llc -verify-machineinstrs -ppc-strip-register-prefix \
; RUN:   -O3 -print-after=codegenprepare -mcpu=pwr7  \
; RUN:   < %s >%t 2>&1 && FileCheck \
; RUN:   --check-prefix=CHECK-UseAA <%t %s
