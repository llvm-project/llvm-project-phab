; RUN: opt < %s -globalopt -S -enable-coldcc-stress-test -mtriple=powerpc64le-unknown-linux-gnu -ppc-enable-coldcc | FileCheck %s -check-prefix=COLDCC
; RUN: opt < %s -globalopt -S -enable-coldcc-stress-test | FileCheck %s -check-prefix=CHECK
...
; COLDCC-LABEL: define internal coldcc i32 @callee_fastcc
...
; COLDCC: call coldcc i32 @callee_fastcc
