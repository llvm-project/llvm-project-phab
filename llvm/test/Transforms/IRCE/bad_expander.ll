...
; CHECK-NEXT:    %denom = load i64, i64* %p2, align 4
; CHECK-NEXT:    [[DIV:%[^ ]+]] = udiv i64 %num, %denom
...
; CHECK-NEXT:    br i1 %maybe_exit, label %range_check, label %exit.loopexit1
; CHECK:       range_check:
; CHECK-NEXT:    %div_result = udiv i64 %num, %denom
; CHECK-NEXT:    %rc = icmp slt i64 %iv.next, %div_result
; CHECK-NEXT:    br i1 true, label %guarded, label %exit.loopexit1
; CHECK:       guarded:
...
entry:
  %num = load i64, i64* %p1, align 4, !range !0
  %denom = load i64, i64* %p2, align 4, !range !1
  br label %loop
