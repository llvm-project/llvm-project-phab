...
; CHECK-NEXT:    br i1 true, label %inner_loop_preheader, label %out_of_bounds.loopexit1
; CHECK:       inner_loop:
; CHECK-NEXT:    %IV.inner = phi i32 [ %IV.inner.next, %inner_loop ], [ 83, %inner_loop_preheader ]
; CHECK-NEXT:    %IV.inner.next = add nsw i32 %IV.inner, -3
; CHECK-NEXT:    %tmp3 = zext i32 %IV.inner.next to i64
; CHECK-NEXT:    %tmp4 = icmp ult i64 %IV, %tmp3
; CHECK-NEXT:    br i1 %tmp4, label %inner_loop, label %latch
; CHECK:       latch:
...
; CHECK-NEXT:    br i1 %range_check.postloop, label %inner_loop_preheader.postloop, label %out_of_bounds.loopexit
...
loop:
  ...
  br i1 %range_check, label %inner_loop_preheader, label %out_of_bounds

inner_loop_preheader:
  br label %inner_loop

inner_loop:
  %IV.inner = phi i32 [ %IV.inner.next, %inner_loop ], [ 83, %inner_loop_preheader ]
  %IV.inner.next = add nsw i32 %IV.inner, -3
  %tmp3 = zext i32 %IV.inner.next to i64
  %tmp4 = icmp ult i64 %IV, %tmp3
  br i1 %tmp4, label %inner_loop, label %latch
...
; CHECK-NEXT:    %IV.inner.next = add nsw i32 %IV.inner, -3
; CHECK-NEXT:    %tmp3 = zext i32 %IV.inner.next to i64
; CHECK-NEXT:    %tmp4 = icmp slt i64 %IV, %tmp3
; CHECK-NEXT:    br i1 %tmp4, label %inner_loop, label %latch
; CHECK:       latch:
...
; CHECK:       loop:                                             ; preds = %loop.preheader, %latch
...
inner_loop_preheader:                             ; preds = %loop
  br label %inner_loop

inner_loop:                                       ; preds = %inner_loop, %inner_loop_preheader
  %IV.inner = phi i32 [ %IV.inner.next, %inner_loop ], [ 83, %inner_loop_preheader ]
  %IV.inner.next = add nsw i32 %IV.inner, -3
  %tmp3 = zext i32 %IV.inner.next to i64
  %tmp4 = icmp ult i64 %IV, %tmp3
...
; CHECK-NEXT:    br i1 %2, label %main.pseudo.exit, label %exit
; CHECK:       loop.postloop:                                    ; preds = %postloop, %latch.postloop
; CHECK-NEXT:    %IV.postloop = phi i64 [ %IV.copy, %postloop ], [ %IV.next.postloop, %latch.postloop ]
; CHECK-NEXT:    %IV.offset.postloop = add i64 %IV.postloop, -5
...
; CHECK-NEXT:    br i1 %range_check.postloop, label %inner_loop_preheader.postloop, label %out_of_bounds.loopexit
; CHECK:       latch.postloop:                                   ; preds = %inner_loop.postloop
; CHECK-NEXT:    %IV.next.postloop = add i64 %IV.postloop, -1
