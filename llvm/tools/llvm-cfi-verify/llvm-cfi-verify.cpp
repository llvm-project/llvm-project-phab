...
void printIndirectCFInstructions(FileAnalysis &Analysis,
  ...
  for (uint64_t Address : Analysis.getIndirectInstructions()) {
    ...
    bool CFIProtected = (ProtectionStatus == PROTECTED);
    ...
    for (auto &K : {"cfi-icall", "cfi-vcall"}) {
      for (auto &V :
           {std::make_pair("src", StringRef(LineInfo.FileName)),
            std::make_pair("fun", StringRef(LineInfo.FunctionName))}) {
        if (!BlameLine)
          BlameLine = SpecialCaseList->inSectionBlame(K, V.first, V.second);
      }
    }
