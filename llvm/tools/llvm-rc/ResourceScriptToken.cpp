...
static bool rcGetAsInteger(llvm::StringRef Representation, uint32_t &Num) {
...
RCToken::RCToken(RCToken::Kind RCTokenKind, llvm::StringRef Value)
...
uint32_t RCToken::intValue() const {
  ...
  // rcGetAsInteger).
  uint32_t Result;
  assert(rcGetAsInteger(TokenValue, Result));
  return Result;
}
...
private:
  ...
  bool willNowRead(llvm::StringRef FollowingChars) const;
...
bool Tokenizer::willNowRead(llvm::StringRef FollowingChars) const {
...
Expected<std::vector<RCToken>> tokenizeRC(llvm::StringRef Input) {
