...
public:
  /// C'tor
  Modifier(BasicBlock *Block, PieceTable *PT, std::mt19937_64 *R):
...
protected:
  ...
  /// Pick a random type.
  Type *pickType() {
    return ((*Ran)() & 1 ? pickVectorType() : pickScalarType());
  }
  ...
  Type *pickScalarType() {
    ...
    return ScalarTypes[(*Ran)() % ScalarTypes.size()];
  }
...
struct LoadModifier: public Modifier {
  LoadModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
...
struct StoreModifier: public Modifier {
  StoreModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
...
struct BinModifier: public Modifier {
  BinModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
...
struct ConstModifier: public Modifier {
  ConstModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
  ...
  void Act() override {
    ...
    if (Ty->isIntegerTy()) {
      ...
      case 6:
        PT->push_back(ConstantInt::get(Ty, (*Ran)()));
      }
    }
...
struct AllocaModifier: public Modifier {
  AllocaModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R){}
...
struct ExtractElementModifier: public Modifier {
  ExtractElementModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
  ...
  void Act() override {
    ...
    Value *V = ExtractElementInst::Create(Val0,
             ...
             (*Ran)() % cast<VectorType>(Val0->getType())->getNumElements()),
             "E", BB->getTerminator());
...
struct ShuffModifier: public Modifier {
  ShuffModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
  ...
  void Act() override {
    ...
    for (unsigned i=0; i<Width; ++i) {
      Constant *CI = ConstantInt::get(I32, (*Ran)() % (Width*2));
...
struct InsertElementModifier: public Modifier {
  InsertElementModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
...
struct CastModifier: public Modifier {
  CastModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
...
struct SelectModifier: public Modifier {
  SelectModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
  ...
  void Act() override {
    // Try a bunch of different select configuration until a valid one is found.
      ...
      if (Val0->getType()->isVectorTy() && ((*Ran)() % 1)) {
...
struct CmpModifier: public Modifier {
  CmpModifier(BasicBlock *BB, PieceTable *PT, std::mt19937_64 *R):
    Modifier(BB, PT, R) {}
...
static void FillFunction(Function *F, std::mt19937_64 &R) {
...
static void IntroduceControlFlow(Function *F, std::mt19937_64 &R) {
