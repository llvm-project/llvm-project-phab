...
void Object<ELFT>::initSymbolTable(const llvm::object::ELFFile<ELFT> &ElfFile,
  ...
  SymTab->setStrTab(getSectionCast<StringTableSection>(
      SymbolTable->Link,
      "Symbol table has link index of " + Twine(SymbolTable->Link) +
...
template <class ELFT>
template <class T>
T *Object<ELFT>::getSectionCast(uint16_t Index, Twine ErrMsg,
                                Twine CastErrMsg) {
...
void Object<ELFT>::readSectionHeaders(const ELFFile<ELFT> &ElfFile) {
  ...
  for (auto &Section : Sections) {
    if (auto RelSec = dyn_cast<RelocationSection<ELFT>>(Section.get())) {
      ...
      auto SymTab = getSectionCast<SymbolTableSection>(
          RelSec->Link,
    ...
    if (auto Sec = dyn_cast<SectionWithStrTab>(Section.get())) {
      Sec->setStrTab(getSectionCast<StringTableSection>(
          Sec->Link,
...
template <class ELFT> Object<ELFT>::Object(const ELFObjectFile<ELFT> &Obj) {
  ...
  SectionNames = getSectionCast<StringTableSection>(
      Ehdr.e_shstrndx,
