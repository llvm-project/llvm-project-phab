...
DWARFContext::create(const object::ObjectFile &Obj, const LoadedObjectInfo *L,
  ...
  return make_unique<DWARFContextInMemory>(Obj, L, HandleError);
}

std::unique_ptr<DWARFContext>
DWARFContext::create(const StringMap<std::unique_ptr<MemoryBuffer>> &Sections,
                     uint8_t AddrSize, bool isLittleEndian) {
  return make_unique<DWARFContextInMemory>(Sections, AddrSize, isLittleEndian);
}
...
DWARFContextInMemory::DWARFContextInMemory(
    ...
    function_ref<ErrorPolicy(Error)> HandleError)
    : DWARFContext(make_unique<DWARFObjInMemory>(Obj, L, HandleError)) {}

DWARFContextInMemory::DWARFContextInMemory(
    const StringMap<std::unique_ptr<MemoryBuffer>> &Sections, uint8_t AddrSize,
    bool IsLittleEndian)
    : DWARFContext(
          make_unique<DWARFObjInMemory>(Sections, AddrSize, IsLittleEndian)) {}
