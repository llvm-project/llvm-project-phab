...
static void dumpAccelSection(raw_ostream &OS, StringRef Name,
                             const DWARFObj &Obj, const DWARFSection &Section,
...
void DWARFContext::dump(raw_ostream &OS, DIDumpOptions DumpOpts) {
  ...
  if (DumpType == DIDT_All || DumpType == DIDT_Line) {
    ...
    for (const auto &CU : compile_units()) {
      ...
      if (auto StmtOffset = toSectionOffset(CUDIE.find(DW_AT_stmt_list))) {
        DWARFDataExtractor lineData(Obj, getLineSection(), isLittleEndian(),
  ...
  if (DumpType == DIDT_All || DumpType == DIDT_LineDwo) {
    ...
    DWARFDataExtractor lineData(Obj, getLineDWOSection(), isLittleEndian(),
                                savedAddressByteSize);
    DWARFDebugLine::LineTable LineTable;
  ...
  if (DumpType == DIDT_All || DumpType == DIDT_Ranges) {
    ...
    DWARFDataExtractor rangesData(Obj, getRangeSection(), isLittleEndian(),
                                  savedAddressByteSize);
    offset = 0;
  ...
  if (DumpType == DIDT_All || DumpType == DIDT_StrOffsets)
    dumpStringOffsetsSection(OS, "debug_str_offsets", Obj,
                             getStringOffsetSection(), getStringSection(),
                             isLittleEndian(), getMaxVersion());

  if (DumpType == DIDT_All || DumpType == DIDT_StrOffsetsDwo) {
    dumpStringOffsetsSection(OS, "debug_str_offsets.dwo", Obj,
  ...
  if (DumpType == DIDT_All || DumpType == DIDT_AppleNames)
    dumpAccelSection(OS, "apple_names", Obj, getAppleNamesSection(),
                     getStringSection(), isLittleEndian());

  if (DumpType == DIDT_All || DumpType == DIDT_AppleTypes)
    dumpAccelSection(OS, "apple_types", Obj, getAppleTypesSection(),
                     getStringSection(), isLittleEndian());

  if (DumpType == DIDT_All || DumpType == DIDT_AppleNamespaces)
    dumpAccelSection(OS, "apple_namespaces", Obj, getAppleNamespacesSection(),
                     getStringSection(), isLittleEndian());

  if (DumpType == DIDT_All || DumpType == DIDT_AppleObjC)
    dumpAccelSection(OS, "apple_objc", Obj, getAppleObjCSection(),
                     getStringSection(), isLittleEndian());
}
...
const DWARFDebugLoc *DWARFContext::getDebugLoc() {
  ...
  if (getNumCompileUnits()) {
    DWARFDataExtractor LocData(Obj, getLocSection(), isLittleEndian(),
...
DWARFContext::getLineTableForUnit(DWARFUnit *U) {
  ...
  DWARFDataExtractor lineData(Obj, U->getLineSection(), isLittleEndian(),
...
class DWARFObjInMemory final : public DWARFObj {
  bool IsLittleEndian;
  uint8_t AddressSize;
  StringRef FileName;
...
DWARFContextInMemory::DWARFContextInMemory(
    ...
    function_ref<ErrorPolicy(Error)> HandleError)
    : DWARFContext(*new DWARFObjInMemory(Obj, L, HandleError)) {
  DObj.reset(&this->Obj);
}
...
DWARFContextInMemory::DWARFContextInMemory(
    ...
    bool IsLittleEndian)
    : DWARFContext(*new DWARFObjInMemory(Sections, AddrSize, IsLittleEndian)) {
  DObj.reset(&this->Obj);
}
