...
bool operator==(const Value &L, const Value &R) {
  ...
  auto FoldedType = [](const ValueRep &V) -> ValueType {
    switch (V.getTag()) {
      case VT_StaticString:
        return VT_OwnedString;
      case VT_SmallInt:
        return VT_Double;
      default:
        return V.getTag();
  ...
  auto FT = FoldedType(L.U);
  if (FT != FoldedType(R.U)) return false;
  switch (FT) {
    case VT_OwnedString:
    ...
    case VT_SmallInt:
    case VT_StaticString:
      llvm_unreachable("Types folded away");
  }
}
...
bool operator==(const Object &L, const Object &R) {
  ...
  for (const auto &E : L) {
    ...
    if (!Other || E.second != *Other) return false;
  }
  return true;
}
...
Literal::Literal(std::initializer_list<Literal> V, bool ForceArray) {
  if (ForceArray || !IsObjectCompatible(V)) {
    ...
    for (const auto &E : V) R.Array.push_back(E);
  } else {
    Type = LT_Object;
...
class Document::Parser {
 public:
  ...
  Expected<Document> parseDocument() {
    ...
    if (!Root) return std::move(*Err);
    eatWhitespace();
    if (P != End) {
      error("Text after end of document");
      return std::move(*Err);
    }
    if (Err) llvm_unreachable("Error should have been reported");
 ...
 private:
  ...
  Optional<ValueRep> parseObject() {
    eatWhitespace();
    if (P == End) return error("Unexpected EOF");
    switch (char C = next()) {
      case 't': {
      ...
      case '[': {
        ...
        eatWhitespace();
        if (peek() == ']') return ValueRep::create<VT_Array>(A);
        for (;;) {
      ...
      case '{': {
        ...
        eatWhitespace();
        if (peek() == '}') return ValueRep::create<VT_Object>(O);
        for (;;) {
          if (next() != '"') return error("Expected object key");
          auto K = parseString();
          if (!K) return None;
          eatWhitespace();
          if (next() != ':') return error("Expected : after object key");
          eatWhitespace();
          auto V = parseObject();
          if (!V) return None;
      ...
      default:
        if (isNumber(C)) {
          ...
          if (!V) return None;
          if (*V == SmallInt(*V))
            return ValueRep::create<VT_SmallInt>(SmallInt(*V));
        ...
        case '\\':
          switch (C = next()) {
            ...
            case 't':
              ...
              break;
            case 'u':
              if (!parseUnicode(Result)) return None;
              break;
            default:
  ...
  llvm::Optional<double> parseNumber(char First) {
    SmallString<24> S;
    S.push_back(First);
    while (isNumber(peek())) S.push_back(next());
    char *End;
    double D = std::strtod(S.c_str(), &End);
    if (End != S.end()) return error("Invalid number");
    return D;
  }
