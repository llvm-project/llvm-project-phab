...
ValueRep Literal::construct(Arena &Alloc) const {
  switch (Type) {
    case LT_Twine: {
      return ValueRep::create<VT_OwnedString>(PString::create(
    ...
    case LT_Double:
      return ValueRep::create<VT_Double>(new (Alloc) double{R.Double});
    case LT_Immediate:
      return R.Immediate;
    case LT_Value:  // Deep copy.
      return deepCopy(ValueRep(*R.Value), Alloc);
  }
}
...
Literal::Literal(const Literal &Copy) {
  ...
  case LT_Double:
  case LT_Value:
    memcpy(&R, &Copy.R, sizeof(Rep)); // POD
  }
  Type = Copy.Type;
}
...
Literal::Literal(Literal &&Move) {
  ...
  case LT_Double:
  case LT_Value:
    memcpy(&R, &Move.R, sizeof(Rep)); // POD
  }
  Type = Move.Type;
  Move.reset();
}
