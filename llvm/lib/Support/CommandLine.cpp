...
bool cl::searchForFile(SmallVectorImpl<char> &CfgFileName,
                       ArrayRef<const char *> Dirs, StringRef ProgramFullPath,
                       StringRef FileName) {
  CfgFileName.clear();
  if (searchDirectoriesForFile(CfgFileName, FileName, Dirs))
    return true;
...
cl::SearchResult cl::findConfigFileFromArgs(SmallVectorImpl<char> &CfgFileName,
  ...
  if (searchForFile(CfgFileName, Dirs, SearchInBinDir ? Argv[0] : StringRef(),
                    CfgFile))
    return SearchResult::Successful;
...
bool cl::checkConfigFileSearchResult(SearchResult Res,
  ...
  case SearchResult::NotFoundCfg:
    ...
    if (SearchInBinDir) {
      SmallString<128> BinDir(ProgramFullPath);
...
void cl::readConfigFile(SmallVectorImpl<char> &CfgFile, StringSaver &Saver,
  ...
  ExpandResponseFile(CfgFile.data(), Saver, tokenizeConfigFile, NewArgs, false,
                     true);
