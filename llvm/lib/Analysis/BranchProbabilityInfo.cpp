...
bool BranchProbabilityInfo::calcLoopBranchHeuristics(const BasicBlock *BB,
  ...
  //    }
  //  }
  // In this sort of situation taking the branch means that at the very least
  // it won't be taken again in the next iteration of the loop, so we should
  ...
  if (BI && BI->isConditional()) {
    ...
    if (CI && isa<Instruction>(CI->getOperand(0)) &&
      ...
      // involving constants that ends in a PHI which we can then collapse into
      // a single value if the PHI value is known.
      ...
      while (!WorkList.empty()) {
        ...
        for (BasicBlock *B : P->blocks()) {
          ...
          if (!L->contains(B))
            continue;
          // If this incoming value is a constant and B is a successor of BB,
          // then we can constant-evaluate the compare to see if it makes the
          ...
          if (isa<Constant>(V) &&
            ...
            for (Instruction *I : InstChain) {
              if (!CmpLHSConst)
                break;
              CmpLHSConst =
                ConstantExpr::get(I->getOpcode(), CmpLHSConst,
                                  dyn_cast<Constant>(I->getOperand(1)), true);
            }
            if (CmpLHSConst) {
              ...
              Constant *Result = ConstantExpr::getCompare(CI->getPredicate(),
                                                          CmpLHSConst,
                                                          CmpConst, true);
              // If the result means we don't branch to the block then that
              // block is unlikely.
              if (Result &&
  ...
  for (succ_const_iterator I = succ_begin(BB), E = succ_end(BB); I != E; ++I) {
    if (UnlikelyBlocks.find(*I) != UnlikelyBlocks.end())
