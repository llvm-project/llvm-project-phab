...
// Return a pair of {IsClobber, IsMustAlias}. It relies on IsMustAlias being
...
static ClobberAlias instructionClobbersQuery(MemoryDef *MD,
  ...
  if (const IntrinsicInst *II = dyn_cast<IntrinsicInst>(DefInst)) {
    ...
    // markers.
    bool checkIfMustAlias;
    switch (II->getIntrinsicID()) {
    case Intrinsic::lifetime_start:
      if (UseCS)
        return {false, false};
      checkIfMustAlias =
          AA.isMustAlias(MemoryLocation(II->getArgOperand(1)), UseLoc);
      return {checkIfMustAlias, checkIfMustAlias};
  ...
  if (auto *DefLoad = dyn_cast<LoadInst>(DefInst)) {
    if (auto *UseLoad = dyn_cast<LoadInst>(UseInst)) {
      ...
      case Reorderability::IfNoAlias:
        AliasResult AResult = AA.alias(MemoryLocation::get(DefLoad), UseLoc);
        return {AResult != NoAlias, AResult == MustAlias};
      }
    }
  }

  // FIXME: avoid calling AA twice here.
  Optional<MemoryLocation> DefLoc = MemoryLocation::getOrNone(DefInst);
  if (DefLoc == None)
    return {(bool)(AA.getModRefInfo(DefInst, UseLoc) & MRI_Mod), false};
  // IsClobber = if location is modified, not AResult != NoAlias;
  bool IsClobber = AA.getModRefInfo(DefInst, UseLoc) & MRI_Mod;
  if (!IsClobber)
    return {false, false};
  return {true, AA.isMustAlias(DefLoc.getValue(), UseLoc)};
}
...
public:
  ...
  MemoryAccess *findClobber(MemoryAccess *Start, UpwardsMemoryQuery &Q) {
    ...
    if (WalkResult.IsKnownClobber) {
      ...
      Q.IsMustAlias = WalkResult.IsMustAlias;
    } else {
...
void MemorySSA::OptimizeUses::optimizeUsesInBlock(
  ...
  for (MemoryAccess &MA : *Accesses) {
    ...
    if (isUseTriviallyOptimizableToLiveOnEntry(*AA, MU->getMemoryInst())) {
      MU->setDefiningAccess(MSSA->getLiveOnEntryDef(), true, true);
      continue;
    }
    ...
    while (UpperBound > LocInfo.LowerBound) {
      ...
      if (!UseMLOC.IsCall && lifetimeEndsAt(MD, UseMLOC.getLoc(), *AA)) {
        ...
        UpperBound = 0;
        FoundClobberResult = true;
        LocInfo.IsMustAlias = true;
        break;
      }
      ClobberAlias CA = instructionClobbersQuery(MD, MU, UseMLOC, *AA);
      if (CA.IsClobber) {
        FoundClobberResult = true;
        LocInfo.IsMustAlias = CA.IsMustAlias;
        break;
      }
      --UpperBound;
    }

    // Note: Phis always have IsMustAlias set to false ATM.
...
MemorySSA::CachingWalker::getClobberingMemoryAccess(MemoryAccess *MA) {
  ...
  if (auto *MUD = dyn_cast<MemoryUseOrDef>(StartingAccess)) {
    ...
    if (MSSA->isLiveOnEntryDef(Result) || Q.IsMustAlias)
      MUD->setDefiningToMustAlias();
  }
