...
cl::opt<bool> DisableCygprofileInlining(
    "disable-cygprofile-inlining",
    cl::desc("Disable inlining of calls to __cyg_profile functions"),
    cl::init(false));
...
bool CallAnalyzer::visitCallSite(CallSite CS) {
  ...
  if (Function *F = CS.getCalledFunction()) {
    // Ignore call instructions generated by -fprofile-functions. They will not
    // be cloned, and should not affect the inlining decision.
    if (DisableCygprofileInlining &&
        (F->getName() == "__cyg_profile_func_enter" ||
         F->getName() == "__cyg_profile_func_exit"))
      return false;
