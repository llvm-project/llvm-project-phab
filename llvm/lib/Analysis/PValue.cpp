...
size_t PVSet::getNumDimensions() const { return isl_set_n_dim(Obj); }
...
PVSet &PVSet::intersect(const PVSet &S) {
  if (!Obj)
    ...
    isl_set *SObj = S;
    if (isl_set_is_params(SObj)) {
      Obj = isl_set_intersect_params(Obj, SObj);
    } else {
      unifySetDimensions(Obj, SObj);
...
PVSet &PVSet::dropLastInputDims(unsigned Dims) {
  Obj = isl_set_eliminate_dims(Obj, getNumDimensions() - Dims, Dims);
  Obj = isl_set_coalesce(Obj);
  return *this;
}
...
PVSet &PVSet::dropDimsFrom(unsigned Dim) {
  if (Dim < getNumDimensions())
    Obj = isl_set_project_out(Obj, isl_dim_set, Dim, getNumDimensions() - Dim);
  return *this;
}
...
PVMap &PVMap::intersectDomain(const PVSet &Dom) {
  auto DomDim = Dom.getNumDimensions();
...
bool PVAff::isComplex() const {
  ...
  if (!Complex)
    Complex = isl_set_n_basic_set(getDomain()) > DOMAIN_N_BASIC_SEK_TRESHOLD;
  return Complex;
}
...
PVAff &PVAff::simplify(PVSet &S) {
  isl_set *Set;
  int DimDiff = S.getNumDimensions() - getNumInputDimensions();
  if (DimDiff > 0) {
    S.dropDimsFrom(S.getNumDimensions() - DimDiff);
    Set = S;
  }
  else if (DimDiff < 0)
