...
Instruction *InstCombiner::visitOr(BinaryOperator &I) {
  ...
  // ((V<<C3)&C1) | ((V&C5)<<C4) --> ((V&C5)<<C3) | ((V&C5)<<C4)
  {
    ...
    static auto MatchAndOfShift =
        [](Value *V, Value *&Source, Instruction::BinaryOps &ShiftOpcode,
           ConstantInt *&ShiftBy, ConstantInt *&PreShiftMask,
           ConstantInt *&PostShiftMask, BinaryOperator *&IntermediateInstr) -> bool {
      if (!match(V, m_And(m_BinOp(IntermediateInstr), m_ConstantInt(PostShiftMask))) ||
          !match(IntermediateInstr, m_LogicalShift(m_Value(Source), m_ConstantInt(ShiftBy))))
        return false;
	  ShiftOpcode = IntermediateInstr->getOpcode();
      Instruction::BinaryOps InverseOpcode = IntermediateInstr->getOpcode() == Instruction::Shl
                                                 ? Instruction::LShr
                                                 : Instruction::Shl;
