...
static void recordDeadLoad(LoadInst *Load,
                           InstCombiner::BuilderTy &Builder,
                           PtrInfoTy &Ptrs) {
  ...
  for (BasicBlock::iterator Iter = BB->begin(), E = BB->end(); Iter != E;) {
    ...
    if (LoadInst *LInstr = dyn_cast<LoadInst>(Iter++)) {
      ...
      if (Ptr && MaxOffset > 0 &&
        ...
        } else Builder.SetInsertPoint(PtrInst->getNextNode());
        Value *Ops[] = {Ptr, Builder.getInt64(MaxOffset)};
        Module *M = BB->getParent()->getParent();
        Value *TheFn = Intrinsic::getDeclaration(M, Intrinsic::phantom_mem,
                                                 {Ptr->getType()});
        const Twine &Name = "";
