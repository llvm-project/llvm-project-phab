    ...
    if (TVI->hasOneUse() && !isa<Constant>(FalseVal)) {
      if (unsigned SFO = getSelectFoldableOperands(TVI)) {
        ...
        if (OpToFold) {
          ...
          // We might be able to simplify this further if we have two constants
          // and the condition is a bit test.
          // TODO: Not handling shifts for now.
          if (OOpIsAPInt && !TVI->isShift()) {
            if (auto *ICI = dyn_cast<ICmpInst>(SI.getCondition())) {
              if (Value *V = foldSelectICmpAnd(SI.getType(), ICI, *OOpC, CI,
                                               Builder)) {
                BinaryOperator *BO = BinaryOperator::Create(TVI->getOpcode(),
                                                            FalseVal, V);
                BO->copyIRFlags(TVI);
    ...
    if (FVI->hasOneUse() && !isa<Constant>(TrueVal)) {
      if (unsigned SFO = getSelectFoldableOperands(FVI)) {
        ...
        if (OpToFold) {
          ...
          // and the condition is a bit test.
          // TODO: Not handling shifts for now.
          if (OOpIsAPInt && !FVI->isShift()) {
            if (auto *ICI = dyn_cast<ICmpInst>(SI.getCondition())) {
              if (Value *V = foldSelectICmpAnd(SI.getType(), ICI, CI, *OOpC,
                                               Builder)) {
                BinaryOperator *BO = BinaryOperator::Create(FVI->getOpcode(),
                                                            TrueVal, V);
                BO->copyIRFlags(FVI);
