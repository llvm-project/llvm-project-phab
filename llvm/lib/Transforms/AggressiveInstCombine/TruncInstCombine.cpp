...
static cl::opt<unsigned> MaxDepth("aggressive-instcombine-depth",
                                  cl::desc("Aggressive InstCombine max depth."),
...
unsigned TruncInstCombine::getMinBitWidth(Value *V, unsigned ValidBitWidth) {
  ...
  // for a smaller valid bit-width, then just return the answer we already have.
  ...
  // prevent infinite loop, when this instruction is part of a loop in the dag.
...
Type *TruncInstCombine::getBestTruncatedType() {
  ...
  assert(MinBitWidth >= ValidBitWidth);
  ...
  return IntegerType::get(DstTy->getContext(), MinBitWidth);
}
