...
OptimizeFunctions(Module &M, TargetLibraryInfo *TLI,
  ...
  for (Module::iterator FI = M.begin(), E = M.end(); FI != E; ) {
    ...
    if (hasChangeableCC(F) && !F->isVarArg() &&
      ...
      // enabled or the target would like to use coldcc on the specified
      // function which is cold at all call sites.
      if (EnableColdCCStressTest ||
          (allCallSitesCold(*F, GetBFI) && TTI.useColdCCForColdCalls(*F))) {
