    ...
    if ((Dep.Type == MemoryDepChecker::Dependence::DepType::Backward)) {
      ...
      if (MSSA->isLiveOnEntryDef(D))
        ...
        if (!TheLoop->contains(DefInst))
          ...
          // instruction is before defining access, so if the store instruction
          // is between phiNode and load instruction, then the instructions can
          // be reordered
          ...
          if (DefPhi) {
            ...
            // If the defining access is MemoryUsOrDef, then it means that load
            // Instruction is after defining access, so if the store instruction
            // is between defining access and load instruction then, the
            // instructions can be reordered
            if (!OI.dominates(St, DefInst))
              Reordered |= checkDepAndReorder(St, Ld);
          }
        }
      }
