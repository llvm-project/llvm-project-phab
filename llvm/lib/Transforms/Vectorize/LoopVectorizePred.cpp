...
bool LoopInstructionsReordering::checkDepAndReorder(
  ...
  while ((moved == 1) && (&*temprit != &*LoadInstIter)) {
    for (BasicBlock::iterator rit = LoadInstIter; rit != StoreInstIter;) {
      ...
      // Get all the operands of load instruction which also needs to be moved
      // before the store instructions
      unsigned opNum;
      for (opNum = 0; opNum < (&*temprit)->getNumOperands(); opNum++) {
        if (((&*temprit)->getOperand(opNum) == (&*rit)))
          isOperand = true;
      }

      if (!isOperand) {
        moved = 1;
        temprit->moveBefore(&*rit);
        reordered |= true;
    ...
    if ((Dep.Type == MemoryDepChecker::Dependence::DepType::Backward)) {
      ...
      for (BasicBlock *BB : TheLoop->blocks()) {
        ...
        while (it != BB->end()) {
          ...
          if (LoadInst *LdInst = dyn_cast<LoadInst>(&*it)) {
            if (LdInst == Ld)
              ldIter = it;
          }
          ++it;
      ...
      MemoryAccess *D = MSSA->getMemoryAccess(Ld)->getDefiningAccess();
