...
void BoUpSLP::setInsertPointAfterBundle(ArrayRef<Value *> VL, Value *OpValue) {
  ...
  if (BlocksSchedules.count(BB)) {
    auto *Bundle =
        BlocksSchedules[BB]->getScheduleData(VL.back());
  ...
  if (!LastInst) {
    ...
    for (auto &I : make_range(BasicBlock::iterator(Front), BB->end())) {
      ...
      bool sameOrAlt = sameOpcodeOrAlt(
          I.getOpcode(), getAltOpcode(I.getOpcode()), Front->getOpcode());
      if (!sameOrAlt)
        NewOpcode = tryToRepresentAsInstArg(I.getOpcode(), Front);
      if (Bundle.erase(&I) && (sameOrAlt || NewOpcode != 0))
        LastInst = &I;
      if (Bundle.empty())
        break;
    }
  ...
  for (const auto &ExternalUse : ExternalUses) {
    ...
    if (auto *VecI = dyn_cast<Instruction>(Vec)) {
      if (PHINode *PH = dyn_cast<PHINode>(User)) {
        ...
        if (!DT->dominates(VecI, cast<Instruction>(User)))
         {
          LocalIgnore.insert(User);
          continue;
        }
