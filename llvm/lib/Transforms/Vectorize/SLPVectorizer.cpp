...
/// OpValue.
static Value *chooseKey(Value *OpValue, Value *Op) {
...
struct InstructionsState {
  ...
  InstructionsState(unsigned Opcode, Value *OpValue, bool IsAltShuffle)
      : Opcode(Opcode), OpValue(OpValue), IsAltShuffle(IsAltShuffle) {}
};
} // namespace
...
static void propagateIRFlags(Value *I, ArrayRef<Value *> VL, Value *OpValue) {
...
static bool matchExtractIndex(Instruction *E, unsigned Idx) {
  assert(E->getOpcode() == Instruction::ExtractElement ||
...
private:
  ...
  struct ScheduleData {
    ...
    void init(int BlockSchedulingRegionID, Value *OpValue) {
      FirstInBundle = this;
...
int BoUpSLP::getEntryCost(TreeEntry *E) {
  ...
  Type *ScalarTy = E->State.OpValue->getType();
  if (StoreInst *SI = dyn_cast<StoreInst>(E->State.OpValue))
    ScalarTy = SI->getValueOperand()->getType();
  VectorType *VecTy = VectorType::get(ScalarTy, VL.size());

  // If we have computed a smaller type for the expression, update VecTy so
  // that the costs will be accurate.
  if (MinBWs.count(E->State.OpValue))
    VecTy = VectorType::get(
        IntegerType::get(F->getContext(), MinBWs[E->State.OpValue].first),
        VL.size());
  ...
  switch (E->State.IsAltShuffle ? Instruction::ShuffleVector
    ...
    case Instruction::Select: {
      ...
      int ScalarCost = VecTy->getNumElements() *
                       TTI->getCmpSelInstrCost(E->State.Opcode, ScalarTy,
                                               Builder.getInt1Ty());
      int VecCost = TTI->getCmpSelInstrCost(E->State.Opcode, VecTy, MaskTy);
      return VecCost - ScalarCost;
    }
    case Instruction::Add:
    case Instruction::FAdd:
...
static bool shouldReorderOperands(
  ...
  if (SplatRight) {
    if (VRight == Right[Idx - 1])
      // Preserve SplatRight
      return false;
    if (VLeft == Right[Idx - 1]) {
  ...
  if (AllSameOpcodeRight) {
    unsigned RightPrevOpcode = cast<Instruction>(Right[Idx - 1])->getOpcode();
    ...
    if (ILeft && RightPrevOpcode == ILeft->getOpcode()) {
      ...
      if (AllSameOpcodeLeft && ILeft &&
          cast<Instruction>(Left[Idx - 1])->getOpcode() == ILeft->getOpcode())
        return false;
      return true;
    }
  }
  ...
  if (AllSameOpcodeLeft) {
    unsigned LeftPrevOpcode = cast<Instruction>(Left[Idx - 1])->getOpcode();
...
void BoUpSLP::setInsertPointAfterBundle(ArrayRef<Value *> VL, Value *OpValue) {
  ...
  if (BlocksSchedules.count(BB)) {
    auto *Bundle =
        BlocksSchedules[BB]->getScheduleData(chooseKey(OpValue, VL.back()));
...
bool BoUpSLP::BlockScheduling::tryScheduleBundle(ArrayRef<Value *> VL,
  ...
  for (Value *V : VL) {
    ScheduleData *BundleMember = getScheduleData(V, chooseKey(OpValue, V));
    assert(BundleMember &&
...
bool BoUpSLP::BlockScheduling::extendSchedulingRegion(Value *V,
                                                      Value *OpValue) {
  if (getScheduleData(V, chooseKey(OpValue, V)))
    return true;
...
void BoUpSLP::scheduleBlock(BlockScheduling *BS) {
  ...
  while (!ReadyInsts.empty()) {
    ...
    while (BundleMember) {
      ...
      bool SameOpcode = pickedInst == BundleMember->OpValue;
