...
class LoopVectorizationPlanner {
  ...
  DenseMap<std::pair<BasicBlock *, BasicBlock *>, VPValue *> EdgeMaskCache;
  DenseMap<BasicBlock *, VPValue *> BlockMaskCache;
...
private:
  ...
  VPValue *createBlockInMask(BasicBlock *BB);
  ...
  VPlan *buildVPlan(VFRange &Range, const SmallPtrSetImpl<Value *> &NeedDef);
};
...
/// A recipe for generating a binary operation out of two VPInstructions.
...
VPValue *LoopVectorizationPlanner::createEdgeMask(BasicBlock *Src,
  ...
  auto ECEntryIt = EdgeMaskCache.find(Edge);
  if (ECEntryIt != EdgeMaskCache.end())
    return ECEntryIt->second;

  VPValue *SrcMask = createBlockInMask(Src);
...
VPValue *LoopVectorizationPlanner::createBlockInMask(BasicBlock *BB) {
  // Look for cached value.
  auto BCEntryIt = BlockMaskCache.find(BB);
...
LoopVectorizationPlanner::tryToWidenMemory(Instruction *I, VFRange &Range) {
  ...
  VPValue *Mask =
      Legal->isMaskRequired(I) ? createBlockInMask(I->getParent()) : nullptr;
...
LoopVectorizationPlanner::createReplicateRegion(Instruction *Instr,
  ...
  VPValue *BlockInMask = createBlockInMask(Instr->getParent());
