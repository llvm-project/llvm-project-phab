...
public:
  ...
  widenIntOrFpInduction(const InductionDescriptor &ID, bool NeedsScalarIV,
  ...
  void buildScalarSteps(Value *ScalarIV, Value *Step, Value *EntryVal,
                        const InductionDescriptor &ID, unsigned MinPart,
                        unsigned MaxPart, unsigned MinLane, unsigned MaxLane);
...
private:
  ...
  widenIntOrFpInduction(VPlan *Plan, unsigned StartRangeVF,
...
std::pair<Value *, Value *>
InnerLoopVectorizer::widenIntOrFpInduction(const InductionDescriptor &ID,
                                           bool NeedsScalarIV, PHINode *IV,
                                           TruncInst *Trunc) {
...
void InnerLoopVectorizer::buildScalarSteps(Value *ScalarIV, Value *Step,
                                           ...
                                           unsigned MinPart, unsigned MaxPart,
                                           unsigned MinLane, unsigned MaxLane) {
...
void InnerLoopVectorizer::widenPHIInstruction(Instruction *PN, unsigned UF,
  ...
  case InductionDescriptor::IK_FpInduction:
    llvm_unreachable("Integer/fp induction handled elsewhere");
...
void InnerLoopVectorizer::vectorizeInstruction(Instruction &I) {
  ...
  case Instruction::PHI: {
    llvm_unreachable("Phi nodes have their own recipe");
  } // End of PHI.
...
void LoopVectorizationPlanner::printCurrentPlans(const std::string &Title,
  ...
  DenseMap<VPlan *, SmallVector<unsigned, 4>> VPlansAndVFs;
  for (auto &Entry : VPlans)
    VPlansAndVFs[Entry.second.get()].push_back(Entry.first);
  for (auto &Entry : VPlansAndVFs) {
    std::sort(Entry.second.begin(), Entry.second.end());
    printPlan(Entry.first, Entry.second, Title);
  }
}
...
public:
  ...
  void print(raw_ostream &O) const override {
    ...
    for (auto It = Begin; It != End; ++It) {
      O << "\\l  ";
      VPlanPrinter::printAsIngredient(O, &*It);
...
/// A recipe which handles all phi nodes except integer inductions.
class VPWidenPHIRecipe : public VPRecipeBase {
  PHINode *Phi;

public:
  VPWidenPHIRecipe(PHINode *Phi, VPlan *Plan) : VPRecipeBase(VPWidenPHISC),
                                                Phi(Phi) {
    Plan->setInst2Recipe(Phi, this);
  }
  ...
  /// The method which generates the phi/select nodes, thereby "executing" the
  /// VPlan.
...
public:
  VPWidenIntOrFpInductionRecipe(bool NeedsScalarIV, PHINode *IV,
                                TruncInst *Trunc = nullptr)
      : VPRecipeBase(VPWidenIntOrFpInductionSC), NeedsScalarIV(NeedsScalarIV),
        IV(IV), Trunc(Trunc) {}
  ...
  getInductionDescriptor(LoopVectorizationLegality *Legal) {
...
private:
  VPWidenIntOrFpInductionRecipe *WIFI;
  Instruction *EntryVal;
  VPLaneRange DesignatedLanes;

public:
  VPBuildScalarStepsRecipe(VPWidenIntOrFpInductionRecipe *WIFI,
                           Instruction *EntryVal, VPlan *Plan)
      : VPRecipeBase(VPBuildScalarStepsSC), WIFI(WIFI), EntryVal(EntryVal) {
...
public:
  ...
  /// Print the recipe.
  void print(raw_ostream &O) const override {
    O << "EXTRACT MASK BIT:\\l  " << MaskedBasicBlock->getName();
  }
...
public:
  ...
  /// Print the recipe.
  void print(raw_ostream &O) const override {
    O << "MERGE SCALARIZE BRANCH:\\l  ";
    VPlanPrinter::printAsIngredient(O, LiveOut);
  }
};
...
std::pair<VPRecipeBase *, VPRecipeBase *>
LoopVectorizationPlanner::widenIntOrFpInduction(VPlan *Plan,
                                                unsigned StartRangeVF,
                                                unsigned &EndRangeVF,
                                                PHINode *IV, TruncInst *Trunc) {
  ...
  auto NeedsScalarInduction = [&](unsigned VF) -> bool {
    if (VF == 1)
      return false;
    if (shouldScalarizeInstruction(EntryVal, VF))
      return true;
    auto isScalarInst = [&](User *U) -> bool {
      auto *I = cast<Instruction>(U);
      return (TheLoop->contains(I) && shouldScalarizeInstruction(I, VF));
    };
    return any_of(EntryVal->users(), isScalarInst);
  };
  bool NeedsScalarIV =
      testVFRange(NeedsScalarInduction, StartRangeVF, EndRangeVF);
  // Generate the widening recipe.
  auto *WIFIRecipe = new VPWidenIntOrFpInductionRecipe(NeedsScalarIV, IV,
                                                       Trunc);
  if (!NeedsScalarIV)
    return std::make_pair<VPRecipeBase *, VPRecipeBase *>(WIFIRecipe, nullptr);
  ...
  // each scalar step.
  auto *BSSRecipe = new VPBuildScalarStepsRecipe(WIFIRecipe, EntryVal, Plan);
  // Determine the number of scalars we need to generate for each unroll
  ...
  return std::make_pair<VPRecipeBase *, VPRecipeBase *>(WIFIRecipe, BSSRecipe);
}
    ...
    for (auto I = BB->begin(), E = BB->end(); I != E; ++I) {
      ...
      if (PHINode *Phi = dyn_cast<PHINode>(Instr)) {
        ...
        if (II.getKind() == InductionDescriptor::IK_IntInduction ||
          ...
          auto Recipes = widenIntOrFpInduction(Plan, StartRangeVF, EndRangeVF,
                                               Phi);
        ...
        // Assign the default recipe for phi nodes.
        appendRecipe(new VPWidenPHIRecipe(Phi, Plan));
        continue;
      }
    ...
    while (!Worklist.empty()) {
      ...
      if (isa<PHINode>(I)) {
        ...
        if (any_of(Worklist, IsNotAPhi) ||
          ...
          Changed = true; // Make sure this phi gets a second chance.
          continue;
        }
      }
      ...
      for (auto &U : I->uses()) {
        ...
        if (isa<GetElementPtrInst>(I) &&
            (isa<VPInterleaveRecipe>(UserRecipe) ||
             (isa<VPVectorizeOneByOneRecipe>(UserRecipe) &&
        ...
        bool UserIsScalarized = isa<VPScalarizeOneByOneRecipe>(UserRecipe) ||
          isa<VPBuildScalarStepsRecipe>(UserRecipe);

        // Scalarizing recipes can have non-scalarizing users. Note that
        // build-scalar-steps cannot - vector users of induction variables use
        // the instructions generated by widen-phi recipes.
        if (isa<VPScalarizeOneByOneRecipe>(Recipe) && !UserIsScalarized) {
        ...
        if (UserRecipe->getParent() != PredBB && UserIsScalarized) {
          // Don't make a decision until all scalarized users have sunk. Note
          // that if the user is not scalarized then it has either prevented us
          // from reaching this point or is irrelevant.
        ...
        else if (auto *SOBO = dyn_cast<VPScalarizeOneByOneRecipe>(UserRecipe))
          DesignatedLanes = SOBO->getDesignatedLanes();
        else; // This is an irrelevant non-scalarized user
...
void VPWidenPHIRecipe::vectorize(VPTransformState &State) {
  State.ILV->widenPHIInstruction(Phi, State.UF, State.VF);
}

void VPWidenPHIRecipe::print(raw_ostream &O) const {
  O << "WIDEN PHI";
  O << ":\\l  ";
  VPlanPrinter::printAsIngredient(O, Phi);
}

void VPWidenIntOrFpInductionRecipe::vectorize(VPTransformState &State) {
  assert(State.Instance == nullptr && "Int induction being replicated");
  auto BuildScalarInfo =
    State.ILV->widenIntOrFpInduction(getInductionDescriptor(State.Legal),
                                     NeedsScalarIV, IV, Trunc);
  ScalarIV = BuildScalarInfo.first;
  Step = BuildScalarInfo.second;
}

void VPWidenIntOrFpInductionRecipe::print(raw_ostream &O) const {
  O << "WIDEN INT/FP INDUCTION";
  if (NeedsScalarIV)
    O << " (needs scalars)";
  O << ":";
  O << "\\l  ";
  VPlanPrinter::printAsIngredient(O, IV);
  if (Trunc) {
    O << "\\l  ";
    VPlanPrinter::printAsIngredient(O, Trunc);
  }
}
...
void VPBuildScalarStepsRecipe::vectorize(VPTransformState &State) {
  ...
  // Generate relevant lanes.
  State.ILV->buildScalarSteps(WIFI->getScalarIV(), WIFI->getStep(), EntryVal,
                              WIFI->getInductionDescriptor(State.Legal),
...
void VPBuildScalarStepsRecipe::print(raw_ostream &O) const {
  ...
  O << ":\\l  ";
  VPlanPrinter::printAsIngredient(O, EntryVal);
}
...
void VPInterleaveRecipe::print(raw_ostream &O) const {
  O << "INTERLEAVE GROUP with factor " << IG->getFactor()
    << " at "; IG->getInsertPos()->printAsOperand(O, false);
  for (unsigned i = 0; i < IG->getFactor(); ++i)
    if (Instruction *I = IG->getMember(i)) {
      O << "\\l  ";
      VPlanPrinter::printAsIngredient(O, I);
      O << " " << i;
      if (willAlsoPackOrUnpack(I))
        O << " (V->S)";
    }
...
void VPExtractMaskBitRecipe::vectorize(VPTransformState &State) {
  ...
  ConditionBit = Cond[Part];
  if (ConditionBit->getType()->isVectorTy())
    ConditionBit =
      State.Builder.CreateExtractElement(ConditionBit,
                                         State.ILV->Builder.getInt32(Lane));
  ConditionBit =
...
void VPlanUtilsLoopVectorizer::removeInstruction(Instruction *Inst,
  ...
  // If source recipe is now empty, remove it.
  if (OBORecipe && OBORecipe->Begin == OBORecipe->End) {
    OBORecipe->getParent()->removeRecipe(OBORecipe);
    delete OBORecipe;
  }
}
...
void VPlanUtilsLoopVectorizer::sinkInstruction(Instruction *Inst,
  ...
  if (auto *FromBSSRecipe = dyn_cast<VPBuildScalarStepsRecipe>(FromRecipe)) {
    ...
    if (MinLane == 0) {
      ...
      SunkRecipe = new VPBuildScalarStepsRecipe(FromBSSRecipe->WIFI,
