...
static Loop *buildClonedLoops(Loop &OrigL, ArrayRef<BasicBlock *> ExitBlocks,
  ...
  for (auto *BB : OrigL.blocks())
    if (auto *ClonedBB = cast_or_null<BasicBlock>(VMap.lookup(BB))) {
      PossibleClonedLoopBlockSet.insert(ClonedBB);
  ...
  for (auto *Pred : predecessors(ClonedHeader)) {
    ...
    // recurse through.
    if (ClonedLoopBlockSet.insert(Pred).second && Pred != ClonedHeader)
  ...
  if (!ClonedLoopBlockSet.empty()) {
    ClonedLoopBlockSet.insert(ClonedHeader);

    while (!Worklist.empty()) {
      BasicBlock *BB = Worklist.pop_back_val();
      assert(ClonedLoopBlockSet.count(BB) &&
    ...
    ClonedL->reserveBlocks(ClonedLoopBlockSet.size());
    ...
    for (auto *BB : OrigL.blocks()) {
      ...
      if (!ClonedBB || !ClonedLoopBlockSet.count(ClonedBB))
        continue;
    ...
    for (Loop *ChildL : OrigL) {
      ...
      if (!ClonedChildHeader || !ClonedLoopBlockSet.count(ClonedChildHeader))
        continue;
      ...
      for (auto *ChildLoopBB : ChildL->blocks())
        assert(ClonedLoopBlockSet.count(
    ...
    do {
      ...
      for (BasicBlock *PredBB : predecessors(BB)) {
        ...
        if (!UnloopedBlockSet.erase(PredBB)) {
          assert(
              (ClonedLoopBlockSet.count(PredBB) || ExitLoopMap.count(PredBB)) &&
...
static bool unswitchInvariantBranch(
  ...
  auto CloneBlock = [&](BasicBlock *OldBB) {
    ...
    BasicBlock *NewBB =
        CloneBasicBlock(OldBB, VMap, ".us", ParentBB->getParent());
