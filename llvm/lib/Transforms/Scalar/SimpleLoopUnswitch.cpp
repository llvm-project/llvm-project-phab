...
static Loop *cloneLoop(Loop &OrigL, Loop *ParentL, ValueToValueMapTy &VMap,
                       LoopInfo &LI) {
  Loop *ClonedL = new Loop();
  if (ParentL)
    ParentL->addChildLoop(ClonedL);
  else
    LI.addTopLevelLoop(ClonedL);

  // Add all of the blocks in the original loop to the cloned loop.
  for (BasicBlock *BB : OrigL.blocks())
    if (LI.getLoopFor(BB) == &OrigL)
      ClonedL->addBasicBlockToLoop(cast<BasicBlock>(VMap.lookup(BB)), LI);
...
static Loop *buildClonedLoops(Loop &OrigL, ArrayRef<BasicBlock *> ExitBlocks,
  ...
  for (auto *ExitBB : ExitBlocks)
    if (auto *ClonedExitBB = cast_or_null<BasicBlock>(VMap.lookup(ExitBB)))
      if (Loop *ExitL = LI.getLoopFor(ExitBB)) {
        ExitL->addBasicBlockToLoop(ClonedExitBB, LI);
  ...
  SmallPtrSet<BasicBlock *, 16> PossibleClonedLoopSet;
  ...
  for (auto *BB : OrigL.blocks())
    if (auto *ClonedBB = cast_or_null<BasicBlock>(VMap.lookup(BB))) {
      PossibleClonedLoopSet.insert(ClonedBB);
      ClonedLoopBlocks.push_back(ClonedBB);
    }
  ...
  for (auto *Pred : predecessors(ClonedHeader)) {
    ...
    // we know we cloned the loop in simplified form. We also check that this
    // is in the possible set.
    if (Pred == ClonedPH || !PossibleClonedLoopSet.count(Pred))
      continue;
    ...
    // recurse through.
    if (ClonedLoopSet.insert(Pred).second && Pred != ClonedHeader)
  ...
  if (!ClonedLoopSet.empty()) {
    ClonedLoopSet.insert(ClonedHeader);

    while (!Worklist.empty()) {
      BasicBlock *BB = Worklist.pop_back_val();
      assert(ClonedLoopSet.count(BB) && "Didn't put block into the loop set!");

      // Insert any predecessors that are in the posible set into the cloned set,
      // and if the insert is successful, add them to the worklist.
      for (auto *Pred : predecessors(BB))
        if (PossibleClonedLoopSet.count(Pred) && ClonedLoopSet.insert(Pred).second)
          Worklist.push_back(Pred);
    }
    ...
    ClonedL->reserveBlocks(ClonedLoopSet.size());
    ...
    for (auto *BB : OrigL.blocks())
      if (LI.getLoopFor(BB) == &OrigL)
        if (auto *ClonedBB = cast_or_null<BasicBlock>(VMap.lookup(BB)))
          if (ClonedLoopSet.count(ClonedBB))
    ...
    // child loop nest.
    for (Loop *ChildL : OrigL)
      if (auto *ClonedChildHeader =
              cast_or_null<BasicBlock>(VMap.lookup(ChildL->getHeader())))
        if (ClonedLoopSet.count(ClonedChildHeader)) {
#ifndef NDEBUG
  ...
  std::stable_sort(ClonedExitsInLoops.begin(), ClonedExitsInLoops.end(),
  ...
  while (!UnloopedBlockSet.empty() && !ClonedExitsInLoops.empty()) {
    assert(Worklist.empty() && "Didn't clear worklist!");
    assert(ExitLoopSet.empty() && "Didn't clear loop set!");

    BasicBlock *ExitBB = ClonedExitsInLoops.pop_back_val();
    ...
    do {
      ...
      for (BasicBlock *PredBB : predecessors(BB)) {
        ...
        if (!UnloopedBlockSet.erase(PredBB)) {
          assert((ExitLoopSet.count(PredBB) ||
        ...
        bool Inserted = ExitLoopSet.insert(PredBB).second;
        (void)Inserted;
    ...
    // even though that is grossly inefficient. FIXME: Find a better source of
    // ordering here.
    if (ExitLoopSet.erase(ClonedPH))
      ExitL.addBasicBlockToLoop(ClonedPH, LI);
    for (auto *ClonedBB : ClonedLoopBlocks)
      if (ExitLoopSet.erase(ClonedBB))
        ExitL.addBasicBlockToLoop(ClonedBB, LI);
    assert(ExitLoopSet.empty() &&
           "Failed to find and handle all blocks in the exit loop set!");
  }
  ...
  for (Loop *ChildL : OrigL)
    if (auto *ClonedChildHeader =
      ...
      if (!ClonedLoopSet.count(ClonedChildHeader)) {
        ...
        for (auto *ChildLoopBB : ChildL->blocks())
          assert(
              VMap.count(ChildLoopBB) &&
              "Cloned a child loop header but not all of that loops blocks!");
#endif
        NonChildClonedLoops.push_back(
            cloneLoop(*ChildL, LI.getLoopFor(ClonedChildHeader), VMap, LI));
      }
...
void recursivelyEraseNode(DominatorTree &DT, BasicBlock *BB) {
  ...
  Worklist.push_back(DT[BB]);
  do {
    DomTreeNode *N = Worklist.pop_back_val();
    if (N->getChildren().empty()) {
      DT.eraseNode(N->getBlock());
      continue;
    }

    Worklist.push_back(N);
    Worklist.append(N->begin(), N->end());
  } while (!Worklist.empty());
}
...
static bool rebuildLoopAfterUnswitch(Loop &L, ArrayRef<BasicBlock *> ExitBlocks,
  ...
  for (auto *ExitBB : ExitBlocks)
    if (Loop *ExitL = LI.getLoopFor(ExitBB)) {
      // FIXME: ExitL->addBasicBlockToLoop(ClonedExitBB, LI);
      ExitLoops.push_back(ExitL);
  ...
  // We essentially need to recompute the set of blocks in the loop. We will do
  // this by walking backwards from the header's backedges. We constrain our
  // search to blocks that were at least originally part of the loop. We also
  // skip over inner loops which, if we encounter, we know remain valid and
  // unchanged in structure. We still have to filter by the original set of
  // blocks however because there may be unreachable code that isn't valid to
  // traverse.
  SmallVector<BasicBlock *, 16> Worklist;
  SmallPtrSet<const BasicBlock *, 16> LoopSet;
  for (auto *Pred : predecessors(Header)) {
    // The only possible non-loop header predecessor is the preheader because
    // we know we cloned the loop in simplified form. We also check that this
    // is in the possible set.
    if (Pred == PH || !L.contains(Pred))
      continue;

    // Insert this block into the loop set and on the first visit (and if it
    // isn't the header we're currently walking) put it into the worklist to
    // recurse through.
    if (LoopSet.insert(Pred).second && Pred != Header)
      Worklist.push_back(Pred);
  }

  // If we successfully found backedges then we still have a loop.
  if (!LoopSet.empty()) {
    // We need to re-set the loop's parent as the exit block set changing may
    // have moved it within the loop nest. Note that this can only happen when
    // this loop has a parent as it can only hoist the loop *up* the nest.
    ...
    // Add the header to the set and complete it by walking the entire CFG in
    // reverse.
    LoopSet.insert(Header);
    while (!Worklist.empty()) {
      BasicBlock *BB = Worklist.pop_back_val();
      assert(LoopSet.count(BB) && "Didn't put block into the loop set!");
      ...
      if (Loop *InnerL = LI.getLoopFor(BB))
        if (InnerL != &L) {
          assert(L.contains(InnerL) && "Should not reach a loop *outside* this loop!");
          ...
          assert(L.contains(InnerPH) && "Cannot contain an inner loop block "
                                        ...
                                        "preheader!");
          if (!LoopSet.insert(InnerPH).second)
          ...
          for (auto *InnerBB : InnerL->blocks()) {
            if (InnerBB == BB) {
              assert(LoopSet.count(InnerBB) &&
                     "Block should already be in the set!");
              continue;
            }

            bool Inserted = LoopSet.insert(InnerBB).second;
            (void)Inserted;
      ...
      // Insert any predecessors that were in the original loop into the new set,
      // and if the insert is successful, add them to the worklist.
      for (auto *Pred : predecessors(BB))
        if (L.contains(Pred) && LoopSet.insert(Pred).second)
          Worklist.push_back(Pred);
    }
  }
  ...
  auto BlocksSplitI =
      LoopSet.empty() ? Blocks.begin()
                      : std::stable_partition(
                            Blocks.begin(), Blocks.end(), [&](BasicBlock *BB) {
                              return LoopSet.count(BB);
                            });
  ...
  auto RemoveUnloopedBlocksFromLoop = [&](Loop &L) {
    for (auto *BB : UnloopedSet)
      L.getBlocksSet().erase(BB);
    llvm::erase_if(L.getBlocksVector(),
                   [&](BasicBlock *BB) { return UnloopedSet.count(BB); });
  };

  while (!UnloopedSet.empty() && !ExitsInLoops.empty()) {
    assert(Worklist.empty() && "Didn't clear worklist!");
    assert(ExitLoopSet.empty() && "Didn't clear loop set!");
    ...
    for (; PrevExitL != &ExitL; PrevExitL = PrevExitL->getParentLoop())
      RemoveUnloopedBlocksFromLoop(*PrevExitL);
  ...
  auto SubLoopsSplitI =
      LoopSet.empty() ? SubLoops.begin()
                      : std::stable_partition(
                            SubLoops.begin(), SubLoops.end(), [&](Loop *SubL) {
                              return LoopSet.count(SubL->getHeader());
                            });
  ...
  if (Blocks.empty()) {
    ...
    // We use the LoopInfo's removal facility even though it is overpowered for
    // what we need because the code above naturally handles all the
    // interesting updates. But this is the simplest way to actually delete the
    // now empty loop itself.
    LI.markAsRemoved(&L);
    return false;
  }
...
static bool unswitchInvariantBranch(
  ...
  auto CloneBlock = [&](BasicBlock *OldBB) {
    ...
    BasicBlock *NewBB = CloneBasicBlock(OldBB, VMap, ".us", ParentBB->getParent());
    ...
    // Add the block to the domtree. We'll move it to the correct position below.
  ...
  for (auto *ExitBB : ExitBlocks) {
    ...
    // dominated by one header, and the other by another, so we know this is
    // a conservative common dominator. While the dominator tree isn't fully
    ...
    for (auto ZippedInsts : llvm::zip_first(
      ...
      assert((isa<PHINode>(I) || isa<LandingPadInst>(I) || isa<CatchPadInst>(I)) &&
  ...
  // Replace the cloned branch with an unconditional branch to the clone's
  // successor.
  ...
  Loop *ClonedL = buildClonedLoops(L, ExitBlocks, VMap, LI, NonChildClonedLoops);
  ...
  if (OuterExitL != &L) {
    ...
    // become top level loops we need to walk all the way out.
    if (ClonedL) {
    ...
    // Finally, update our list of outer loops. This is nicely ordered to work inside-out.
...
int computeDomSubtreeCost(DomTreeNode &N,
  ...
  int Cost = std::accumulate(N.begin(), N.end(), BBCostIt->second,
                             [&](int Sum, DomTreeNode *ChildN) {
                               return Sum + computeDomSubtreeCost(
                                                *ChildN, BBCostMap, DTCostMap);
                             });
...
unswitchLoop(Loop &L, DominatorTree &DT, LoopInfo &LI, AssumptionCache &AC,
  ...
  TerminatorInst *UnswitchTI = nullptr;
  int UnswitchCost;
  ...
  if (UnswitchCost < UnswitchThreshold) {
    DEBUG(dbgs() << "  Trying to unswitch non-trivial (cost = " << UnswitchCost
                 << ") branch: " << *UnswitchTI << "\n");
    Changed |= unswitchInvariantBranch(L, cast<BranchInst>(*UnswitchTI), DT, LI,
                                       AC, NonTrivialUnswitchCB);
  } else {
    DEBUG(dbgs() << "Cannot unswitch, lowest cost found: " << UnswitchCost
                 << "\n");
  }
...
PreservedAnalyses SimpleLoopUnswitchPass::run(Loop &L, LoopAnalysisManager &AM,
  ...
  auto NonTrivialUnswitchCB = [&L, &U](bool CurrentLoopValid,
