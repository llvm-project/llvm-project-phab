...
static Loop *buildClonedLoops(Loop &OrigL, ArrayRef<BasicBlock *> ExitBlocks,
  ...
  assert((ParentL == OrigL.getParentLoop() ||
  ...
  for (auto *Pred : predecessors(ClonedHeader)) {
    ...
    if (Pred == ClonedPH || !ClonedLoopBlocks.count(Pred))
      continue;
  ...
  if (!BlocksInClonedLoop.empty()) {
    ...
    // a way that is stable (rather than relying on predecessor ordering which
    // is less stable). Rather than re-invent that logic, we just re-walk the
    // original blocks (and those of the child loops) and filter them as we add
    // them into the cloned loop.
    for (auto *BB : OrigL.blocks()) {
