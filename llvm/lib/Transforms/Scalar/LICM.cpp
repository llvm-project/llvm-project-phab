...
static bool isFreeInLoop(const Instruction *I, const Instruction *UserI,
                         const TargetTransformInfo *TTI) {
  /// FIXME: for now we simply check if the addressing mode defined by a GEP is
  /// directly foldable into a load/store. The GEP could also be a part of
  /// addressing mode in a load/store instruction through other instructions
  /// (e.g., sext or other GEPs).
  /// We might not even have to restrict this to GEPs, but rather, we could
  /// allow all zero-cost instructions. However, we need to take wide tests
  /// before opening up this for all other instructions.
  const GetElementPtrInst *GEP = dyn_cast<GetElementPtrInst>(I);
  // Specificially check if the GEP is used by a load/store in the same block.
  if (GEP && (isa<StoreInst>(UserI) || isa<LoadInst>(UserI)) &&
      (I->getParent() == UserI->getParent())) {
