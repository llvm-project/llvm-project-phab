...
LoopStructure::parseLoopStructure(ScalarEvolution &SE,
  ...
  const SCEVAddRecExpr *IVRec = cast<SCEVAddRecExpr>(LeftSCEV);
  bool IsIncreasing = false;
  bool IsSignedPredicate = true;
  bool IsIndVarNext = false;
  ConstantInt *StepCI;
  if (!IsInductionVar(IVRec, IsIncreasing, StepCI)) {
  ...
  if (isa<PHINode>(LeftValue) &&
    ...
    // The comparison is made against IV.
    IndVarStart = IVRec->getStart();
  } else {
    // Assume that the comparison is made against IV.next.
    const SCEV *StartNext = IVRec->getStart();
    const SCEV *Addend = SE.getNegativeSCEV(IVRec->getStepRecurrence(SE));
