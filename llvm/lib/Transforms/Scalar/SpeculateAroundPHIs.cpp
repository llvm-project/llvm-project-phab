...
/// Check wether speculating a particular user of a PHI node around the PHI
/// will be safe.
///
/// This checks both that this user is safe but also that all of its operands
/// are either recursively safe or already available.
///
/// Note that caching the results of this exploration in `PotentialSpecSet` is
/// crucial to reasonable algorithmic performance here. We call this routine on
/// each user of a PHI node and will re-walk the same subgraphs repeatedly if
/// we don't cache things in this set.
static bool
isSafeToSpeculatePHIUser(PHINode &PN, Instruction &UserI, DominatorTree &DT,
                         SmallPtrSetImpl<Instruction *> &PotentialSpecSet) {
  if (PotentialSpecSet.count(&UserI))
    return true;
  ...
  // change this to do at least a limited scan of the intervening instructions
  // and allow handling stores in easily proven safe cases.
  if (mayBeMemoryDependent(UserI)) {
    DEBUG(dbgs() << "  Unsafe: can't speculate use: " << UserI << "\n");
    return false;
  }

  auto *PhiBB = PN.getParent();

  SmallPtrSet<Instruction *, 4> SpecSet;

  SmallVector<Instruction *, 4> Worklist;
  // Queue up the operands of the user to check recursively -- we've already
  // checked the user itself.
  for (Value *OpV : UserI.operand_values())
    // All non-instruction values are inherently available, only recurse
  ...
  do {
    ...
    if (ParentBB == PhiBB) {
      if (isa<PHINode>(I)) {
        assert(I != &UserI &&
               "Cannot handle a PHI user, this should be checked above.");
    ...
    // If we've already explored this subgraph and found it to be safe, we can
    // skip it.
    if (PotentialSpecSet.count(I))
      continue;

    // Insert into the set we will try to speculate.
    if (!SpecSet.insert(I).second)
      // If we've already checked this instruction, continue.
      continue;

    // Otherwise we'll need to speculate into the incoming edge, check that
    // this is valid.
    if (mayBeMemoryDependent(*I)) {
      DEBUG(dbgs() << "  Unsafe: can't speculate transitive use: " << *I
                   << "\n");
      return false;
    }

    // And then recurse through all of the operands.
  ...
  // All checks passed! Fold the speculation set we just explored into the safe
  // set to avoid re-exploring it, and return the result.
  PotentialSpecSet.insert(SpecSet.begin(), SpecSet.end());
  return true;
}
...
static bool isSafeAndProfitableToSpeculateAroundPHI(
    ...
    SmallPtrSetImpl<Instruction *> &PotentialSpecSet, DominatorTree &DT,
  ...
  // occur.
  int MatCost = TTI.TCC_Free;
  SmallDenseMap<ConstantInt *, int, 16> IncomingConstants;
  ...
  for (int i : llvm::seq<int>(0, PN.getNumIncomingValues())) {
    ...
    // Only track the first time we see a given incoming block with a constant.
  ...
  int FoldedImmCost = TTI.TCC_Free;
  for (Use &U : PN.uses()) {
    auto *UserI = cast<Instruction>(U.getUser());
    // Check whether speculating this use around the PHI node is safe.
    if (!isSafeToSpeculatePHIUser(PN, *UserI, DT, PotentialSpecSet)) {
    ...
    for (auto IncomingConstantAndCount : IncomingConstants) {
      ConstantInt *IncomingC = IncomingConstantAndCount.first;
      int Count = IncomingConstantAndCount.second;

      int Cost;
      if (IID)
        Cost = TTI.getIntImmCost(IID, Idx, IncomingC->getValue(),
                                 IncomingC->getType());
      else
        Cost = TTI.getIntImmCost(UserI->getOpcode(), Idx, IncomingC->getValue(),
                                 IncomingC->getType());

      FoldedImmCost += Count * Cost;

      // Early exit if we exceed the basic materialization cost.
      if (FoldedImmCost > MatCost) {
        DEBUG(dbgs() << "  Not profitable, folded cost: " << PN << "\n"
                        "    Materializing cost:    " << MatCost << "\n"
                        "    Folded immediate cost: " << FoldedImmCost << "\n");
        return false;
      }
    }
  }
  ...
  DEBUG(dbgs() << "    Cost savings " << (MatCost - FoldedImmCost) << ": " << PN
               << "\n");
  CostSavingsMap[&PN] = MatCost - FoldedImmCost;
  return true;
}

template <typename IsVisitedT, typename VisitT>
static void visitPHIUsersAndDepsInPostOrder(
    ArrayRef<PHINode *> PNs,
    const SmallPtrSetImpl<Instruction *> &PotentialDepSet, IsVisitedT IsVisited,
    VisitT Visit) {
    ...
    for (auto &U : PN->uses()) {
      ...
      do {
        ...
        while (OpIt != UI->value_op_end()) {
          ...
          // to the next.
          if (!OpI || !PotentialDepSet.count(OpI) || IsVisited(OpI))
            continue;
...
findProfitablePHIs(ArrayRef<PHINode *> PNs,
  ...
  visitPHIUsersAndDepsInPostOrder(
      PNs, PotentialSpecSet,
      /*IsVisited*/ [&](Instruction *I) { return SpecCostMap.count(I); },
      /*Visit*/ [&](Instruction *I) {
...
static void speculatePHIs(ArrayRef<PHINode *> SpecPNs,
  ...
  visitPHIUsersAndDepsInPostOrder(
      SpecPNs, PotentialSpecSet,
      /*IsVisited*/ [&](Instruction *I) { return SpecSet.count(I); },
      /*Visit*/ [&](Instruction *I) {
        // All operands scheduled, schedule this node.
        SpecSet.insert(I);
...
static bool tryToSpeculatePHIs(SmallVectorImpl<PHINode *> &PNs,
  ...
  // If we find that a particular instruction is unsafe to speculate, remember
  // that and avoid re-checking it. This is important in case we only discover
  // the problem after walking up N levels of operands.
  ...
  PNs.erase(llvm::remove_if(PNs,
                            [&](PHINode *PN) {
                              return !isSafeAndProfitableToSpeculateAroundPHI(
                                  *PN, CostSavingsMap, PotentialSpecSet, DT,
                                  TTI);
                            }),
            PNs.end());
