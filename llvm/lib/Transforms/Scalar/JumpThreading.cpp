...
findColdBlockBasedOnMetaData(TerminatorInst *TI,
  ...
  WeightHot  = std::max<uint64_t>(WeightTrue, WeightFalse);
  WeightCold = std::min<uint64_t>(WeightTrue, WeightFalse);
...
setMetaDataBasedOnColdBlock(BasicBlock* ColdBlock, unsigned WeightHot, unsigned WeightCold) {
  ...
  // We identify the BBs post-dominated by ColdBlock as the cold region.
