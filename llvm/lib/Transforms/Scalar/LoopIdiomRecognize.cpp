...
bool LoopIdiomRecognize::processLoopStoreOfLoopLoad(
  ...
  for (unsigned i = 0, e = SL.size(); i < e; ++i) {
    ...
    assert(FirstStoreLoad->isUnordered() && "Expected only non-volatile non-ordered loads.");
    Value *FirstLoadPtr =
	...
	if(i==0)
      ...
      assert(SL[k]->isUnordered() && "Expected only non-volatile non-ordered stores.");
      Value *SecondStorePtr = SL[k]->getPointerOperand();
      const SCEVAddRecExpr *SecondStoreEv =
                        cast<SCEVAddRecExpr>(SE->getSCEV(SecondStorePtr));
      ...
      assert(SecondStoreLoad->isUnordered() && "Expected only non-volatile non-ordered loads.");
      Value *SecondLoadPtr =
