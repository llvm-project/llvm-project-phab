...
// 5. Update DOM
//
// TODO: Add const to const-functions.
//===----------------------------------------------------------------------===//
...
private:
  ...
  void addNewPhisToNewHeader(const SmallVectorImpl<BasicBlock *> &Blocks,
...
bool LoopRotate::isProfitableToRotate(const BasicBlock *OrigH,
  ...
  for(const BasicBlock *BB : Blocks) {
    ...
    // indirectbr, invoke in the loop but we can cut the cloning part
    // at that point and that will be the last exit BB.
...
void LoopRotate::addNewPhisToNewHeader(const SmallVectorImpl<BasicBlock *> &Blocks,
...
Pass *llvm::createLoopRotatePass() {
  return new LoopRotateLegacyPass();
}
