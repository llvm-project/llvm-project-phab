...
bool LoopRotate::processLoop() {
  ...
  if (!LoopLatch->getSingleSuccessor()) {
    ...
    // If the NewH is the loop-latch => NewH would change as the loop-latch has
  ...
  // Anything ScalarEvolution may know about this loop or the PHI nodes in its
  // header will soon be invalidated.
  if (SE)
    SE->forgetLoop(L);
