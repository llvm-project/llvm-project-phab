...
void MemorySanitizer::initializeKmsanCallbacks(Module &M) {
  ...
  GetRetvalTLSFn = M.getOrInsertFunction(
      "__kmsan_get_retval_tls",
      PointerType::get(ArrayType::get(IRB.getInt64Ty(), kRetvalTLSSize / 8),
                       0));
  GetRetvalOriginTLSFn = M.getOrInsertFunction("__kmsan_get_retval_origin_tls",
                                               PointerType::get(OriginTy, 0));
  GetParamTLSFn = M.getOrInsertFunction(
      "__kmsan_get_param_tls",
      PointerType::get(ArrayType::get(IRB.getInt64Ty(), kParamTLSSize / 8), 0));
  GetParamOriginTLSFn = M.getOrInsertFunction(
      "__kmsan_get_param_origin_tls",
      PointerType::get(ArrayType::get(OriginTy, kParamTLSSize / 4), 0));
  GetVAArgTLSFn = M.getOrInsertFunction(
      "__kmsan_get_va_arg_tls",
      PointerType::get(ArrayType::get(IRB.getInt64Ty(), kParamTLSSize / 8), 0));
  GetVAArgOverflowSizeTLSFn =
      M.getOrInsertFunction("__kmsan_get_va_arg_overflow_size_tls",
                            PointerType::get(IRB.getInt64Ty(), 0));
  GetOriginTLSFn = M.getOrInsertFunction("__kmsan_get_origin_tls",
  ...
  for (int ind = 0, size = 1; ind < 4; ind++, size <<= 1) {
    ...
    LoadShadowOrigin_1_8_Fn[ind] =
        M.getOrInsertFunction(name_load, ShadowOriginStruct[ind],
