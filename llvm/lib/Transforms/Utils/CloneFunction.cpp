...
extern cl::opt<bool> DisableCygprofileInlining;
...
void PruningFunctionCloner::CloneBlock(const BasicBlock *BB,
  ...
  for (BasicBlock::const_iterator II = StartingInst, IE = --BB->end();
       II != IE; ++II) {
    // Drop call instructions generated by -fprofile-functions.
    if (DisableCygprofileInlining) {
      if (const CallInst *CI = dyn_cast<CallInst>(II)) {
        if (const Function *Callee = CI->getCalledFunction()) {
          if (Callee->getName() == "__cyg_profile_func_enter" ||
              Callee->getName() == "__cyg_profile_func_exit")
            continue;
        }
      }
    }
