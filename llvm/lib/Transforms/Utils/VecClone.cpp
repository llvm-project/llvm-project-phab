...
std::vector<Attribute> VecClone::getVectorVariantAttributes(Function &F) {
...
void VecClone::getFunctionsToVectorize(llvm::Module &M,
                                       std::vector<Function *> FuncVars) {
  ...
  for (auto It = M.begin(), End = M.end(); It != End; ++It) {
    Function &F = *It;
    if (F.hasFnAttribute(Attribute::HasVectorVariants)) {
...
Instruction *VecClone::expandReturn(Function *Clone, BasicBlock *EntryBlock,
  ...
  if (!LoadFromAlloca) {
    ...
    GetElementPtrInst *VecGep = GetElementPtrInst::Create(
        ReturnType->getElementType(), VecReturn, Phi, GepName);
...
void VecClone::updateScalarMemRefsWithVector(
    Function *Clone, BasicBlock *EntryBlock, BasicBlock *ReturnBlock,
    PHINode *Phi, SmallVectorImpl<ParmRef *> &VectorParmMap) {
...
void VecClone::removeScalarAllocasForVectorParams(
    SmallVectorImpl<ParmRef *> &VectorParmMap) {
  SmallVectorImpl<ParmRef *>::iterator VectorParmMapIt;
...
bool VecClone::runOnModule(Module &M) {
  ...
  std::vector<Function *> FunctionsToVectorize;
  for (auto It = M.begin(), End = M.end(); It != End; ++It) {
    Function &F = *It;
    if (F.hasFnAttribute(Attribute::HasVectorVariants))
  ...
  // variable value.
  //
  // IMPORTANT NOTE: SmallVector was used here because later we emit LLVM
  // instructions using the members of ParmRef, and these instructions should be
  // ordered consistently for easier testability.
  SmallVector<ParmRef *, 1> VectorParmMap;

  for (auto *I : FunctionsToVectorize) {
    ...
    for (; ItA != EndA; ++ItA) {
      ...
      StringRef AttributeKind = ItA->getKindAsString();
      if (VectorVariant::isVectorVariant(AttributeKind)) {
        // Found vector variant. Create a new function for it.
        VectorVariant Variant(AttributeKind);
        ...
        // The cloned function is still needed, but no additional processing is
        // necessary.
        if (isSimpleFunction(Clone, Variant, ReturnOnly))
          continue;

        BasicBlock *LoopBlock =
            splitEntryIntoLoop(Clone, Variant, &*EntryBlock);
        ...
        PHINode *Phi = createPhiAndBackedgeForLoop(
            Clone, &*EntryBlock, LoopBlock, LoopExitBlock, ReturnBlock,
            Variant.getVlen());
        ...
        Instruction *ExpandedReturn = expandVectorParametersAndReturn(
            ...
            VectorParmMap);
        updateScalarMemRefsWithVector(Clone, &*EntryBlock, ReturnBlock, Phi,
                                      VectorParmMap);

        // Update any linear variables with the appropriate stride. This
        // function will insert a mul/add sequence before the use of the
        // parameter. For linear pointer parameters, the stride calculation is
        // just a mul instruction using the loop induction var and the stride
        // value on the parameter. This mul instruction is then used as the
        // index of the gep that will be inserted before the next use of the
        // parameter. The function also updates the users of the parameter with
        // the new calculation involving the stride.
        ...
        // if/else blocks.
        if (Variant.isMasked()) {
          insertSplitForMaskedVariant(Clone, LoopBlock, LoopExitBlock, Mask,
                                      Phi);
        }
