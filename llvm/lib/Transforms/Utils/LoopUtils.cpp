...
bool llvm::isGuaranteedToExecute(const Instruction &Inst,
  ...
  if (SafetyInfo->ComputedEarlyExits) {
    ...
    const BasicBlock *IBB = Inst.getParent();
    for (Instruction *ExitInst : SafetyInfo->EarlyExits) {
      if (IBB == ExitInst->getParent()) {
        auto OBB = SafetyInfo->OBBMap.find(IBB);
        if (OBB == SafetyInfo->OBBMap.end())
          OBB = SafetyInfo->OBBMap
                 .insert({IBB, make_unique<OrderedBasicBlock>(IBB)}).first;
        if (!OBB->second->dominates(&Inst, ExitInst))
          return false;
      } else if (!DT->dominates(&Inst, ExitInst))
        return false;
    }
  } else {
