...
void DiagnosticHandler::isRemarkEnable(const std::string &PassName, RemarkInfo &Output) {
  Output.MissedOptRemarkEnable = 0;
  Output.PassedOptRemarkEnable = 0;
  Output.AnalysisRemarkEnable = 0;

	if (isMissedOptRemarkEnable(PassName))
      Output.MissedOptRemarkEnable = 1;

	if (isPassedOptRemarkEnable(PassName))
      Output.PassedOptRemarkEnable = 1;

  if (isAnalysisRemarkEnable(PassName))
...
bool  DiagnosticHandler::handleDiagnostics(const DiagnosticInfo &DI, void *Context) {
