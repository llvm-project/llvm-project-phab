  ...
  for (auto &SU : SUnits) {
    ...
    for (auto V : Objs) {
      ...
      if (I == PendingLoadsStores.end())
        continue;
      for (auto LdSt : I->second) {
        if (isSuccOrder(LdSt, &SU))
          continue;

        MachineInstr &LdStMI = *LdSt->getInstr();
        ...
        if (!TII->getMemOpBaseRegImmOfs(LdStMI, BaseReg1, Offset1, TRI) ||
            !TII->getMemOpBaseRegImmOfs(MI, BaseReg2, Offset2, TRI)) {
          SU.addPred(SDep(LdSt, SDep::Barrier));
          continue;
        }
        if (BaseReg1 == BaseReg2 && (int)Offset1 < (int)Offset2) {
          assert(TII->areMemAccessesTriviallyDisjoint(LdStMI, MI, AA) &&
        ...
        MachineMemOperand *MMO1 = *LdStMI.memoperands_begin();
