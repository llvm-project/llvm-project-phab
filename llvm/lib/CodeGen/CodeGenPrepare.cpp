    ...
    if (GlobalVariable *GV = dyn_cast<GlobalVariable>(GEP->getOperand(0))){
      if (GV->isConstant() && GV->hasDefinitiveInitializer()) {
        ...
        if ((!isa<ConstantArray>(Init) && !isa<ConstantDataArray>(Init)) ||
            ...
            Ty->getArrayNumElements() > Ty->getArrayElementType()->getScalarSizeInBits())
          return nullptr;

        // Check if the array's constant elements are suitable to our case.
        uint64_t ArrayElementCount = Init->getType()->getArrayNumElements();
        for (uint64_t i = 0; i < ArrayElementCount; i++) {
          ConstantInt *Elem = dyn_cast<ConstantInt>(Init->getAggregateElement(i));
        ...
        APInt ElemSize = APInt(ElemTy->getScalarSizeInBits(), ElemTy->getScalarSizeInBits());
