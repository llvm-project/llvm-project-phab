...
//
// * libunwind
// In WebAssembly EH, V8 engine is responsible for unwinding stack once an
...
//
// Unwinding stack is not done by libunwind but JavaScript engine, so the
// personality function in libcxxabi cannot be called from libunwind during
// the unwinding process. So after returned from a catch instruction, which
// means stack was unwound by the JS engine, we actively call a wrapper function
// in libunwind that in turn calls the real personality function.
//
// In Itanium EH, if the personality function decides there is no
// matching catch cluase in a call frame and there is no cleanup action to
// perform, the unwinder doesn't stop there and continues unwinding. But in Wasm
// EH, the unwinder stops at every call frame with a landing pad, after which
// the personality function is called actively from the code.
//
...
//    __wasm_landingpad_context.lsda = wasm.eh.lsda();
//    _Unwind_CallPersonality(exn);
//    int selector = __wasm.landingpad_context.seledtor;
...
class WasmEHPrepare : public FunctionPass {
  Type *LPadContextTy;           // type of 'struct _Unwind_LandingPadContext'
  GlobalVariable *LPadContextGV; // __wasm_landingpad_context

  // Field addresses of struct _Unwind_LandingPadContext
  Value *LPadIndexField;   // lpad_index field
  Value *PersonalityField; // personality field
  Value *LSDAField;        // lsda field
  Value *SelectorField;    // selector

  Function *CatchF;           // wasm.catch() intrinsic
  Function *LPadIndexF;       // wasm.eh.landingpad.index() intrinsic
  Function *LSDAF;            // wasm.eh.lsda() intrinsic
  Function *CallPersonalityF; // _Unwind_CallPersonality() wrapper

  void prepareLandingPad(BasicBlock *BB, unsigned Index);
...
public:
  ...
  WasmEHPrepare()
      : FunctionPass(ID), LPadContextTy(nullptr), LPadIndexField(nullptr),
        PersonalityField(nullptr), LSDAField(nullptr), SelectorField(nullptr),
        CatchF(nullptr), LPadIndexF(nullptr), CallPersonalityF(nullptr) {}
...
bool WasmEHPrepare::runOnFunction(Function &F) {
  ...
  LPadContextGV = cast<GlobalVariable>(
      M.getOrInsertGlobal("__wasm_landingpad_context", LPadContextTy));
...
void WasmEHPrepare::prepareLandingPad(BasicBlock *BB, unsigned Index) {
  ...
  IRB.CreateStore(IRB.CreateBitCast(F->getPersonalityFn(), IRB.getInt8PtrTy()),
