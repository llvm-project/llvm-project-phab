...
static Optional<APInt> ConstantFoldUnaryOp(unsigned Opcode, LLT DstTy,
                                           const unsigned Op,
                                           const MachineRegisterInfo *MRI) {
  ...
  if (MaybeOp1Cst) {
    ...
    case TargetOpcode::G_SEXT:
      return Val.sextOrTrunc(DstTy.getSizeInBits());
    case TargetOpcode::G_ANYEXT:
    case TargetOpcode::G_TRUNC:
    case TargetOpcode::G_ZEXT:
      return Val.zextOrTrunc(DstTy.getSizeInBits());
...
MachineInstrBuilder MachineIRBuilder::buildSExt(unsigned Res, unsigned Op) {
  validateTruncExt(Res, Op, true);
  if (auto Val =
          ConstantFoldUnaryOp(TargetOpcode::G_SEXT, MRI->getType(Res), Op, MRI))
    return buildConstant(Res, Val->getSExtValue());
...
MachineInstrBuilder MachineIRBuilder::buildZExt(unsigned Res, unsigned Op) {
  validateTruncExt(Res, Op, true);
  if (auto Val =
          ConstantFoldUnaryOp(TargetOpcode::G_ZEXT, MRI->getType(Res), Op, MRI))
