...
void SelectionDAGBuilder::lowerWorkItem(SwitchWorkListItem W, Value *Cond,
  ...
  for (MachineCaseClusterIt I = W.FirstCluster, E = W.LastCluster; I <= E; ++I) {
...
void SelectionDAGBuilder::visitSwitch(const SwitchInst &SI) {
  BranchProbabilityInfo *BPI = FuncInfo.BPI;
  CaseClusterVector IndexClusters;
  ...
  MachineClusters.reserve(IndexClusters.size());

  for (CaseCluster &C : IndexClusters) {
    ...
    if (C.Kind == CC_Range) {
      MachineClusters.push_back(MachineCaseCluster::range(
          C.Low, C.High, FuncInfo.MBBMap[C.getSingleSuccessor(&SI)],
          ClusterProb));
    } else if (C.Kind == CC_JumpTable) {
      unsigned JTCasesIndex = prepareJumpTable(&SI, C, DefaultMBB);
      MachineClusters.push_back(MachineCaseCluster::jumpTable(
          C.Low, C.High, JTCasesIndex, ClusterProb));
    } else if (C.Kind == CC_BitTests) {
      unsigned BTCasesIndex = prepareBitTests(&SI, C, DefaultMBB, ClusterProb);
      MachineClusters.push_back(MachineCaseCluster::bitTests(
          C.Low, C.High, BTCasesIndex, ClusterProb));
    } else
