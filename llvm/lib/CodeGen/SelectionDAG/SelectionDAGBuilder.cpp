...
void SelectionDAGBuilder::lowerWorkItem(SwitchWorkListItem W, Value *Cond,
  ...
  if (Size == 2 && W.MBB == SwitchMBB) {
    ...
    MachineCaseCluster &Small = *W.FirstCluster;
    MachineCaseCluster &Big = *W.LastCluster;

    if (Small.getLow() == Small.getHigh() && Big.getLow() == Big.getHigh() &&
        Small.MBB == Big.MBB) {
      const APInt &SmallValue = Small.getLow()->getValue();
      const APInt &BigValue = Big.getLow()->getValue();
  ...
  if (TM.getOptLevel() != CodeGenOpt::None) {
    ...
    std::sort(W.FirstCluster, W.LastCluster + 1,
              [](const MachineCaseCluster &a, const MachineCaseCluster &b) {
                return a.Prob > b.Prob;
              });

    // Rearrange the case blocks so that the last one falls through if possible
    // without without changing the order of probabilities.
    for (MachineCaseClusterIt I = W.LastCluster; I > W.FirstCluster;) {
      --I;
      if (I->Prob > W.LastCluster->Prob)
        break;
      if (I->getKind() == CC_Range && I->MBB == NextMBB) {
  ...
  for (MachineCaseClusterIt I = W.FirstCluster; I <= W.LastCluster; ++I)
    UnhandledProbs += I->Prob;

  MachineBasicBlock *CurMBB = W.MBB;
  for (MachineCaseClusterIt I = W.FirstCluster, E = W.LastCluster; I <= E;
       ++I) {
...
unsigned SelectionDAGBuilder::caseClusterRank(const MachineCaseCluster &CC,
                                              MachineCaseClusterIt First,
                                              MachineCaseClusterIt Last) {
  return std::count_if(First, Last + 1, [&](const MachineCaseCluster &X) {
    if (X.Prob != CC.Prob)
      return X.Prob > CC.Prob;

    // Ties are broken by comparing the case value.
    return X.getLow()->getValue().slt(CC.getLow()->getValue());
  });
}
...
void SelectionDAGBuilder::splitWorkItem(SwitchWorkList &WorkList,
                                        ...
                                        Value *Cond,
                                        MachineBasicBlock *SwitchMBB) {
  assert(W.FirstCluster->getLow()->getValue().slt(
  ...
  MachineCaseClusterIt LastLeft = W.FirstCluster;
  MachineCaseClusterIt FirstRight = W.LastCluster;
  ...
  for (;;) {
    ...
    if (std::min(NumLeft, NumRight) < 3 && std::max(NumLeft, NumRight) > 3) {
      ...
      if (NumLeft < NumRight) {
        ...
        MachineCaseCluster &CC = *FirstRight;
        ...
        MachineCaseCluster &CC = *LastLeft;
        unsigned LeftSideRank = caseClusterRank(CC, W.FirstCluster, LastLeft);
        unsigned RightSideRank = caseClusterRank(CC, FirstRight, W.LastCluster);
  ...
  MachineCaseClusterIt PivotCluster = FirstRight;
  assert(PivotCluster > W.FirstCluster);
  assert(PivotCluster <= W.LastCluster);

  MachineCaseClusterIt FirstLeft = W.FirstCluster;
  MachineCaseClusterIt LastRight = W.LastCluster;

  const ConstantInt *Pivot = PivotCluster->getLow();
  ...
  if (FirstLeft == LastLeft && FirstLeft->getKind() == CC_Range &&
      FirstLeft->getLow() == W.GE &&
      (FirstLeft->getHigh()->getValue() + 1LL) == Pivot->getValue()) {
  ...
  if (FirstRight == LastRight && FirstRight->getKind() == CC_Range && W.LT &&
      (FirstRight->getHigh()->getValue() + 1ULL) == W.LT->getValue()) {
...
unsigned SelectionDAGBuilder::prepareBitTests(const SwitchInst *SI,
  ...
  for (int64_t I = First + 1; I < Last; ++I) {
    if (BTCluster.getCaseValueAt(SI, I)->getValue() !=
        BTCluster.getCaseValueAt(SI, I - 1)->getValue() + 1) {
  ...
  for (unsigned I = First; I < Last; ++I) {
    auto CI = BTCluster.getCase(SI, I);
    MachineBasicBlock *MBB = FuncInfo.MBBMap[CI.getCaseSuccessor()];
    ...
    uint64_t Val = (CI.getCaseValue()->getValue() - LowBound).getZExtValue();
    CB->Mask |= (1ULL) << Val;
    CB->Bits++;
    CB->ExtraProb +=
...
void SelectionDAGBuilder::visitSwitch(const SwitchInst &SI) {
  ...
  // Populate clusters for lowering.
  MachineCaseClusterVector MachineClusters;
  ...
  DEBUG({
    dbgs() << "Machine case clusters: ";
    for (const MachineCaseCluster &C : MachineClusters) {
      if (C.getKind() == CC_JumpTable)
        dbgs() << "JT:[";
      ...
      C.getLow()->getValue().print(dbgs(), true);
      if (C.getLow() != C.getHigh()) {
        dbgs() << '-';
        C.getHigh()->getValue().print(dbgs(), true);
      }
      dbgs() << "] ";
    }
    dbgs() << '\n';
  ...
  assert(!MachineClusters.empty());
  SwitchWorkList WorkList;
  MachineCaseClusterIt First = MachineClusters.begin();
  MachineCaseClusterIt Last = MachineClusters.end() - 1;
