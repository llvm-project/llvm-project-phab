...
void SelectionDAGBuilder::visitJumpTable(JumpTableCase &JT) {
...
void SelectionDAGBuilder::visitJumpTableHeader(JumpTableCase &JT,
                                               JumpTableHeader &JTH,
                                               MachineBasicBlock *SwitchBB) {
  SDLoc dl = getCurSDLoc();
  ...
  for (CaseClusterIt I = W.FirstCluster, E = W.LastCluster; I <= E; ++I) {
    ...
    switch (I->Kind) {
      case CC_JumpTable: {
        ...
        JumpTableCase *JT = &JTCases[I->JTCasesIndex].second;
...
void SelectionDAGBuilder::visitSwitch(const SwitchInst &SI) {
  ...
  if (!CaseClusters)
    CaseClusters = new SwitchLoweringCaseCluster(
        DAG.getDataLayout(), DAG.getTargetLoweringInfo(), TM.getOptLevel(),
        FuncInfo, this);
  const BasicBlock *DefaultBB =
      CaseClusters->findCaseClusters(SI, Clusters, BPI);
