...
SDValue DAGCombiner::foldVSelectOfConstants(SDNode *N) {
  ...
  // vselect <N x i1> Cond, C1, C2
  // Check if we can use the condition value to increment/decrement a single
  // constant value. This saves a logic op and a constant from the general case.
  bool AllAddOne = true;
  bool AllSubOne = true;
  ...
  SDValue BitDiff = DAG.getNode(ISD::XOR, DL, VT, N1, N2);
  SDValue Mask = DAG.getNode(ISD::SIGN_EXTEND, DL, VT, Cond);
  SDValue And = DAG.getNode(ISD::AND, DL, VT, Mask, BitDiff);
  return DAG.getNode(ISD::XOR, DL, VT, And, N2);
}
