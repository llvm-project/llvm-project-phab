...
void ModuleBitcodeWriter::setGUIDToValueIdMap() {
  auto insertToGUIDToValueIdMap = [&](GlobalValue::GUID TheGUID, unsigned TheValueId) {
    ...
    GUIDToValueIdMap[TheGUID] = TheValueId;
  };

  for (const Function &F : M) {
    insertToGUIDToValueIdMap(F.getGUID(), VE.getValueID(&F));
    ...
    auto *FS = cast<FunctionSummary>(VI.getSummaryList()[0].get());
    for (auto &RI : FS->refs())
      insertToGUIDToValueIdMap(RI.getGUID(), VE.getValueID(RI.getValue()));
    for (auto &ECI : FS->calls())
      insertToGUIDToValueIdMap(ECI.first.getGUID(), getValueId(ECI.first));
  }

  for (const GlobalVariable &G : M.globals()) {
    insertToGUIDToValueIdMap(G.getGUID(), VE.getValueID(&G));
    ValueInfo VI = Index->getValueInfo(G.getGUID());
    if (!VI || VI.getSummaryList().empty()) {
      // Only declarations should not have a summary (a declaration might
      // however have a summary if the def was in module level asm).
    ...
    auto *VS = cast<GlobalVarSummary>(VI.getSummaryList()[0].get());
    for (auto &RI : VS->refs())
      insertToGUIDToValueIdMap(RI.getGUID(), VE.getValueID(RI.getValue()));
  }
  ...
  for (const GlobalAlias &A : M.aliases()) {
    ...
    if (!Aliasee->hasName())
      continue;
    insertToGUIDToValueIdMap(A.getGUID(), VE.getValueID(&A));
    insertToGUIDToValueIdMap(Aliasee->getGUID(), VE.getValueID(Aliasee));
  }
...
void ModuleBitcodeWriter::writePerModuleGlobalValueSummary() {
  ...
  auto MaybeEmitOriginalName = [&](const GlobalValue &GV) {
    if (!GlobalValue::isLocalLinkage(GV.getLinkage()))
      return;
    NameVals.push_back(GlobalValue::getGUID(GV.getValueName()->getKey()));
    Stream.EmitRecord(bitc::FS_ORIGINAL_NAME, NameVals);
    NameVals.clear();
  };  
