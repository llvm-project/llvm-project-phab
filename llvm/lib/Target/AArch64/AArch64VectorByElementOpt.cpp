...
struct AArch64VectorByElementOpt : public MachineFunctionPass {
  ...
  shouldReplaceInstruction(MachineFunction *MF, const MCInstrDesc *InstDesc,
                           const MCInstrDesc *InstDescRepl[],
                           unsigned NumRepl,
  ...
  bool earlyExitVectElement(MachineFunction *MF, Subpass sss);
...
bool AArch64VectorByElementOpt::shouldReplaceInstruction(
  ...
  const MCSchedClassDesc *SCDescRepl[MaxNumRepl];
	for (unsigned i=0; i<NumRepl; i++)
    SCDescRepl[i] =
      SchedModel.getMCSchedModel()->getSchedClassDesc(SCIdxRepl[i]);
  ...
  unsigned ReplCost = 0;
  for (unsigned i=0; i<NumRepl; i++)
    ReplCost += SchedModel.computeInstrLatency(InstDescRepl[i]->getOpcode());
...
/// subpass early. This makes sure that Targets with no need for this
/// optimization do not spend any compile time on this subpass other than
/// the simple check performed below.
/// This check is done by comparing the latency of the original instruction
/// to the latency of the replacement instructions. We only check for a
/// representative instruction in the class of instructions (ex. FMLA
/// instruction) and not all related instructions (ex. FMLS instruction).
...
bool AArch64VectorByElementOpt::earlyExitVectElement(MachineFunction *MF,
  ...
  {
    OriginalMCID = &TII->get(AArch64::ST2Twov4s);
    const MCInstrDesc* ReplInstrMCID[3] = {&TII->get(AArch64::ZIP1v4i32),
                                           &TII->get(AArch64::ZIP2v4i32),
                                           &TII->get(AArch64::STPQi)};
    if (!shouldReplaceInstruction(MF, OriginalMCID, ReplInstrMCID, 3,
                                  VecInstElemTable))
      return true;
  }
...
bool AArch64VectorByElementOpt::optimizeVectElement(
  ...
  const MCInstrDesc* ReplInstrMCID[2];
  ReplInstrMCID[0] = DupMCID;
  ReplInstrMCID[1] = MulMCID;
  if (!shouldReplaceInstruction(MI.getParent()->getParent(),
                                &TII->get(MI.getOpcode()), ReplInstrMCID, 2,
                                *VecInstElemTable))
    return false;
...
/// Return true if the SIMD instruction is modified.
bool AArch64VectorByElementOpt::optimizeLdStInterleave(
    MachineInstr &MI, std::map<unsigned, bool> *VecInstElemTable) const {

  unsigned SeqReg, AddrReg;
  unsigned StReg[4], StRegKill[4];
  unsigned ZipDest[MaxNumRepl];
  const MCInstrDesc* ReplInstrMCID[MaxNumRepl];
  unsigned NumReplInst;
  ...
  case AArch64::ST2Twov2d:
    ...
    break;
  case AArch64::ST2Twov4s:
		prepareStmtParam(AArch64::ZIP1v4i32, AArch64::ZIP2v4i32, RC128, ReplInstrMCID,
      ZipDest, TwoVectors);
    NumReplInst = 3;
    break;
  case AArch64::ST2Twov2s:
		prepareStmtParam(AArch64::ZIP1v2i32, AArch64::ZIP2v2i32, RC64, ReplInstrMCID,
      ZipDest, TwoVectors);
    NumReplInst = 3;
    break;
  case AArch64::ST2Twov8h:
		prepareStmtParam(AArch64::ZIP1v8i16, AArch64::ZIP2v8i16, RC128, ReplInstrMCID,
      ZipDest, TwoVectors);
    NumReplInst = 3;
    break;
  case AArch64::ST2Twov4h:
		prepareStmtParam(AArch64::ZIP1v4i16, AArch64::ZIP2v4i16, RC64, ReplInstrMCID,
      ZipDest, TwoVectors);
    break;
  case AArch64::ST2Twov16b:
		prepareStmtParam(AArch64::ZIP1v16i8, AArch64::ZIP2v16i8, RC128, ReplInstrMCID,
      ZipDest, TwoVectors);
    NumReplInst = 3;
    break;
  case AArch64::ST2Twov8b:
		prepareStmtParam(AArch64::ZIP1v8i8, AArch64::ZIP2v8i8, RC64, ReplInstrMCID,
      ZipDest, TwoVectors);
    NumReplInst = 3;
    break;
  ...
  if (!shouldReplaceInstruction(MI.getParent()->getParent(),
                                &TII->get(MI.getOpcode()), ReplInstrMCID,
                                NumReplInst, *VecInstElemTable))
    return false;
...
///    %dest = REG_SEQUENCE %st2_src1, dsub0, %st2_src2, dsub1;
/// Return true when the instruction is processed successfully.
bool AArch64VectorByElementOpt::processSeqRegInst(MachineInstr *DefiningMI,
...
/// Prepare the parameters needed to build the replacement statements
void AArch64VectorByElementOpt::prepareStmtParam(unsigned Opcode1,
     unsigned Opcode2, const TargetRegisterClass *RC,
     const MCInstrDesc* ReplInstrMCID[], unsigned ZipDest[], AccessType AccT) const
{
  assert(AccT == TwoVectors || AccT == FourVectors);
  if ( AccT == TwoVectors)
  {
    ReplInstrMCID[0] = &TII->get(Opcode1);
    ZipDest[0] = MRI->createVirtualRegister(RC);
    ReplInstrMCID[1] = &TII->get(Opcode2);
    ZipDest[1] = MRI->createVirtualRegister(RC);
    if (RC == &AArch64::FPR128RegClass)
      ReplInstrMCID[2] = &TII->get(AArch64::STPQi);
    else
      ReplInstrMCID[2] = &TII->get(AArch64::STPDi);
  }
	else // AccT == FourVectors
  {
    for (int i=0; i<4; i++)
    {
       ReplInstrMCID[2*i] = &TII->get(Opcode1);
       ZipDest[2*i] = MRI->createVirtualRegister(RC);
       ReplInstrMCID[2*i+1] = &TII->get(Opcode2);
       ZipDest[2*i+1] = MRI->createVirtualRegister(RC);
    }
    if (RC == &AArch64::FPR128RegClass)
    {
      ReplInstrMCID[8] = &TII->get(AArch64::STPQi);
      ReplInstrMCID[9] = &TII->get(AArch64::STPQi);
    }
    else
    {
      ReplInstrMCID[8] = &TII->get(AArch64::STPDi);
      ReplInstrMCID[9] = &TII->get(AArch64::STPDi);
    }
  }
}
...
bool AArch64VectorByElementOpt::runOnMachineFunction(MachineFunction &MF) {
  ...
  // that do not need it.
  if (!earlyExitVectElement(&MF, FMLA))
  {
    std::map<unsigned, bool> VecInstElemTable;
  ...
  // Another simple check to exit the interleaved store sub-pass early for targets
	// that do not need it.
  if (!earlyExitVectElement(&MF, Interleave))
  {
    std::map<unsigned, bool> VecInstElemTable;
    SmallVector<MachineInstr *, 8> RemoveMIs;
    for (MachineBasicBlock &MBB : MF) {
      for (MachineBasicBlock::iterator MII = MBB.begin(), MIE = MBB.end();
           MII != MIE;) {
        MachineInstr &MI = *MII;
        if (optimizeLdStInterleave(MI, &VecInstElemTable)) {
          // Add MI to the list of instructions to be removed given that it has
          // been replaced.
          RemoveMIs.push_back(&MI);
          Changed = true;
