...
struct AArch64SIMDInstrOpt : public MachineFunctionPass {
  ...
  bool shouldReplaceInst(MachineFunction *MF, const MCInstrDesc *InstDesc,
                         ...
                         std::map<unsigned, bool> &SIMDInstrTable) const;
  ...
  bool shouldExitEarlyShort(MachineFunction *MF, Subpass SP);
  ...
  bool optimizeLdStInterleave(
      MachineInstr &MI, std::map<unsigned, bool> *SIMDInstrTable) const;
...
/// to replace the instruction InstDesc by the instructions stored in the
/// array InstDescRepl.
/// Return true if replacement is is expected to be faster.
...
shouldReplaceInst(MachineFunction *MF, const MCInstrDesc *InstDesc,
  ...
  if (!SIMDInstrTable.empty() &&
    ...
    return SIMDInstrTable[InstDesc->getOpcode()];
  ...
  {
    SIMDInstrTable[InstDesc->getOpcode()] = false;
    return false;
  }
  ...
  {
    SIMDInstrTable[InstDesc->getOpcode()] = true;
    return true;
  }
  else
  {
...
/// Return true if early exit of the subpass is recommended.
bool AArch64SIMDInstrOpt::shouldExitEarlyShort(MachineFunction *MF, Subpass SP) {
  ...
  case VectorElem:
    ...
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    break;
  case Interleave:
    OriginalMCID = &TII->get(AArch64::ST2Twov4s);
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v4i32));
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v4i32));
    ReplInstrMCID.push_back(&TII->get(AArch64::STPQi));
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;

    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST4Fourv4s);
...
bool AArch64SIMDInstrOpt::shouldExitEarly(MachineFunction *MF, Subpass SP) {
  ...
  case VectorElem:
    ...
    break;
  case Interleave:
    OriginalMCID = &TII->get(AArch64::ST2Twov2d);
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v2i64));
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v2i64));
    ReplInstrMCID.push_back(&TII->get(AArch64::STPQi));
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    ...
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST2Twov2s);
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v2i32));
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v2i32));
    ReplInstrMCID.push_back(&TII->get(AArch64::STPDi));
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST2Twov8h);
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v8i16));
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v8i16));
    ReplInstrMCID.push_back(&TII->get(AArch64::STPQi));
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST2Twov4h);
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v4i16));
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v4i16));
    ReplInstrMCID.push_back(&TII->get(AArch64::STPDi));
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST2Twov16b);
    ...
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST2Twov8b);
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v8i8));
    ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v8i8));
    ReplInstrMCID.push_back(&TII->get(AArch64::STPDi));
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;

    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST4Fourv2d);
for (unsigned i=0; i<4; i++) {
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v2i64));
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v2i64));
    }
    ...
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST4Fourv2s);
    for (unsigned i=0; i<4; i++) {
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v2i32));
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v2i32));
    }
    ...
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST4Fourv8h);
    for (unsigned i=0; i<4; i++) {
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v8i16));
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v8i16));
    }
    ...
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST4Fourv4h);
    for (unsigned i=0; i<4; i++) {
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v4i16));
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v4i16));
    }
    ...
    if (shouldReplaceInst(MF, OriginalMCID, ReplInstrMCID, SIMDInstrTable))
      return false;
    ReplInstrMCID.clear();
    OriginalMCID = &TII->get(AArch64::ST4Fourv8b);
    for (unsigned i=0; i<4; i++) {
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP1v8i8));
      ReplInstrMCID.push_back(&TII->get(AArch64::ZIP2v8i8));
    }
...
///    fmla v0.4s, v1.4s, v3.4s
/// Return true if the SIMD instruction is modified.
bool AArch64SIMDInstrOpt::optimizeVectElement(
    MachineInstr &MI, std::map<unsigned, bool> *SIMDInstrTable) const {
...
/// Return true if the SIMD instruction is modified.
bool AArch64SIMDInstrOpt::optimizeLdStInterleave(
    MachineInstr &MI, std::map<unsigned, bool> *SIMDInstrTable) const {
      ...
      for (MachineBasicBlock &MBB : MF) {
        for (MachineBasicBlock::iterator MII = MBB.begin(), MIE = MBB.end();
          ...
          if (OptimizationKind == VectorElem)
            InstRewrite = optimizeVectElement(MI, &SIMDInstrTable) ;
          else
            InstRewrite = optimizeLdStInterleave(MI, &SIMDInstrTable);
          if (InstRewrite) {
