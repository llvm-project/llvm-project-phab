...
SDValue ARMTargetLowering::LowerUnsignedALUO(SDValue Op,
  ...
  case ISD::UADDO:
    ...
    Overflow =
        DAG.getNode(ARMISD::ADDE, dl, VTs, DAG.getConstant(0, dl, MVT::i32),
                    DAG.getConstant(0, dl, MVT::i32), Value.getValue(1));
    break;
  case ISD::USUBO: {
    ...
    // value. We do this doing (ARMISD::SUBC 1, C)
...
static SDValue LowerADDSUBCARRY(SDValue Op, SelectionDAG &DAG) {
  ...
  if (Op.getOpcode() == ISD::ADDCARRY) {
    ...
    // ARMISD::ADDC Carry, ~0
    Carry = DAG.getNode(ARMISD::ADDC, DL, DAG.getVTList(CarryVT, MVT::i32),
    ...
    Carry = DAG.getNode(ARMISD::ADDE, DL, VTs, DAG.getConstant(0, DL, MVT::i32),
      ...
      Carry = DAG.getNode(ARMISD::SUBC, DL, VTs, DAG.getConstant(1, DL, MVT::i32),
              Carry);
      // This converts the boolean value carry into the carry flag by doing
      // ARMISD::ADDC Carry, ~0
      Carry = DAG.getNode(ARMISD::ADDC, DL, DAG.getVTList(CarryVT, MVT::i32),
...
static SDValue PerformAddcSubcCombine(SDNode *N,
  ...
  if (N->getOpcode() == ARMISD::SUBC) {
    ...
    // (SUBC 1, (SUBC 1, X)) -> X
    if (isOneConstant(LHS) && RHS->getOpcode() == ARMISD::SUBC &&
        isOneConstant(RHS->getOperand(0))) {
        return DCI.CombineTo(N, RHS->getOperand(1), SDValue(N, 1));
    }
  }
...
void ARMTargetLowering::computeKnownBitsForTargetNode(const SDValue Op,
  ...
  case ARMISD::SUBE:
    ...
    if (Op.getResNo() == 0) {
      ...
      // SUBC (1, ADDE(0, 0, C)) gives us a single bit.
      // FIXME: This can be probably generalized in that if the RHS just
      // gives 1 bit, 1 - RHS will also give us 1 bit.
      if (Op->getOpcode() == ARMISD::SUBC && isOneConstant(LHS) &&
          RHS->getOpcode() == ARMISD::ADDE &&
          isNullConstant(RHS->getOperand(0)) &&
          isNullConstant(RHS->getOperand(1))) {
        Known.Zero |= APInt::getHighBitsSet(BitWidth, BitWidth - 1);
        return;
      }
    }
