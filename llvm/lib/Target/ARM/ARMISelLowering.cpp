...
SDValue ARMTargetLowering::LowerConstantPool(SDValue Op,
  ...
  // When generating execute-only code, Constant Pools
  // must be promoted to the global data section.
  if (Subtarget->genExecuteOnly()) {
    ARMFunctionInfo *AFI = DAG.getMachineFunction().getInfo<ARMFunctionInfo>();
    unsigned LID = AFI->createPICLabelUId();
    ...
    auto GV = new GlobalVariable(*M, T, /*isConstant=*/true,
                                 GlobalVariable::InternalLinkage, C,
                                 std::string(".LCPI.XO.")+std::to_string(LID));
