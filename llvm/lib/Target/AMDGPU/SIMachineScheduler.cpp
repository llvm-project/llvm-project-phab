...
SISchedulerRPTracker::SISchedulerRPTracker(
  ...
  ShouldTrackLaneMasks(ShouldTrackLaneMasks), ItemSuccs(ItemSuccs),
  ItemPreds(ItemPreds), CurrentVGPRUsage(0), CurrentSGPRUsage(0)
{
...
SmallVector<RegisterMaskPair, 8>
SISchedulerRPTracker::getPairsForRegs(const SmallVector<RegisterMaskPair, 8> &Regs)
{
  SmallVector<RegisterMaskPair, 8> Result;
...
void
SISchedulerRPTracker::addLiveRegs(SmallVector<RegisterMaskPair, 8> &Regs) {
...
void
SISchedulerRPTracker::decreaseLiveRegs(SmallVector<RegisterMaskPair, 8> &Regs) {
  for (const RegisterMaskPair &RegPair : Regs) {
    // For now only track virtual registers.
    unsigned Reg = RegPair.RegUnit;
    if (!TargetRegisterInfo::isVirtualRegister(Reg))
...
void SIScheduleBlockCreator::removeUseFromDef(SmallVector<RegisterMaskPair, 8> &Uses,
...
void SIScheduleBlockCreator::addDefFromUse(SmallVector<RegisterMaskPair, 8> &Defs,
                                           unsigned Reg, const SUnit *SUDef,
                                           const SUnit *SUUse)
{
  LaneBitmask LanesMaskMax, DefMask, UseMask;
  const std::vector<int> Node2Index = DAG->getTopo()->getNode2Index();
  int DefIndex = Node2Index[SUDef->NodeNum];
  ...
  for (const SDep& PredDep : SUUse->Preds) {
    ...
    // topological sort.
    if (Node2Index[Pred->NodeNum] <= DefIndex)
      continue;
