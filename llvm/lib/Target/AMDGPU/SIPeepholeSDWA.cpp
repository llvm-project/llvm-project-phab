...
#define FIND_SINGLE_REG_DEF_USE(NAME, FNAME)                                  \
static MachineOperand *findSingleReg##NAME(const MachineOperand *Reg,         \
  ...
  for (MachineOperand &DefUse: MRI->FNAME(Reg->getReg())) {                   \
...
FIND_SINGLE_REG_DEF_USE(Use, use_operands)
FIND_SINGLE_REG_DEF_USE(Def, def_operands)

#undef FIND_SINGLE_REG_DEF_USE
...
bool SDWASrcOperand::convertToSDWA(MachineInstr &MI, const SIInstrInfo *TII) {
  ...
  if (!TRI->isSameReg(*Src, *getReplacedOperand())) {
    ...
    if ((MI.getOpcode() == AMDGPU::V_MAC_F16_sdwa ||
        ...
        !TRI->isSameReg(*Src, *getReplacedOperand())) {
      // In case of v_mac_f16/32_sdwa this pass can try to apply src operand to
      // src2. This is not allowed.
      return false;
    }

    assert(TRI->isSameReg(*Src, *getReplacedOperand()) && SrcSel && SrcMods);
  }
...
MachineInstr *SDWADstOperand::potentialToConvert(const SIInstrInfo *TII) {
  ...
  for (MachineOperand &UseMO : MRI->use_operands(PotentialMO->getReg())) {
    if (getTRI()->isSubregOf(UseMO, *PotentialMO) &&
...
bool SDWADstOperand::convertToSDWA(MachineInstr &MI, const SIInstrInfo *TII) {
  ...
  assert(Operand &&
         Operand->isReg() &&
         getTRI()->isSameReg(*Operand, *getReplacedOperand()));
