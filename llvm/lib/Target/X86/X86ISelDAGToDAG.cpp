...
static bool hasNoCarryFlagUses(SDNode *N) {
  ...
  for (SDNode::use_iterator UI = N->use_begin(), UE = N->use_end(); UI != UE;
    ...
    // Only examine CopyToReg uses.
    switch (UI->getOpcode()) {
    // Trivially safe opcodes.
    // FIXME: There are many more things that should be here.
    case ISD::STORE:
      continue;

    // When copying to a register, we need to more carefully examine things.
    case ISD::CopyToReg:
      break;

    // Everything else is rejected conservatively.
    default:
      return false;
    }

    // Special logic for handling CopyToReg nodes by examining *which* flags
    // are actually used.
