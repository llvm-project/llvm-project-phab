...
class X86InterleavedAccessGroup {
  ...
  void deinterleave8bit_32x4(ArrayRef<Instruction *> InputVectors,
                          SmallVectorImpl<Value *> &TransposedMatrix);
public:
...
static void createConcatShuffleMask(int NumElement,
  ...
  int CurrentIndex = Low ? 0 : NumElement / 2;
  int EndIndex = CurrentIndex + NumElement / 2;
  for (int i = 0; i < NumElement; ++i) {
    if (CurrentIndex == EndIndex)
      CurrentIndex += NumElement / 2;
    Mask.push_back(CurrentIndex);
    CurrentIndex++;
  }
}

// Create shuffle mask for pattern Unpack with double size type in VT.
// For example: shuffle of V32I8
// {0,1,32,33,2,3,34,35,4,5,36,37,6,7,38,39,16,17,48,49,18,19,50,51,20,21,52,53,22,23,54,55}
// After simplifies the shuffle will convert to <16 x i16> shuffle with mask:
// {0,16,1,17,2,18,3,19,8,24,9,25,10,26,11,27} Unpacklwd
...
void X86InterleavedAccessGroup::deinterleave8bit_32x4(
  ...
  // shuffle pattern.
  createUnpackShuffleMaskDType<uint32_t>(MVT::v32i8, MaskLowTemp1, true, false);
  createUnpackShuffleMaskDType<uint32_t>(MVT::v32i8, MaskHighTemp1, false,
                                         false);
...
bool X86InterleavedAccessGroup::lowerIntoOptimizedSequence() {
  ...
  case 4:
    ...
    break;
  case 32:
    deinterleave8bit_32x4(DecomposedVectors, TransposedVectors);
    break;
  default:
    return false;
  }
