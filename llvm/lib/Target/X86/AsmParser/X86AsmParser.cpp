...
private:
  ...
  public:
    ...
    bool onOffset(const MCExpr *Val, StringRef ID, StringRef &ErrMsg) {
      PrevState = State;
      switch (State) {
      default:
  ...
  bool ParseIntelOffsetOperator(const MCExpr *&Val, StringRef &ID, SMLoc &End);
...
std::unique_ptr<X86Operand> X86AsmParser::CreateMemForInlineAsm(
  ...
  if (Info.isKind(InlineAsmIdentifierInfo::LabelKind)) {
  ...
  if (Info.isKind(InlineAsmIdentifierInfo::VarKind)) {
...
bool X86AsmParser::ParseIntelNamedOperator(StringRef Name,
  ...
  if (Name.equals_lower("not")) {
    ...
    StringRef ID;
    if ((ParseError = ParseIntelOffsetOperator(Val, ID, End)))
      return true;
    StringRef ErrMsg;
    if ((ParseError = SM.onOffset(Val, ID, ErrMsg)))
...
X86AsmParser::ParseIntelInlineAsmIdentifier(const MCExpr *&Val,
  ...
  assert((End.getPointer() == EndPtr ||
          Info.isKind(InlineAsmIdentifierInfo::Invalid)) &&
          "frontend claimed part of a token?");

  // If the identifier lookup was unsuccessful, assume that we are dealing with
  // a label.
  if (Info.isKind(InlineAsmIdentifierInfo::Invalid)) {
  ...
  } else if (Info.isKind(InlineAsmIdentifierInfo::EnumValKind))
    return false;
...
bool X86AsmParser::ParseIntelOffsetOperator(const MCExpr *&Val, StringRef &ID,
  ...
  } else if (Info.isKind(InlineAsmIdentifierInfo::VarKind) &&
             !Info.Var.IsGlobalLV) {
