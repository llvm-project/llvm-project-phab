...
static bool isDividendAllOnes(MachineFunction &MF, unsigned DividendReg) {
  // The dividend could be AllOnes value and in this case we should not create
...
static void genReciprocalDiv(MachineInstr &Root,
  ...
  bool DivIsRcp = DividendIsAllOnes && !Iterations;
  ...
  unsigned LoadVReg = 0;
  if (!DividendIsAllOnes && Iterations) {
    // 2: load
  ...
  for (int i = 0; i < Iterations; i++) {
    ...
    MachineInstrBuilder SubMI =
        BuildMI(MF, Root.getDebugLoc(), TII->get(Instrs[3]), SubVReg)
            .addReg(DividendIsAllOnes ? DividendReg : LoadVReg)
            .addReg(MulVReg);
    ...
    if (DividendIsAllOnes && (i + 1 == Iterations))
