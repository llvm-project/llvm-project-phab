...
bool PPCInstrInfo::transformToImmForm(MachineInstr &MI, const ImmInstrInfo &III,
  ...
  // special and we're changing register zero, we can't transform this.
  if (PostRA && III.ZeroIsSpecialOrig != !III.ZeroIsSpecialNew) {
    unsigned Reg1 = MI.getOperand(ConstantOpNo).getReg();
    unsigned Reg2 = MI.getOperand(III.ConstantOpNo).getReg();
    if (Reg1 == PPC::X0 || Reg1 == PPC::R0 ||
        Reg2 == PPC::X0 || Reg2 == PPC::R0)
      return false;
  }
