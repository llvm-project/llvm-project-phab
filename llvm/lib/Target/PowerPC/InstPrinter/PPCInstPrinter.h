...
private:
  ...
  bool showRegistersWithoutPrefix(const char *RegName);
  const char *stripRegisterPrefix(const char *RegName);
  const char *getFullRegisterName(const char *RegName, unsigned RegNum,
                                  unsigned RegEncoding);
public:
  PPCInstPrinter(const MCAsmInfo &MAI, const MCInstrInfo &MII,
                 const MCRegisterInfo &MRI, Triple T)
    : MCInstPrinter(MAI, MII, MRI), TripleInfo(T) {}
