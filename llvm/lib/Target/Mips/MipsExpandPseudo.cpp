...
//
// Currently only used for expanding atomic pseudo after register allocation.
// We do this to avoid the fast register allocator introducing spills between
// ll and sc. These stores cause some MIPS implementations to abort the
// atomic RMW sequence.
//
//===----------------------------------------------------------------------===//
...
bool MipsExpandPseudo::expandAtomicCmpSwapSubword(
  ...
  sinkMBB->addLiveIn(Dest);
  sinkMBB->addLiveIn(ShiftAmnt);
  sinkMBB->addLiveIn(Scratch2);
...
bool MipsExpandPseudo::expandAtomicBinOpSubword(
  ...
  loopMBB->addLiveIn(Ptr);
  loopMBB->addLiveIn(Incr);
  loopMBB->addLiveIn(Mask);
  loopMBB->addLiveIn(Mask2);
  ...
  addPostLoopLiveIns(sinkMBB, LiveRegs);
