...
MipsSETargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  case Mips::MSA_FP_TO_UINT:
    return emitFP_TO_UINT_MSA(MI, BB);
...
//
MachineBasicBlock *
MipsSETargetLowering::emitFP_TO_UINT_MSA(MachineInstr &MI,
                                         MachineBasicBlock *BB) const {
...
MachineBasicBlock *MipsSETargetLowering::emitFP_TO_UINT(MachineInstr &MI,
  ...
  if (IsSrc64) {
    if (!IsFP64)
      ...
      unsigned GPTemp = RegInfo.createVirtualRegister(&Mips::GPR64RegClass);
      BuildMI(BB, DL, TII->get(Mips::DSLL), GPTemp)
          .addReg(GPSubValue)
          .addImm(32);
  ...
  if (IsDest64) {
    ...
    BuildMI(correctionMBB, DL, TII->get(Mips::DSLL32), GPTemp)
        .addReg(MSBBit)
        .addImm(0);
