...
static DecodeStatus DecodeGPR32ReducedRegister(MCInst &Inst, unsigned RegNo,
                                               uint64_t Address,
                                               const void *Decoder) {
...
static void DecodeSymbolicOperandOff(MCInst &Inst, uint64_t Address,
  ...
  if (!DecodeSymbolicOperand(Inst, Address, nextAddress, Decoder))
    Inst.addOperand(MCOperand::createImm(nextAddress));
}
...
// Sequential ranges from lowest to highest.
...
static uint64_t ReadFields(uint64_t Insn) {
  return ReadField<_first, _last>(Insn) |
         (ReadFields<_ranges...>(Insn) << (_last - _first + 1));
}

template <class STOP = void>
static uint64_t ReadFields(uint64_t Insn) {
  return 0;
}

template <uint8_t _first, uint8_t _last>
static uint64_t ReadField(uint64_t Insn) {
  static_assert(_first <= _last, "invalid range!");
  return fieldFromInstruction(Insn, _first, _last - _first + 1);
}
...
static DecodeStatus DecodeMoveHRegInstruction(MCInst &Inst, uint64_t Insn,
  ...
  uint64_t h = ReadFields<5, 7, 0, 1>(Insn);
  uint64_t g = ReadFields<8, 10, 3, 4>(Insn);
  ...
  auto DecodeRegisterOrImm = [&Inst, Address, Decoder](decltype(h) field,
    ...
    return DecodeGPR32RegisterClass(Inst, field, Address, Decoder);
  };
