...
class RuleMatcher {
  ...
  /// emitCxxCaptureStmts().
  std::map<const InstructionMatcher *, unsigned> InsnVariableNames;
...
public:
  ...
  void emitCxxPredicateExpr(raw_ostream &OS, RuleMatcher &Rule,
                            unsigned InsnVarID, unsigned OpIdx) const override {
    OS << "    GIM_CheckInt, /*MI*/" << InsnVarID << ", /*Op*/" << OpIdx << ", " << Value
       << ",\n";
  }
};
...
public:
  ...
  std::string getOperandExpr(unsigned InsnVarID) const {
    return "MIs[" + llvm::to_string(InsnVarID) + "]->getOperand(" +
...
public:
  ...
  void emitCxxCaptureStmts(raw_ostream &OS, RuleMatcher &Rule,
    ...
    OS << "    GIM_PushInsnOpDef, /*MI*/" << InsnID << ", /*OpIdx*/" << OpIdx << ",\n";
...
unsigned
RuleMatcher::implicitlyDefineInsnVar(raw_ostream &OS,
                                     const InstructionMatcher &Matcher) {
  unsigned NewInsnVarID = NextInsnVarID++;
  InsnVariableNames[&Matcher] = NewInsnVarID;
  return NewInsnVarID;
}
...
unsigned RuleMatcher::defineInsnVar(raw_ostream &OS,
  ...
  unsigned NewInsnVarID = NextInsnVarID++;
  OS << "    GIM_RecordInsn, " << NewInsnVarID << ", // MIs[" << NewInsnVarID << "]\n";
  InsnVariableNames[&Matcher] = NewInsnVarID;
  return NewInsnVarID;
}
...
void RuleMatcher::emitCxxCaptureStmts(raw_ostream &OS) {
  ...
  unsigned InsnVarID = implicitlyDefineInsnVar(OS, *Matchers.front());
...
void RuleMatcher::emit(raw_ostream &OS) {
  ...
  if (!RequiredFeatures.empty()) {
    OS << "    GIM_CheckFeatures, GIFBS";
    for (const auto &Predicate : RequiredFeatures)
       OS << "_" << Predicate->getName();
    OS << ",\n";
  }
  ...
  OS << "    GIM_Accept,\n"
     ...
     << "  MIs.clear();\n"
     << "  MIs.push_back(&I);\n"
     << "  if (executeMatchTable(*this, MIs, MIStack, Renderers, TypeObjects, "
        "FeatureBitsets, ComplexPredicates, MatchTable"
  ...
  if (InsnVariableNames.size() >= 2) {
    for (const auto &Pair : InsnVariableNames) {
      ...
      OS << "    if (!isObviouslySafeToFold(*MIs[" << Pair.second << "]))\n"
         << "      return false;\n";
...
void GlobalISelEmitter::run(raw_ostream &OS) {
  ...
  OS << "#ifdef GET_GLOBALISEL_TEMPORARIES_DECL\n"
     << "  mutable std::vector<ComplexRendererFn> Renderers;\n"
     << "  typedef "
        "ComplexRendererFn("
     << Target.getName()
  ...
  OS << "#ifdef GET_GLOBALISEL_TEMPORARIES_INIT\n"
     << ", Renderers(" << MaxTemporaries << ", nullptr),\n"
        "ComplexPredicates({nullptr, // GICP_Invalid\n";
