...
Expected<RuleMatcher> GlobalISelEmitter::runOnPattern(const PatternToMatch &P) {
  ...
  if (DstI.TheDef->getName() == "COPY_TO_REGCLASS") {
    ...
    if (DefInit *SubRegInit =
      ...
      CodeGenRegisterClass *DstRC = nullptr;
      SrcRC = SrcRC->getMatchingSubClassWithSubRegs(CGRegs, SubIdx, &DstRC);
      assert(DstRC && "Couldn't find a matching subclass");
      M.addAction<ConstrainOperandToRegClassAction>("NewI", 0, *DstRC);
      M.addAction<ConstrainOperandToRegClassAction>("NewI", 1, *SrcRC);
