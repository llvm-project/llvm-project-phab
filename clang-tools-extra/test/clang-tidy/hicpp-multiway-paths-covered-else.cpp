...
void problematic_if(int i, enum OS os) {
  if (i > 0) {
    ...
    // CHECK-MESSAGES: [[@LINE-1]]:10: warning: potential uncovered codepath; add an ending else branch
    return;
  }
  ...
  if (os == Mac) {
    ...
    if (true) {
      ...
      // CHECK-MESSAGES: [[@LINE-1]]:12: warning: potential uncovered codepath; add an ending else branch
      return;
    }
    return;
  } else {
