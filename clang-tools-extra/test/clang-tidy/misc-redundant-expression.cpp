...
// Overloaded operators comparing two instances of a struct.
struct MyStruct {
  int x;  
  bool operator==(const MyStruct& rhs) const {return this->x == rhs.x; } // not modifing
  bool operator>=(MyStruct rhs) const { rhs.x++; return this->x >= rhs.x; } // not modifing
...
bool operator<(const MyStruct& lhs, MyStruct rhs) { rhs.x++; return lhs.x < rhs.x; } // not modifing
bool operator>(const MyStruct& lhs, MyStruct& rhs) { return lhs.x > rhs.x; }
...
// Overloaded operators comparing an instance of a struct and an integer constant.
...
bool operator<=(const S &s, int i) {return s.x <= i;}
bool operator>=(S s, const int &i) {s.x--; return s.x >= i;}

// Overloaded comparison operators that are able to modify their params,
// therefore may have side effects.
struct S2 {
  S2(){x = 1;}
  int x;
  bool operator==(const int &i)  { this->x++; return x == i; }
  bool operator!=(int i)  { return x != i; }
  bool operator>(const int &i) { return x > i; }
  bool operator<(int i) { this->x--; return x < i; }
};

bool operator>=(S2 &s, const int &i) { return s.x >= i; }
bool operator<=(S2 &s, int i) { s.x++; return s.x <= i; }
