...
struct CompletionCandidate {
  ...
  // FIXME: use a more natural score, e.g. double 0-1, higher is better.
  int Score;

  // Comparison: better candidates are bigger.
  bool operator<(const CompletionCandidate &C) const {
    if (Score != C.Score)
      return Score > C.Score;
    return *Result > *C.Result; // Reversed so descending list is alpha-sorted.
  }
  ...
  std::string sortText() const {
    ...
    StringRef Name = Result->orderedName(NameStorage);
