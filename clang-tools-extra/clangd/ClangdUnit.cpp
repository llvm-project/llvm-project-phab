...
CppFile::~CppFile() {
  ...
  // Increase RebuildCounter to cancel any ongoing rebuilds and wait for the
  // processing to finish.
  ++RebuildCounter;
  RebuildCond.wait(Lock, [this]() { return !RebuildInProgress; });

  // Make sure anyone waiting for these will get a response.
...
CppFile::deferRebuild(StringRef NewContents,
  ...
  // thread.
  auto FinishRebuild = [this, OldPreamble, VFS, RequestRebuildCounter,
                        PCHs](std::string NewContents)
      -> llvm::Optional<std::vector<DiagWithFixIts>> {
    ...
    RebuildGuard Rebuild(*this, RequestRebuildCounter);
    ...
    std::unique_ptr<llvm::MemoryBuffer> ContentsBuffer =
        llvm::MemoryBuffer::getMemBufferCopy(NewContents, FileName);
    ...
    // Publish the new Preamble.
    {
      ...
      this->LatestAvailablePreamble = NewPreamble;
      if (RequestRebuildCounter != this->RebuildCounter)
        return llvm::None; // Our rebuild request was cancelled, do nothing.
      PreamblePromise.set_value(NewPreamble);
    } // unlock Mutex
    ...
    // Publish the new AST.
    {
      ...
      if (RequestRebuildCounter != this->RebuildCounter)
        return Diagnostics; // Our rebuild request was cancelled, don't set
                            // ASTPromise.
      ASTPromise.set_value(ParsedASTWrapper(std::move(NewAST)));
    } // unlock Mutex
