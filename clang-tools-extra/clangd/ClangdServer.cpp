...
void ClangdScheduler::addToFront(Func &&F, Args &&... As) {
  if (RunSynchronously) {
...
template <class Func, class... Args>
void ClangdScheduler::addToEnd(Func &&F, Args &&... As) {
  if (RunSynchronously) {
    std::forward<Func>(F)(std::forward<Args>(As)...);
    return;
  }
...
ClangdServer::codeComplete(PathRef File, Position Pos,
  ...
  CppFile *Resources = Units.getFile(File);
