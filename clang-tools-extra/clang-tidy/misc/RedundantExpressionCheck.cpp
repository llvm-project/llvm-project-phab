...
void RedundantExpressionCheck::check(const MatchFinder::MatchResult &Result) {
  ...
  if (const auto *BinaryAndExpr = Result.Nodes.getNodeAs<BinaryOperator>(
    ...
    if (!ShiftingConst ||
        !ShiftingConst->isIntegerConstantExpr(ShiftingValue, *Result.Context))
      return;

    const auto *AndConst = Result.Nodes.getNodeAs<Expr>("and-const");
    APSInt AndValue;
    if (!AndConst ||
        !AndConst->isIntegerConstantExpr(AndValue, *Result.Context))
      return;
    ...
    SourceLocation ShiftOperatorLoc = ShiftOperator->getOperatorLoc();
