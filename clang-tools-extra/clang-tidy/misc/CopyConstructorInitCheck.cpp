...
void CopyConstructorInitCheck::registerMatchers(MatchFinder *Finder) {
  Finder->addMatcher(
      cxxConstructorDecl(isCopyConstructor(),
                         hasAnyConstructorInitializer(ctorInit.bind("init")))
          .bind("ctor"),
      this);
}
...
void CopyConstructorInitCheck::check(const MatchFinder::MatchResult &Result) {
  ...
  const auto Matches = match(
      cxxConstructorDecl(isCopyConstructor(),
                         forEachConstructorInitializer(ctorInit.bind("init"))),
      *Ctor, *Result.Context);
  ...
  for (const auto &Match : Matches) {
    ...
    if (Init->getSourceRange().isValid()) {
      const auto *CExpr = Match.getNodeAs<CXXConstructExpr>("construct-expr");
      ...
      if (const auto *TemplateDecl = dyn_cast<ClassTemplateSpecializationDecl>(
