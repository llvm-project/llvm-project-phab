...
static bool hasWriteback(uint32_t Instr) {
  return isLoadStoreImmediatePre(Instr) ||
         isLoadStoreImmediatePost(Instr) || isSTPPre(Instr) ||
         isSTPPost(Instr) || isST1SinglePost(Instr) ||
...
static bool is843419ErratumSequence(uint32_t Instr1, uint32_t Instr2,
  ...
  return isLoadStoreClass(Instr2) &&
         (isLoadStoreExclusive(Instr2) || isLoadLiteral(Instr2) ||
          isV8SingleRegisterNonStructureLoadStore(Instr2) ||
          isSTP(Instr2) || isSTNP(Instr2) || isST1(Instr2)) &&
