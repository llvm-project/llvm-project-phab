...
template <class ELFT> void SymbolTable::addFile(InputFile *File) {
  ...
  if (auto *F = dyn_cast<BinaryFile>(File)) {
    Files<ELFT>::BinaryFiles.push_back(F);
    F->parse<ELFT>();
    return;
  }
  ...
  if (auto *F = dyn_cast<SharedFile<ELFT>>(File)) {
    ...
    if (ErrorCount || !SoNames.insert(F->SoName).second)
      return;
    Files<ELFT>::SharedFiles.push_back(F);
    F->parseRest();
    return;
  }
  ...
  if (auto *F = dyn_cast<BitcodeFile>(File)) {
    Files<ELFT>::BitcodeFiles.push_back(F);
  ...
  Files<ELFT>::ObjectFiles.push_back(F);
...
template <class ELFT> void SymbolTable::addCombinedLTOObject() {
  if (Files<ELFT>::BitcodeFiles.empty())
    return;

  // Compile bitcode files and replace bitcode symbols.
  LTO.reset(new BitcodeCompiler);
  for (BitcodeFile *F : Files<ELFT>::BitcodeFiles)
    LTO->add(*F);
  ...
  for (InputFile *File : LTO->compile()) {
    ...
    Files<ELFT>::ObjectFiles.push_back(Obj);
  }
}
...
template <class ELFT> void SymbolTable::scanShlibUndefined() {
  for (SharedFile<ELFT> *File : Files<ELFT>::SharedFiles) {
