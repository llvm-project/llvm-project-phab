...
struct A64 {
  ...
  static bool isV8NonStructureLoad(uint32_t Instr) {
    ...
    if (A64::isLoadLiteral(Instr))
      return true;
    else if (A64::isV8SingleRegisterNonStructureLoadStore(Instr)) {
...
static bool LoadStoreWritesToReg(uint32_t Instr, uint32_t Reg) {
  return (A64::isV8NonStructureLoad(Instr) && A64::getRt(Instr) == Reg) ||
...
// and 4.) in the Scanner for Cortex-A53 errata comment above. The Adrp is
// assumed to have already been decoded as an ADRP instruction.
static bool is843419ErratumSequence(uint32_t Adrp, uint32_t Instr2,
  ...
  return A64::isLoadStoreClass(Instr2) &&
         (A64::isLoadStoreExclusive(Instr2) || A64::isLoadLiteral(Instr2) ||
          A64::isV8SingleRegisterNonStructureLoadStore(Instr2) ||
          A64::isSTP(Instr2) || A64::isSTNP(Instr2) || A64::isST1(Instr2)) &&
