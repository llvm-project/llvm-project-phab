...
namespace {
template<typename T = uint16_t> uint16_t calculateForLDI(uint16_t X, T Val) {
...
void AVR::relocateOne(uint8_t *Loc, uint32_t Type, uint64_t Val) const {
  ...
  case R_AVR_LO8_LDI_NEG: {
    int16_t SRel = Val;
    adjustNeg(SRel);
    write16le(Loc, calculateForLDI<int16_t>(X, SRel));
    break;
  }
  case R_AVR_HI8_LDI_NEG: {
    int16_t SRel = Val;
