...
scanEhFrameSection(EhInputSection &EH, ArrayRef<RelTy> Rels,
  ...
  // Assuming the worst case where there is a separate CIE for every FDE.
  Cies.reserve(EH.Pieces.size() / 2);
  for (EhSectionPiece &Piece : EH.Pieces) {
    ...
    if (ID == 0) {
      assert(Cies.empty() || Cies.back()->InputOff < Piece.InputOff);
      Cies.push_back(&Piece);
      continue;
    }
    ...
    const typename ELFT::uint PieceEnd = Piece.InputOff + Piece.size();
    for (unsigned I = Piece.FirstRelocation + 1, N = Rels.size(); I < N; ++I) {
    ...
    const uint32_t CieOffset = Piece.InputOff + 4 - ID;
