...
// Return a CIE referenced by a given FDE.
// Return nullptr if the CIE could not be found.
// Note: we could read ID from the FDE's data, but that would require
// this function to become a template; and anyway, ID has been already read
// in scanEhFrameSection().
...
static void scanCie(EhInputSection &EH, ArrayRef<RelTy> Rels,
  ...
  resolveReloc<ELFT>(EH, Rels[Cie.FirstRelocation], Fn);
}
