...
static int getRankProximity(OutputSection *A, BaseCommand *B) {
  if (auto *Cmd = dyn_cast<OutputSection>(B))
    if (Cmd->Live)
      return getRankProximityAux(A, Cmd);
  return -1;
}
...
findOrphanPos(std::vector<BaseCommand *>::iterator B,
  ...
  for (; I != E; ++I) {
    auto *Cmd = dyn_cast<OutputSection>(*I);
    if (!Cmd || !Cmd->Live)
      continue;
    if (getRankProximity(Sec, Cmd) != Proximity ||
        Sec->SortRank < Cmd->SortRank)
      break;
  }
  auto J = std::find_if(
...
template <class ELFT> void Writer<ELFT>::sortSections() {
  ...
  for (BaseCommand *Base : Script->Opt.Commands)
    if (auto *Cmd = dyn_cast<OutputSection>(Base))
      Cmd->SortRank = getSectionRank(Cmd);
  ...
  auto NonScriptI = std::find_if(I, E, [](BaseCommand *Base) {
    if (auto *Cmd = dyn_cast<OutputSection>(Base))
      return Cmd->Live && Cmd->SectionIndex == INT_MAX;
    return false;
  });
  ...
  for (InputSectionBase *S : llvm::reverse(InputSections)) {
    ...
    if (OS->Commands.empty()) {
      ...
      auto I = std::remove_if(Cmds.begin(), Cmds.end(), [&](BaseCommand *Cmd2) {
        if (auto *OSCmd = dyn_cast<OutputSection>(Cmd2))
          return OSCmd == OS;
        return false;
      });
