...
private:
  ...
  std::vector<DefinedRegular *> SectionStartSymbols;
...
Writer<ELFT>::addOptionalRegular(StringRef Name, SectionBase *Sec, uint64_t Val,
  ...
  DefinedRegular *Sym = cast<DefinedRegular>(
      Symtab->addRegular<ELFT>(Name, StOther, STT_NOTYPE, Val, /*Size=*/0,
  ...
  // section size yet. Therefore we save the symbol and the section now and
  // update the size once we know the final size of the output section
  if (Sec && Val == 0)
    SectionStartSymbols.push_back(Sym);
  return Sym;
}
...
template <class ELFT> void Writer<ELFT>::finalizeSections() {
  ...
  if (InX::DynSymTab) {
    Symbol *Sym = Symtab->addRegular<ELFT>("_DYNAMIC", STV_HIDDEN, STT_NOTYPE,
                                           0 /*Value*/, /*Size=*/0, STB_WEAK,
                                           InX::Dynamic, /*File=*/nullptr);
    // We don't know the final size of the dynamic section yet so defer setting
    // the size of the output section
    SectionStartSymbols.push_back(cast<DefinedRegular>(Sym));
  }
  ...
  for (DefinedRegular* Sym : SectionStartSymbols)
    if (Sym->Section)
      if (OutputSection* OS = Sym->Section->getOutputSection())
        Sym->Size = OS->Size;
}
