...
template <class ELFT> static bool needsInterpSection() {
  return !Files<ELFT>::SharedFiles.empty() && !Config->DynamicLinker.empty() &&
...
template <class ELFT> void Writer<ELFT>::createSyntheticSections() {
  ...
  bool HasDynSymTab =
      !Files<ELFT>::SharedFiles.empty() || Config->Pic || Config->ExportDynamic;
...
template <class ELFT> void Writer<ELFT>::copyLocalSymbols() {
  if (!InX::SymTab)
    return;
  for (elf::ObjectFile<ELFT> *F : Files<ELFT>::ObjectFiles) {
...
template <class ELFT> static void sortBySymbolsOrder() {
  ...
  for (elf::ObjectFile<ELFT> *File : Files<ELFT>::ObjectFiles) {
