...
static uint32_t tailHash(StringRef S) {
  if (S.size() < 4)
    return 0;
  const uint8_t *P = (const uint8_t *)S.data() + S.size();
  return hash_value((P[-4] << 24) | (P[-3] << 16) | (P[-2] << 8) | P[-1]);
}
...
void MergeTailSection::finalizeContents() {
  ...
  parallelForEach(Sections, [&](MergeInputSection *Sec) {
    for (size_t I = 0, E = Sec->Pieces.size(); I != E; ++I)
      if (Sec->Pieces[I].Live)
        Sec->Pieces[I].TailHash = tailHash(Sec->getData(I).val());
  });
  ...
  parallelForEachN(0, Concurrency, [&](size_t ThreadId) {
    for (MergeInputSection *Sec : Sections) {
      for (size_t I = 0, E = Sec->Pieces.size(); I != E; ++I) {
        if (!Sec->Pieces[I].Live)
          continue;
        size_t ShardId = Sec->Pieces[I].TailHash % NumShards;
        if ((ShardId & (Concurrency - 1)) == ThreadId)
