...
OutputSection *LinkerScript::createOutputSection(StringRef Name,
  ...
  if (CmdRef && CmdRef->Location.empty()) {
    ...
    Cmd = make<OutputSection>(Name, SHT_PROGBITS, 0);
    if (!CmdRef)
      CmdRef = Cmd;
  }
...
void LinkerScript::processCommands(OutputSectionFactory &Factory) {
  ...
  for (size_t I = 0; I < Opt.Commands.size(); ++I) {
    ...
    if (auto *Cmd = dyn_cast<OutputSection>(Opt.Commands[I])) {
      ...
      for (InputSectionBase *S : V)
        Factory.addInputSec(S, Cmd->Name, Cmd);
      assert(Cmd->SectionIndex == INT_MAX);
      Cmd->SectionIndex = I;
      if (Cmd->Noload)
        Cmd->Type = SHT_NOBITS;
    }
...
void LinkerScript::addOrphanSections(OutputSectionFactory &Factory) {
  ...
  for (InputSectionBase *S : InputSections) {
    ...
    if (I == End) {
      ...
      OutputSection *Cmd = cast<OutputSection>(*I);
      Factory.addInputSec(S, Name, Cmd);
      unsigned Index = std::distance(Opt.Commands.begin(), I);
      assert(Cmd->SectionIndex == INT_MAX || Cmd->SectionIndex == Index);
      Cmd->SectionIndex = Index;
    }
  }
}
...
void LinkerScript::adjustSectionsAfterSorting() {
  ...
  for (BaseCommand *Base : Opt.Commands) {
    if (auto *Cmd = dyn_cast<OutputSection>(Base)) {
      ...
      if (Cmd->AlignExpr)
        Cmd->updateAlignment(Cmd->AlignExpr().getValue());
    }
  }
...
void LinkerScript::assignAddresses() {
  ...
  for (BaseCommand *Base : Opt.Commands) {
    ...
    auto *Cmd = cast<OutputSection>(Base);
    assignOffsets(Cmd);
  }
  CurAddressState = nullptr;
}
