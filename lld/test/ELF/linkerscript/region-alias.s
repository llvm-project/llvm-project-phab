...
# RUN: not ld.lld %t --script %t.script -o %t2 2>&1 | FileCheck %s --check-prefix=ERR1
# ERR1: {{.*}}script.inc:1: redefinition of memory region 'ROM'

## Redefinition of alias.
# RUN: echo "REGION_ALIAS (\"ALIAS_TEXT\", ROM);" > %t.script.inc
# RUN: echo "REGION_ALIAS (\"ALIAS_TEXT\", ROM);" >> %t.script.inc
# RUN: not ld.lld %t --script %t.script -o %t2 2>&1 | FileCheck %s --check-prefix=ERR2
...
# RUN: not ld.lld %t --script %t.script -o %t2 2>&1 | FileCheck %s --check-prefix=ERR3
