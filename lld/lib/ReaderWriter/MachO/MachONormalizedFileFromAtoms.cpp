...
private:
  ...
  void fixLazyReferenceImm(const DefinedAtom *atom, uint32_t offset, NormalizedFile &file);
    ...
    for (const AtomInfo &info : sect->atomsAndOffsets) {
      ...
      for (const Reference *ref : *atom) {
        ...
        if (_archHandler.isPointer(*ref)) {
          ...
          // 5 bytes for opcodes + variable sizes (target name + \0 and offset encode's size)
          offsetInBindInfo += 6 + targ->name().size() + llvm::getULEB128Size(bind.segOffset);
...
void Util::fixLazyReferenceImm(const DefinedAtom *atom, uint32_t offset, NormalizedFile &file) {
  for (const auto &ref: *atom) {
    const DefinedAtom *da = dyn_cast<DefinedAtom>(ref->target());
    if (da == nullptr)
      return;

    auto helperRef = std::find_if(*da->begin(), *da->end(), [&](const Reference &hr) -> bool {
      return hr.kindValue() == _archHandler.lazyImmediateLocationKind();
    });
    ...
    for (SectionInfo *sectInfo : _sectionInfos) {
      for (const AtomInfo &atomInfo : sectInfo->atomsAndOffsets) {
        if (atomInfo.atom == helperRef->target()) {
          auto sectionContent = file.sections[sectInfo->normalizedSectionIndex].content;
          uint8_t *rawb = file.ownedAllocations.Allocate<uint8_t>(sectionContent.size());
          llvm::MutableArrayRef<uint8_t> newContent{rawb, sectionContent.size()};
          std::copy(sectionContent.begin(), sectionContent.end(), newContent.begin());
          llvm::support::ulittle32_t *loc =
              reinterpret_cast<llvm::support::ulittle32_t *>(&newContent[atomInfo.offsetInSection
