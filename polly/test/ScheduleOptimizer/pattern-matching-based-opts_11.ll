...
; RUN: -polly-target-2nd-cache-level-size=262144 -polly-parallel < %s 2>&1 \
; RUN: | FileCheck %s
;
;    /* C := alpha*A*B + beta*C */
;    for (i = 0; i < _PB_NI; i++)
...
;        }
;
; This test case checks whether Polly detects the expected parallel loops
; in case of GEMM and the pattern matching optimizations.
;
