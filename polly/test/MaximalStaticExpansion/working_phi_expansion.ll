; RUN: opt %loadPolly -polly-mse -analyze < %s 2>1| FileCheck %s
;
...
;   }
;
; Check that the SAI are created.
;
...
;
; CHECK: new: { Stmt_for_body[i0] -> MemRef_tmp_04__phi_Stmt_for_body_expanded[i0] };
...
; CHECK: new: { Stmt_for_end[i0] -> MemRef_tmp_04__phi_Stmt_for_body_expanded[1 + i0] : i0 <= 9998 };
