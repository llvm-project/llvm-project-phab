...
void Scop::foldSizeConstantsToRight() {
   isl::union_set Accessed = isl::manage(getAccesses.release());
   for (auto Array : arrays()) {
      if (Array->getNumberOfDimensions() <= 1)
        continue;
  
      isl::space Space = isl::manage(Array->getSpace());
      ...
      for (int i = 0; i < Dims; i++) {
        isl::set DimOnly =
            Elements.project_out(isl::dim::set, 0, i);
       ...
       if (DimHull.dim(isl::dim::div) == 1) {
          ...
          isl::constraint C = C.alloc_equality(isl::local_space::local_space(Transform.get_space()));
