...
static isl_stat makeStmtSingleValued(__isl_take isl_map *MapToAdd, void *User) {
  ...
  SourceDomain = CurrentStmt->getDomain();
  DEBUG(dbgs() << "Original Domain:" << SourceDomain << "\n");
  ...
  while (!isl_map_is_empty(MapToAdd)) {
    ...
    if (i != 0) {
      ...
      Domain = isl_set_intersect(Domain, isl_set_copy(SourceDomain));
      Domain = isl_set_set_tuple_name(Domain, Name.c_str());
      DEBUG(dbgs() << "New Domain:" << Domain << "\n");
