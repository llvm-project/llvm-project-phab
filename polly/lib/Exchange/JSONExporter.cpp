...
static isl_stat copyScopStatements(__isl_take isl_set *set, void *user){
  ...
  data->S->copyScopStmt(NewStmt, data->CopyNo);
...
isl_union_map *makeScheduleSingleValued(isl_union_map *ScheduleMap, Scop &S){
	...
	while(!isl_union_map_is_empty(ScheduleDup)){
		...
		ScopData.CopyNo = i;
		assert(isl_union_set_foreach_set(domain, &copyScopStatements, &ScopData) >= 0 &&
				"error during foreach for copyScopStatements!");

		// fix the schedule
		ScheduleData.CopyNo = i;
		if (isl_union_set_foreach_set(domain, &copyScheduleStatements, &ScheduleData) >= 0 &&
				"error during foreach for copyScheduleStatements!");
