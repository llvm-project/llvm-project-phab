	...
	for (int i = 0; i < expr->u.op.n_arg; i++) {
		if (expr->u.op.args[i]->type != isl_ast_expr_bound_t) {
			if (expr->u.op.args[i]->type == isl_ast_expr_op) {
				expr->u.op.args[i] = isl_ast_bin_op_set_size(expr->u.op.args[i], build);
			}
		}
	}
	switch (expr->u.op.op) {
...
static __isl_give isl_ast_expr_constructor *isl_ast_expr_constructor_mul(
	__isl_take isl_ast_expr_constructor *a, __isl_take isl_val *b,
	__isl_keep isl_ast_build *build)
{
	isl_ast_expr *b_expr;
	...
	b_expr = isl_ast_expr_from_val(isl_val_copy(b));

	a->expr = isl_ast_expr_mul(b_expr, a->expr);
	a->aff = isl_aff_scale_val(a->aff, b);
	isl_ast_expr_constructor_set_size(a, build);
	...
	__isl_take isl_constraint *constraint, __isl_keep isl_ast_build *build)
{
	int i, n;
	isl_ctx *ctx;
	isl_ast_expr *expr_pos, *expr_pos_t;
	isl_ast_expr *expr_neg, *expr_neg_t;
	isl_ast_expr *expr;
