...
__isl_give isl_ast_expr *isl_ast_bin_op_set_size_max(__isl_take isl_ast_expr *expr,
	...
	if (expr->u.op.args[0]->type == isl_ast_expr_bound_t &&
		...
		if (flag == 1) {
			...
			if (expr->u.op.args[1]->u.bound.size > bits) {
			...
			} else if (!is_signed && expr->u.op.args[1]->u.bound.is_signed) {
				is_signed = expr->u.op.args[1]->u.bound.is_signed;
				if (bits == expr->u.op.args[1]->u.bound.size)
					bits++;
			}
		} else {
			// Result is minimum type
			if (expr->u.op.args[1]->u.bound.size < bits ||
			    (expr->u.op.args[1]->u.bound.size == bits &&
			     !expr->u.op.args[1]->u.bound.is_signed)) {
				bits = expr->u.op.args[1]->u.bound.size;
				is_signed = expr->u.op.args[1]->u.bound.is_signed;
			}
		}
	...
	if (build->approximate_bounds) {
		...
		if (expr->type == isl_ast_expr_id) {
			...
			switch (expr->u.op.op) {
				...
				// Default: Fall throught to solver
			}
		}
	}
	...
	__isl_keep isl_ast_build *build)
{
	...
	for (i = 0; i < n; ++i) {
		...
		if (!expr_i)
			goto error;
		isl_ast_build_set_size(build, expr_i, aff);
