...
void GPUNodeBuilder::createKernel(__isl_take isl_ast_node *KernelStmt) {
  ...
  Value *KernelSize = Builder.getInt64(ASMString.length() + 1);
...
std::string GPUNodeBuilder::createKernelASM() {
  ...
  if (Arch == GPUArch::AMDGCN64) {
    ...
    if (TargetM->addPassesToEmitFile(PM, ASMStream,
  ...
  if (Arch == GPUArch::AMDGCN64) {
    char InFileName[] = "/tmp/polly_kernelXXXXXX";
    char OutFileName[] = "/tmp/polly_kernelObjXXXXXX";
    mkstemp(InFileName);
    mkstemp(OutFileName);

    std::ofstream ASMFile;
    ASMFile.open(InFileName);
    ASMFile << Assembly;
    ASMFile.close();

    std::string LLDCommand = "ld.lld -shared " + std::string(InFileName) +
                             " -o " + std::string(OutFileName);
    system(LLDCommand.c_str());

    std::ifstream ifs(OutFileName);
    Assembly.assign((std::istreambuf_iterator<char>(ifs)),
