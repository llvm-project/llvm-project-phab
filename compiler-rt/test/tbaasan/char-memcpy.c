// RUN: %clang_tbaasan -O0 %s -o %t && %run %t >%t.out.0 2>&1
// RUN: FileCheck %s < %t.out.0
// RUN: %clang_tbaasan -O2 %s -o %t && %run %t >%t.out 2>&1
// RUN: FileCheck %s < %t.out

#include <stdio.h>

// There's no TBAA violation here: the memcpy is implemented using only char*
// or unsigned char* (both of which may alias anything).
