// RUN: %clangxx_tbaasan -O0 %s -c -o %t.o && \
// RUN:   %clangxx_tbaasan -O0 %s -DPMAIN -c -o %tm.o && \
// RUN:   %clangxx_tbaasan -O0 %t.o %tm.o -o %t && %run %t >%t.out 2>&1
...
void foo(void *context, int i) {
  X *x = (X *) context;
  x->b = i;
// CHECK: ERROR: TBAASanitizer: type-aliasing-violation
