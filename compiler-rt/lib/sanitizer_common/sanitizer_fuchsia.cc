...
u64 NanoTime() {
  return _mx_time_get(MX_CLOCK_UTC);
}

uptr internal_getpid() {
  mx_info_handle_basic_t info;
  mx_status_t status = _mx_object_get_info(_mx_process_self(),
...
void SleepForMillis(int millis) {
  internal_nanosleep(MX_MSEC(millis));
}
...
_Unwind_Reason_Code Unwind_Trace(struct _Unwind_Context *ctx, void *param) {
  ...
  return (arg->stack->size == arg->max_depth ?
...
static void *DoAnonymousMmapOrDie(uptr size, const char *mem_type,
  ...
  if (status != MX_OK) {
    if (status != MX_ERR_NO_MEMORY || die_for_nomem)
      ReportMmapFailureAndDie(size, mem_type, "mx_vmo_create",
                              status, raw_report);
    return nullptr;
  }
  _mx_object_set_property(vmo, MX_PROP_NAME,
                          mem_type, internal_strlen(mem_type));
  ...
  if (status != MX_OK) {
    if (status != MX_ERR_NO_MEMORY || die_for_nomem)
      ReportMmapFailureAndDie(size, mem_type, "mx_vmar_map",
                              status, raw_report);
    return nullptr;
  }
...
void *MmapNoAccess(uptr size) {
  ...
  uintptr_t base;
  mx_status_t status = _mx_vmar_allocate(_mx_vmar_root_self(), 0, size,
...
static void *DoMmapFixedOrDie(uptr fixed_addr, uptr size, bool die_for_nomem) {
  ...
  if (status != MX_OK) {
    if (status != MX_ERR_NO_MEMORY || die_for_nomem)
      ReportMmapFailureAndDie(size, kAllocatorVmoName,
                              "mx_vmo_create", status);
    return nullptr;
  }
  _mx_object_set_property(vmo, MX_PROP_NAME,
                          kAllocatorVmoName, sizeof(kAllocatorVmoName) - 1);
  ...
  uintptr_t addr;
  status = _mx_vmar_map(allocator_vmar, offset, vmo, 0, size,
...
void *MmapAlignedOrDieOnFatalError(uptr size, uptr alignment,
  ...
  if (status == MX_OK) {
    ...
    if (addr != map_addr) {
      mx_info_vmar_t info;
      status = _mx_object_get_info(_mx_vmar_root_self(), MX_INFO_VMAR,
                                   &info, sizeof(info), NULL, NULL);
      if (status == MX_OK) {
        uintptr_t new_addr;
        status = _mx_vmar_map(_mx_vmar_root_self(), addr - info.base,
                              vmo, 0, size,
                              MX_VM_FLAG_PERM_READ | MX_VM_FLAG_PERM_WRITE |
