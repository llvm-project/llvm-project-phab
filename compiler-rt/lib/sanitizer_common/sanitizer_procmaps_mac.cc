...
MemoryMappingLayout::MemoryMappingLayout(bool cache_enabled) {
  data_ = static_cast<MemoryMappingLayoutData *>
  (MmapOrDie(sizeof(MemoryMappingLayoutData), "MemoryMappingLayoutData", 0));
  Reset();
}

MemoryMappingLayout::~MemoryMappingLayout() {
  UnmapOrDie(data_, sizeof(MemoryMappingLayoutData));
}
...
MemoryMappingLayoutData *layout_data) {
  ...
  if (((const load_command *)lc)->cmd == kLCSegment) {
    ...
    segment->offset =
        (layout_data->current_filetype ==
     /*MH_EXECUTE*/ 0x2) ? sc->vmaddr : sc->fileoff;
    if (segment->filename) {
...
bool MemoryMappingLayout::Next(MemoryMappedSegment *segment) {
  for (; data_->current_image >= kDyldImageIdx; data_->current_image--) {
    ...
    if (data_->current_load_cmd_count < 0) {
      ...
      data_->current_arch =
      ModuleArchFromCpuType(hdr->cputype, hdr->cpusubtype);
      ...
      FindUUID((const load_command *)
      data_->current_load_cmd_addr, data_->current_uuid);
      data_->current_instrumented =
      IsModuleInstrumented((const load_command *)data_->current_load_cmd_addr);
    }

    for (; data_->current_load_cmd_count >= 0;
    data_->current_load_cmd_count--) {
      switch (data_->current_magic) {
