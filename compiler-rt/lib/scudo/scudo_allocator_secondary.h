...
class ScudoLargeMmapAllocator {
 public:
  ...
  void *Allocate(AllocatorStats *Stats, uptr Size, uptr Alignment) {
    ...
    CHECK_EQ(MapBeg + PageSize, AddressRange.Map(MapBeg + PageSize,
                                                 MapSize - 2 * PageSize));
  ...
  void Deallocate(AllocatorStats *Stats, void *Ptr) {
    ...
    {
      SpinMutexLock l(&StatsMutex);
      Stats->Sub(AllocatorStatAllocated, StoredRange->size() - 2 * PageSize);
      Stats->Sub(AllocatorStatMapped, StoredRange->size() - 2 * PageSize);
    }
    // Since we're unmapping the entirety of where StoredRange actually is,
    // copy onto the stack first, so we don't get AVs while updating base and
    // size.
    ReservedAddressRange AddressRange;
    AddressRange = *StoredRange;
    AddressRange.Unmap(reinterpret_cast<uptr>(StoredRange->base()),
                       StoredRange->size());
  }
 ...
 private:
  ReservedAddressRange *getReservedAddressRange(uptr Ptr) {
    return reinterpret_cast<ReservedAddressRange*>(Ptr - sizeof(
  ...
  const uptr HeadersSize = AlignedReservedAddressRangeSize +
