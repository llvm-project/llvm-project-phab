...
//
// Platform specific information for TBAASan.
//===----------------------------------------------------------------------===//

#ifndef TBAASAN_PLATFORM_H
#define TBAASAN_PLATFORM_H
...
# define TBAASAN_RUNTIME_VMA 1
#else
# error "TBAASan not supported for this platform!"
#endif

#if TBAASAN_RUNTIME_VMA
extern int vmaSize;
#endif
...
#elif defined(__powerpc64__)
  if (vmaSize == 44)
    return MappingImpl<Mapping44>(mem);
  else
    return MappingImpl<Mapping46>(mem);
  DCHECK(0);
#else
