...
XRayLogFlushStatus fdrLoggingFlush() XRAY_NEVER_INSTRUMENT {
  ...
  BQ->apply([&](const BufferQueue::Buffer &B) {
    ...
    // the records identified by the extents of the buffer. We know that the
    // third metadata record for every buffer must be a MetadataRecord
    // identifying how many bytes from the start of the buffer should be
    // recorded, so we read that record and use that as the number of bytes we
    // ought to write.
    auto &ExtentsRecord = reinterpret_cast<MetadataRecord *>(B.Buffer)[2];
    size_t BufferSize = 0;
    std::memcpy(&BufferSize, ExtentsRecord.Data, sizeof(size_t));
    assert(BufferSize <= B.Size);
    if (BufferSize > 0)
      retryingWriteAll(Fd, reinterpret_cast<char *>(B.Buffer),
                       reinterpret_cast<char *>(B.Buffer) + BufferSize);
  });
