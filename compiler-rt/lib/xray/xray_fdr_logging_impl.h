...
struct alignas(64) ThreadLocalData {
  ...
  // This points to the Extents record we've written at the beginning of the
  // buffer, which should give us a way of tracking the number of *bytes* we've
  // written to this particular buffer.
  MetadataRecord *ExtentsRecord = nullptr;
};
...
static void writeNewBufferPreamble(pid_t Tid,
  ...
  // As of version 2 of the FDR log, we use a metadata record to also keep track
  // of the extents of the log.
  {
    auto &ExtentsRecord = Records[2];
    ExtentsRecord.Type = uint8_t(RecordType::Metadata);
    ExtentsRecord.RecordKind =
        uint8_t(MetadataRecord::RecordKinds::BufferExtents);

    // For now we assume we're going to write the size of the three records
    // (including this record) as the extent of this buffer.
    constexpr size_t Extent = sizeof(MetadataRecord) * 3;
    std::memcpy(ExtentsRecord.Data, &Extent, sizeof(Extent));
  }

  TLD.NumConsecutiveFnEnters = 0;
  TLD.NumTailCalls = 0;

  // We need to keep track of the record holding the extents, because we're
  // going to be updating that regularly, everytime we write new records.
  TLD.Extents = sizeof(MetadataRecord) * 3;
  if (TLD.BQ && !TLD.BQ->finalizing()) {
    std::memcpy(TLD.RecordPtr, Records,
                sizeof(MetadataRecord) * InitRecordsCount);
    TLD.ExtentsRecord = reinterpret_cast<MetadataRecord *>(TLD.RecordPtr) + 2;
...
static void incrementExtents(size_t Add) {
  auto &TLD = getThreadLocalData();
  TLD.Extents += Add;
  std::memcpy(TLD.ExtentsRecord->Data, &TLD.Extents, sizeof(TLD.Extents));
}

static void decrementExtents(size_t Subtract) {
  auto &TLD = getThreadLocalData();
  assert(TLD.Extents > Subtract);
  TLD.Extents -= Subtract;
  std::memcpy(TLD.ExtentsRecord->Data, &TLD.Extents, sizeof(TLD.Extents));
}
