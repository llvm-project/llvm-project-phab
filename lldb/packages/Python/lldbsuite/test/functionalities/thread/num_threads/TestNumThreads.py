...
class NumberOfThreadsTestCase(TestBase):
    ...
    def test_unique_stacks(self):
        ...
        for i in range(num_threads):
            ...
            if not "thread3" in thread.GetFrameAtIndex(0).GetFunctionName():
                continue
            ...
            # Resume can fail so only include this in the expected result if we failed
            if thread.GetStopReason() == lldb.eStopReasonBreakpoint:
                threads_found += 1
                expect_threads += " #%d"%(i+1)

        # Construct our expected back trace string
        expect_string = "%d thread(s)%s" % (threads_found, expect_threads)
